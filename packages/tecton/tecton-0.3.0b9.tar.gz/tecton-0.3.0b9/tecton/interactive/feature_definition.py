...
class FeatureDefinition(Fco):
    ...
    def _delete_keys(
        ...
        if isinstance(keys, pd.DataFrame):
            ...
            keys = keys.to_spark()
        utils.validate_entity_deletion_keys_dataframe(df=keys, join_keys=fd.join_keys)

        if online:
            # Create async deletion for online store
        ...
        if offline:
            # Sync deletion for offline store
            if all([not x.offline_enabled for x in self._proto.materialization_state_transitions]):
            ...
            offline_store_writer = get_offline_store_writer(
                offline_store_params, fd.offline_store_config, fd.get_feature_store_format_version, spark
            )
            offline_store_writer.delete_keys(keys)
        return None
