...
def get_historical_features(
    ...
    for item in feature_set_items:
        ...
        if not fd.is_on_demand:
            ...
            if from_registered_view:
                source = f"SELECT * FROM {feature_view.enrichments.snowflake_enrichments.snowflake_view_name}"
            else:
    ...
    for item in feature_set_items:
        ...
        if fd.is_on_demand:
            ...
            assert len(view_schema.column_names()) == 1, f"Schema should have 1 column: {view_schema}"
            output_col_name = view_schema.columns[0]
            output_df = pipeline_to_df_with_input(
                session=session,
                input_df=output_df,
                pipeline=fd.pipeline,
                transformations=fd.transformations,
                output_type=view_schema.spark_type(output_col_name),
...
def run_batch(
    ...
    if feature_view.HasField("temporal_aggregate"):
        if aggregation_level == "full":
            ...
            sql_str = TIME_LIMIT_TEMPLATE.render(
                source=feature_view.enrichments.snowflake_enrichments.snowflake_view_name,
                timestamp_key=feature_view.timestamp_key,
