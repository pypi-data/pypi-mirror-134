...
def transform_geom(
    proj: pyproj.transformer.Transformer, geom: shapely.geometry
) -> shapely.geometry:
...
def get_local_catchment(x: float, y: float) -> tuple[int, shapely.geometry]:
    ...
    if (
        ...
        while r < len(features["geometry"]["coordinates"]):
            ...
            catchment_geom.append(Polygon(features["geometry"]["coordinates"][r][0]))
            r += 1
...
def get_local_flowline(catchment_id: int) -> tuple[dict, shapely.geometry]:
...
def get_total_basin(catchment_id: int) -> shapely.geometry:
    ...
    try:
        r = requests.get(NLDI_URL + catchment_id + "/basin", params=payload)
...
def get_coordsys() -> tuple[
    pyproj.transformer.Transformer, pyproj.transformer.Transformer
]:
...
def project_point(
    x: float, y: float, transform_to_raster: pyproj.transformer.Transformer
) -> shapely.geometry:
...
def get_flowgrid(
    ...
    transform_to_raster: pyproj.transformer.Transformer,
) -> tuple[pyflwdir.pyflwdir.FlwdirRaster, pyproj.transformer.Transformer]:
...
def split_catchment(
    ...
    transform_to_raster: pyproj.transformer.Transformer,
    transform_to_wgs84: pyproj.transformer.Transformer,
) -> shapely.geometry:
...
def get_intersection_point(
    ...
    *raindrop_path: shapely.geometry.linestring.LineString,
    ...
    if on_flowline is False:
        ...
        coord_id = len(raindrop_path.coords) - 1
        intersection_point = Point(
            raindrop_path.coords[coord_id][0], raindrop_path.coords[coord_id][1]
...
def split_flowline(
    intersection_point: shapely.geometry.point.Point, flowline: dict
) -> tuple[
