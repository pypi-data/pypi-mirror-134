...
class _TestInfo(object):
    ...
    def get_description(self):
        name = self.test_method.id().split(".")[-1]
        doc = getattr(self.test_method, "_testMethodDoc")
...
class HtmlTestResult(TextTestResult):
    ...
    def addUnexpectedSuccess(self, test):
        ...
        self._prepare_callback(
            self.infoclass(self, test, self.infoclass.XPASS),
    ...
    def get_results_summary(self, tests):
        ...
        failures = (
            errors
        ) = skips = successes = expected_failure = unexpected_success = 0
        for test in tests:
