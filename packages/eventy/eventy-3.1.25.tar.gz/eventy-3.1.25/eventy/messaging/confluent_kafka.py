...
class CKStore(RecordStoreWithInitialization):
    ...
    def read(
        ...
        for message in messages:
            ...
            logger.info(
                f"New record {record.qualified_name} from kafka message {message.topic()}[{message.partition()}]:{message.offset()}."
                )
            yield record
            if auto_ack:
                try:
                    ...
                    raise MessagingError(f"Error committing message {message.topic()}[{message.partition()}]:{message.offset()}.")
