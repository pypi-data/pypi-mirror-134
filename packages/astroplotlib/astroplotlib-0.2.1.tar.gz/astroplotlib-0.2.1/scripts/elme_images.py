...
# This script plot a figure with several caratheristic, like, contours, 
# scales, diferent color map, etc.
...
try :
    ...
    print '      # scale image units/pix (e.g, 0.146 arcsec/pix, 75 kpc/arcsec) '
...
scale1  = float(finput[i].split()[0].split(',')[0])                  # scale
units1  = finput[i].split()[0].split(',')[1];i+=1                    # units
...
for i in arange(len(image)-1):
    ...
    figure(image[i], cent, delta, [limt[i*3], limt[i*3+1], limt[i*3+2]], cmap, 
           scale=scale1, figtitle=titleg[i],  
           nameaxis=[labaxi[i*2], labaxi[i*2+1]], pgrid=[True,'k'], AX=ax)
           
ax = fig.add_subplot(subp[0], subp[1],len(image))

figure(image[-1], cent,delta, [limt[-3], limt[-2], limt[-1]], cmap, 
