...
Label(root, text="(0,file,col_X,col_Y) or (1,file,col_RA,col_DEC) " + 
      "or (2,gal,cat,size_X,size_Y,Fmag<22,jmag<22)").grid(row=rowl, 
      column=exp, sticky=W)
...
Label(root, text="Radius of the circle, color format, and" + 
                 " linewidth").grid(row=rowl,column=exp,sticky=W)
...
Label(root, text="image scale, unit").grid(row=rowl,column=exp,sticky=W)
...
def go(event=None):
    ...
    units = var11.get().split()[0].split(',')[1]           
...
def fsave(event=None):
    ...
    fs.write(('%s          # wcs coord or pix coord or no ' + 
             '(not print) \n')%(var4.get()))
    fs.write('%s          # scale (arcsec/pix,kpc/pix), units\n'
             %(var11.get()))
...
def unlearn() :
    ...
    var11.set('1.0,pix') # scale (arcsec/pix,kpc/pix), units
