...
def ellipse (x0, y0, sma, eps, pa, step=400,
        ...
        # radis array
        rad = np.sqrt(np.square(a*b)/(np.square(b*np.cos(th)) + np.square(a*np.sin(th))))    
...
def image_orientation(image):
    ...
    print (('PA of N and E axes with respect x+ axis measured in ' +
            ...
            np.fmod(np.rad2deg(thn), 360), np.fmod(rad2deg(the), 360)))
...
def zetas(image, delta, scale, parr='3', larrow=5.0, nx=2.0, ny=2.0, lhat=1.16,
    ...
    print (('PA N and E axis with respect x+ axis in ' +
           'counter-closewise {:.1f} {:.1f}').format(np.fmod(np.rad2deg(thn), 360), 
...
def figure(image, cent=[0,0], delta=[0,0], limt=[0,0,0], cmap=['jet'],
           units='pix', scale=1.0, outima='none', figtitle='none', 
           ...
           barinvt='no', 
           subplot1=None, szplot=None, aextend='on',
           interpolation = 'none', alpha=1,
           aspect='equal', AX=None):
    ...
    Nivels : Let user choose the contour nivels ([n1,n2,n3,...,nf])
    sigmag : Standard deviation for Gaussian kernel. The standard deviations of 
    ...
    contxt : manual labels for contours ['1 sig','2 sig']
    cbar   : int,
           by default 0,  
           We have three option to plot the color bar:
            (i) Traditional one
            (ii) One that "dealing colorbar size with all types of axes is",
                 source:
                 https://stackoverflow.com/questions/18195758/ \ 
                     set-matplotlib-colorbar-size-to-match-graph
            (iii) Sugest by http://joseph-long.com/writing/colorbars/
    cbar   : plot color bar ([orientation='vertical' or 'horizotal',shrink=1.0,
             pad=0.0,format='%.2f'])
    ...
    cbarl  : bar label
    splot  : Subplot option ([numRows, numCols, plotNum])
    szplot : Figure size ([width,heigth,idfig])  
    AX     : object,
             Image object or None.
    ...
    #if szplot is not None: 
    #   fig  = plt.figure(int(szplot[2]),figsize=(float(szplot[0]), 
    #                  float(szplot[1]))) 
    #elif AX is None:
    #   fig = plt.figure()
  
    #if subplot1 is not None: 
    #   AX = fig.add_subplot(subplot1[0],subplot1[1],subplot1[2])         

    #if subplot1 is not None and AX is None: 
    ...
    if cont != 'none':
        ...
        if sigmag != 0:
            smooth = filters.gaussian_filter(temp, sigmag)
        else:
            smooth = np.copy(temp)
...
def calcu(poly, ima_xy, ima_array, verbose='+'):
    ...
    if len(skyv)!=0: 
        statsky = [np.mean(skyv), np.median(skyv), np.std(skyv),
                         len(skyv), np.amin(skyv), np.amax(skyv), 
                         np.sum(skyv)]
        if verbose == '+': 
...
def histo(ax, data, nbin, outima=None, titlehist=None, xlab='Counts', 
    ...
    if dist != 'none':
       ...
       if dist == 'gauss':
            gauss_norm = (1./(sig*sqrt(2.0*pi)))*exp((((x-cent)/sig)**2)*-0.5)
            if norm: 
                y=gauss_norm
            else: 
                y = Area*gauss_norm
