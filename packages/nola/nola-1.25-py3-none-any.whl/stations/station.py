...
def completeness_metric(series,  truth_df, expected_df, *args, **kwargs):
    ...
    num_missing = len([each for each in expected_vals if each not in series.index])
...
def validity_nondecreasing_smoothness(series,  truth_df, expected_df, *args, **kwargs):
    """
    Assumes series is of increasing dates by nondecreasing observations
    ...
    max_val = obsvs.iloc[-1][0]
    num_expected_dates = expected_df[expected_df.iloc[:, 0] < max_date].shape[0]
    total_area = max_val * num_expected_dates
    covered_area = sum([rise * run for rise, run in zip(rises, runs)])
...
def validity_nondecreasing(series, *args, **kwargs):
    ...
    return len([each for each in series[series.notna()].diff() if each < 0])
