...
from dftt_dispatch import InstanceMethodDispatch
from error import DFTTTimecodeValueError, DFTTTimecodeTypeError
from pattern import *
...
class DfttTimecode:
    ...
    def _(self, timecode_value, timecode_type='auto', fps=24.0, drop_frame=False, strict=True):
        if timecode_value[0] == '-':
            ...
            minus_flag = False
        self.__fps = fps
        self.__nominal_fps = ceil(fps)  # 读入帧率取整为名义帧率便于后续计算
        self.__drop_frame = drop_frame
        self.__strict = strict
        if round(self.__fps, 2) % 29.97 != 0 and self.__drop_frame == True:
        ...
        if timecode_type == 'auto':
            if SMPTE_NDF_REGEX.match(timecode_value):
                timecode_type = 'smpte'
                self.__drop_frame = False
            elif SMPTE_DF_REGEX.match(timecode_value):
                timecode_type = 'smpte'
                if round(self.__fps, 2) % 29.97 == 0:
        ...
        if timecode_type == 'smpte':  # smpte TC
            if SMPTE_REGEX.match(timecode_value):
                pass
            else:
            ...
            temp_timecode_list = [int(x) if x else 0 for x in SMPTE_REGEX.match(timecode_value).groups()]
            hh = temp_timecode_list[0]
            mm = temp_timecode_list[1]
            ss = temp_timecode_list[2]
            ff = temp_timecode_list[3]
            if ff > self.__nominal_fps - 1:
                logging.log(40,
            ...
            if self.__strict == True:  # strict输入逻辑
                if minus_flag == False:
                    frame_index = frame_index % (self.__fps * 86400) if self.__drop_frame == True else frame_index % (
                            self.__nominal_fps * 86400)  # 对于DF时码来说，严格处理取真实FPS的模，对于非DF时码，则取名义FPS的模
                else:
                    frame_index = frame_index % (self.__fps * 86400) if self.__drop_frame == True else frame_index % (
                            self.__nominal_fps * 86400)  # 对于DF时码来说，严格处理取真实FPS的模，对于非DF时码，则取名义FPS的模
                    frame_index = self.__fps * 86400 - frame_index
            else:
                pass
            self.__precise_time = Fraction(frame_index / self.__fps)
        elif timecode_type == 'srt':  # srt字幕
            if SRT_REGEX.match(timecode_value):
                pass
            else:
            ...
            mm = temp_timecode_list[1]
            ss = temp_timecode_list[2]
            sub_sec = temp_timecode_list[3]  # 取子帧戳
            ...
            if FRAME_REGEX.match(timecode_value):
                ...
                raise DFTTTimecodeTypeError
            self.__type = timecode_type
            temp_frame_index = int(FRAME_REGEX.match(timecode_value).group(1))  # 注意，此处的TC指的实际上是帧计数
            ...
            if self.__strict == True:
                temp_frame_index = round(self.__precise_time * self.__fps)
                temp_frame_index = temp_frame_index % (
                        self.__fps * 86400) if self.__drop_frame == True else temp_frame_index % (
                        self.__nominal_fps * 86400)  # 对于DF时码来说，严格处理取真实FPS的模，对于非DF时码，则取名义FPS的模
    ...
    def _(self, timecode_value, timecode_type='time', fps=24.0, drop_frame=False, strict=True):
        ...
        instance_success_log = 'Timecode.__init__.fraction: Timecode instance, type=' + str(
            self.__type) + ', fps=' + str(self.__fps) + ', dropframe=' + str(self.__drop_frame) + ', strict=' + str(
            self.__strict)
        logging.log(10, instance_success_log)
    ...
    def _(self, timecode_value, timecode_type='frame', fps=24.0, drop_frame=False, strict=True):
        ...
        if self.__strict == True:
            ...
            pass
        self.__precise_time = Fraction(temp_frame_index / self.__fps)
        instance_success_log = 'Timecode.__init__.int: Timecode instance, type=' + str(self.__type) + ', fps=' + str(
            self.__fps) + ', dropframe=' + str(self.__drop_frame) + ', strict=' + str(self.__strict)
        logging.log(10, instance_success_log)
    ...
    def set_strict(self, is_strict=True):
        if is_strict == self.__strict:
            pass
        else:
            temp_timecode_object = DfttTimecode(self.__precise_time, self.__type, self.__fps, self.__drop_frame, is_strict)
            self.__precise_time = temp_timecode_object.__precise_time
        return self

    def __repr__(self):
        return '<DfttTimecode>({}{}, {}{}, {}{:.02f} {})'.format('Time Stamp:', self.timecode_output('time'),
                                                                 'Type:', self.__type, 'FPS:', self.__fps,
