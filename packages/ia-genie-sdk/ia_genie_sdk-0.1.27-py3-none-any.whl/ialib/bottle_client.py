...
class BottleClient:
    ...
    def clear_genome(self):
        ...
        *genome* must be either a JSON-serializable object or a file-like object.
        """
        response = requests.post(
            f'{self.url}/genome/clear', verify=self.secure, headers=self.headers
        ).json()
        if 'error' not in response:
            self.genome = None
            self.genie = None
    ...
    def kb_analysis_progress(self):
        ...
        return requests.get(f'{self.url}kb/analyze/progress', verify=self.secure, json=[], headers=self.headers).json()
    
    @_ensure_connected
    def kb_analysis_results(self):
        """Monitors the KB analysis progress kickstarted by analyze_kb."""
        return requests.get(f'{self.url}kb/analyze/results', verify=self.secure, json=[], headers=self.headers).json()
    
    @_ensure_connected
    def get_kbs(self):
        """Returns the KBs for the agent.
        This is a mongo-db and can be used to store or analyze locally."""
        return requests.get(f'{self.url}database', verify=self.secure, json=[], headers=self.headers).json()
    
    @_ensure_connected
    def put_kbs(self, kb):
        """Returns the KBs for the agent.
        This is a mongo-db and can be used to store or analyze locally."""
        return requests.put(f'{self.url}database', verify=self.secure, json=[kb], headers=self.headers).json()
