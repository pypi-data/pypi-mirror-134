...
class BottleClient:
    ...
    def get_kbs(self):
        ...
        return requests.get(f'{self._url}database', verify=self._secure, json=[], headers=self._headers).json()
    
    @_ensure_connected
    def put_kbs(self, kb):
        """Returns the KBs for the agent.
        This is a mongo-db and can be used to store or analyze locally."""
        return requests.put(f'{self._url}database', verify=self._secure, json=[kb], headers=self._headers).json()
