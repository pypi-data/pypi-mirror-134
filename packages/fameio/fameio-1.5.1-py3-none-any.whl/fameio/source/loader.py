...
class PathResolver:
    """Class responsible of locating files imported in a YAML file via the `!include` syntax.

    It provides a default behaviour that can easily be customized by the caller."""

    def __init__(self, resolver: Callable[[str, str], List[str]] = None) -> None:
        self._resolver = PathResolver.default_resolver if resolver is None else resolver
    ...
    def resolve_file_pattern(self, root_path: str, file_pattern: str) -> List[str]:
        ...
        return self._resolver(root_path, file_pattern)

    @staticmethod
    def default_resolver(root_path: str, file_pattern: str) -> List[str]:
        """Returns a list of file paths matching the given `file_pattern` based on the given `root_path` directory."""
...
def resolve_imported_path(loader: FameYamlLoader, included_path: str):
    ...
    """
    file_list = loader.path_resolver.resolve_file_pattern(
