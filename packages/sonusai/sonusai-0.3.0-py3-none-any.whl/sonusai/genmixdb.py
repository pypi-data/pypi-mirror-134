...
def genmixdb(config: dict, verbose: bool = False, show_progress: bool = False) -> dict:
    ...
    if not isinstance(class_weights_threshold, list):
        class_weights_threshold = np.repeat(class_weights_threshold, config['num_classes'])

    if len(class_weights_threshold) != config['num_classes']:
                    ...
                    for target_augmentation_index, target_augmentation in enumerate(target_augmentations):
                        ...
                        if 'snr' in target_augmentation:
                            if target_augmentation['snr'] < -96:
                                ...
                                target_energy = np.mean(np.square(augmented_target_audio.astype(np.single)))
                                noise_energy = np.mean(np.square(noise_segment.astype(np.single)))
                        ...
                        if target_gain == 0:
                            ...
                            truth = generate_truth(
                                audio=(augmented_target_audio.astype(np.single) / target_gain).astype(np.int16),
                                function=target['truth_function'],
                                config=truth_config)

                        for c in target['truth_index']:
                            class_count[c - 1] += np.sum(truth[c - 1, :] >= class_weights_threshold[c - 1])
    ...
    mixture_database['class_count'] = [int(i) for i in class_count]
    mixture_database['mixtures'] = mixtures
            ...
            try:
                if ext == '.txt':
                    ...
                    for key, value in tokens.items():
                        original_name.replace(value, key)
...
def main():
    ...
    for config_file in args['CONFIG']:
        ...
        mixdb = genmixdb(config=config, verbose=args['--verbose'], show_progress=True)
