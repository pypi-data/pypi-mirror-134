...
def genmix(mixdb: dict,
           ...
           verbose: bool = False,
           show_progress: bool = False) -> (np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, dict):
    update_console_handler(verbose)
    initial_log_messages('genmix')

    mixdb_out = deepcopy(mixdb)
    mixdb_out['mixtures'] = eval("mixdb_out['mixtures'][{}]".format(mixid))

    # Ensure mixtures is a list (in the case of a single mixid)
    if not isinstance(mixdb_out['mixtures'], list):
        mixdb_out['mixtures'] = [mixdb_out['mixtures']]
    ...
    truth_t = np.empty((mixdb['num_classes'], total_samples), dtype=np.single)
    class_count = np.zeros(mixdb['num_classes'], dtype=np.int)
    if compute_segsnr:
    ...
    for mixture_record in (tqdm(mixdb_out['mixtures'], desc='Processing') if show_progress else mixdb_out['mixtures']):
        ...
        if mixture_record['target_gain'] == 0:
            target_audio = np.zeros(len(target_audio), dtype=np.int16)
        else:
        ...
        if mixture_record['target_gain'] == 0:
            truth = np.zeros((mixdb['num_classes'], len(target_audio)), dtype=np.int16)
        else:
        ...
        for c in truth_config['index']:
            class_count[c - 1] += np.sum(truth[c - 1, :] >= mixdb['class_weights_threshold'][c - 1])

        if compute_segsnr:
            for offset in range(0, len(target_audio), mixdb['frame_size']):
                target_energy = fft.energy(int16_to_float(target_audio[offset:offset + mixdb['frame_size']]))
                noise_energy = fft.energy(int16_to_float(noise_segment[offset:offset + mixdb['frame_size']]))
                frame_offset = i_sample_offset + offset
                segsnr[frame_offset:frame_offset + mixdb['frame_size']] = np.single(target_energy / noise_energy)
        ...
        i_frame_offset += len(target_audio) // mixdb['frame_size']
        o_frame_offset += len(target_audio) // mixdb['feature_step_samples']
    ...
    return mixture, truth_t, target, noise, segsnr, class_count, mixdb_out
...
def main():
    ...
    mixture, truth_t, target, noise, segsnr, class_count, mixdb_out = genmix(mixdb=mixdb,
                                                                             mixid=mixid,
                                                                             compute_segsnr=True,
                                                                             verbose=verbose,
                                                                             show_progress=True)
    ...
    with h5py.File(output_name, 'w') as f:
        ...
        f.create_dataset(name='class_count', data=class_count)
