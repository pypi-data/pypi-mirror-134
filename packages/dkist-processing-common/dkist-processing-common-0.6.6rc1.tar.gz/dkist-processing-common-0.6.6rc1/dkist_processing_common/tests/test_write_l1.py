...
def test_write_l1_frame(write_l1_task, mocker):
    ...
    mocker.patch(
        "dkist_processing_common.tasks.base.ParsedL0InputTaskBase.instrument",
...
def test_documentation_keys(write_l1_task, mocker):
    ...
    for file in files:
        header = fits.open(file)[1].header
        assert header["INFO_URL"] == task.docs_url_base
        headvers = dkist_fits_specifications.__version__
        assert header["HEADVERS"] == headvers
        assert header["HEAD_URL"] == f"{task.docs_url_base}/projects/data-products/{headvers}"
        inst_module_name = f"dkist_processing_{task.instrument.lower()}"
        calvers = task._get_version_from_module_name(inst_module_name)
        assert header["CALVERS"] == calvers
        assert (
            header["CAL_URL"]
            == f"{task.docs_url_base}/projects/{task.instrument.lower()}/{calvers}"
        )


def test_get_version_from_module(write_l1_task):
    task, _ = write_l1_task
    assert task._get_version_from_module_name("dkist_processing_common") == common_version
