    ...
    def _add_doc_headers(self, header: fits.Header) -> fits.Header:
        ...
        Values follow these rules:
            ...
            3. header['HEAD_URL']:
                ...
                docs.dkist.nso.edu/projects/data-products/<version> where <version> is header['HEADVERS']
            4. header['CALVERS']:
                The version of the calibration codes used for this calibration
                dkist_processing_<instrument>.__version__
                <instrument> is available as self.instrument
            5. header['CAL_URL']:
                The URL for documentation of this version of the calibration codes for the current instrument:
                docs.dkist.nso.edu/projects/<instrument>/<version>
        """
        header["INFO_URL"] = self.docs_url_base
        # headvers = spec_version
        header["HEADVERS"] = spec_version
        header["HEAD_URL"] = f"{self.docs_url_base}/projects/data-products/{spec_version}"
        inst_name = self.instrument.lower()
        inst_module_name = f"dkist_processing_{inst_name}"
        calvers = self._get_version_from_module_name(inst_module_name)
        header["CALVERS"] = calvers
        header["CAL_URL"] = f"{self.docs_url_base}/projects/{inst_name}/{calvers}"
        return header

    @staticmethod
    def _get_version_from_module_name(module_name: str) -> str:
        """
