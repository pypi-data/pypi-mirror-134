            ...
            for frame in frames:
                for quality_task_type_datum in quality_task_type_data:
                    if frame.ip_task_type == quality_task_type_datum.quality_task_type:
                        ...
                        squared_data = frame.data ** 2
                        squared_mean = np.nanmean(squared_data)
                        rms = np.sqrt(squared_mean)
                        normalization_factor = frame.exposure_time
                        normalized_rms = rms / frame.exposure_time
                        quality_task_type_datum.rms_values_across_frame.append(normalized_rms)
                        logger.info(
                            f"{squared_data.min()=}, {squared_data.max()=}, {squared_mean=}, {rms=}, {normalization_factor=}, {normalized_rms=}, {type(normalized_rms)=}"
                        )
                        # more debug stats
                        data_has_nans = np.isnan(frame.data).any()
                        data_is_all_zeros = np.all(frame.data == 0)
                        num_nans = np.isnan(frame.data).sum()
                        array_size = frame.data.size
                        array_min = frame.data.min()
                        array_max = frame.data.max()
                        logger.info(
                            f"{data_has_nans=}, {num_nans=}, {data_is_all_zeros=}, {array_size=}, {array_min=}, {array_max=}"
                        )
                        if np.isnan(normalized_rms):
                            logger.error(f"BAD FRAME")
                            logger.error(f"{frame.header}")
                            logger.error("---------------")
                            logger.error(f"{frame.data}")
                            raise RuntimeError("NAN found")
                        # TODO Theories: data is empty, data is all nan's, exposure time is a nan
