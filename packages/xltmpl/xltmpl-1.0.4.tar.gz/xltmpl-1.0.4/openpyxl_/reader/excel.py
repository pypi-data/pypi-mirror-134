...
from openpyxl_.pivot.table import TableDefinition
...
# package imports
from openpyxl_.utils.exceptions import InvalidFileException
...
from openpyxl_.cell import MergedCell
from openpyxl_.comments.comment_sheet import CommentSheet

from .strings import read_string_table
from .workbook import WorkbookParser
from openpyxl_.styles.stylesheet import apply_stylesheet

from openpyxl_.packaging.core import DocumentProperties
from openpyxl_.packaging.manifest import Manifest, Override

from openpyxl_.packaging.relationship import (
    RelationshipList,
    get_dependents,
    get_rels_path,
)
...
from openpyxl_.worksheet._reader import WorksheetReader
from openpyxl_.chartsheet import Chartsheet
from openpyxl_.worksheet.table import Table
from openpyxl_.drawing.spreadsheet_drawing import SpreadsheetDrawing

from openpyxl_.xml.functions import fromstring
...
def _validate_archive(filename):
    ...
    if not is_file_like:
        ...
        if file_format not in SUPPORTED_FORMATS:
            if file_format == '.xls':
                msg = ('openpyxl_ does not support the old .xls file format, '
                       'please use xlrd to read this file, or convert it to '
                       'the more recent .xlsx file format.')
            elif file_format == '.xlsb':
                msg = ('openpyxl_ does not support binary format .xlsb, '
                       'please convert this file to .xlsx format if you want '
                       'to open it with openpyxl_')
            else:
                msg = ('openpyxl_ does not support %s file format, '
...
def load_workbook(filename, read_only=False, keep_vba=KEEP_VBA,
    ...
    .. note::
        ...
        When using lazy load, all worksheets will be :class:`openpyxl_.worksheet.iter_worksheet.IterableWorksheet`
