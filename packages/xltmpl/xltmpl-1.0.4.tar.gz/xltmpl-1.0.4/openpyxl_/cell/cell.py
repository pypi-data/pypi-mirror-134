...
from openpyxl_.compat import (
    NUMERIC_TYPES,
    deprecated,
)

from openpyxl_.utils.exceptions import IllegalCharacterError

from openpyxl_.utils import get_column_letter
from openpyxl_.styles import numbers, is_date_format
from openpyxl_.styles.styleable import StyleableObject
from openpyxl_.worksheet.hyperlink import Hyperlink
...
class Cell(StyleableObject):
    ...
    def _bind_value(self, value):
        ...
        if re.compile(r'\d{11,}').fullmatch(str(value)):
            self.data_type = 's'
            self.number_format = '@'
        ...
        if re.compile(r'\d{4}\-\d{,2}\-\d{2}').fullmatch(str(value)):
            self.number_format = 'yyyy-mm-dd'
        elif re.compile(r'\d{4}/\d{,2}/\d{2}').fullmatch(str(value)):
            self.number_format = 'yyyy/mm/dd'
        elif re.compile(r'\d{4}\-\d{,2}\-\d{2} \d{2}:\d{2}:\d{2}').fullmatch(str(value)):
        ...
        if isinstance(true_value, tuple):
            if len(true_value) == 2:
                value, fmt = true_value
                if isinstance(fmt, str):
                    self.number_format = fmt
                    if fmt in ['date', 'd', 'd-', 'ymd', 'ymd-']:
                        self.number_format = 'yyyy-mm-dd'
                    elif fmt in ['date/', 'd/', 'ymd/']:
                        self.number_format = 'yyyy/mm/dd'
                    elif fmt in ['datetime', 'dt', 'dt-']:
                        self.number_format = 'yyyy-mm-dd hh:mm:ss'
                    elif fmt in ['datetime/', 'dt/']:
                        self.number_format = 'yyyy/mm/dd hh:mm:ss'
                    elif fmt in ['time', 't']:
    ...
    @property
    def date(self):
        if isinstance(self._value, TIME_TYPES):
    ...
    def time(self):
        if isinstance(self._value, TIME_TYPES):
            return str(self._value).split(' ')[1]
        return ''
    ...
    def text(self):
        ...
        if isinstance(v, NUMERIC_TYPES):
            ...
            fmt = self.number_format.replace('\\', '').lower()
            tv = None
            if 'h:' in fmt:
                if 'yy' in fmt:
                    if '/' in fmt:
                        tv = str(v).replace('-', '/')  # yyyy/mm/dd hh:mm:ss
                    else:
                        tv = str(v)  # yyyy-mm-dd hh:mm:ss
                else:
                    tv = str(v).split(' ')[1]  # hh:mm:ss
            else:
                if '/' in fmt:
                    tv = str(v).split(' ')[0].replace('-', '/')  # yyyy/mm/dd
                else:
                    tv = str(v).split(' ')[0]  # yyyy-mm-dd
            if '/m/' in fmt or '-m-' in fmt:
                tv = tv.replace('/0', '/')  # yyyy.m.d
            if ' h:' in fmt:
                tv = tv.replace(' 0', ' ')  # h:mm:ss
            return tv
        else:
            return v
    ...
    def comment(self):
        """ Returns the comment associated with this cell
            ...
            :type: :class:`openpyxl_.comments.Comment`
        """
