...
from openpyxl_.xml.constants import ARC_STYLE, SHEET_MAIN_NS
...
def apply_stylesheet(archive, wb):
    ...
    if stylesheet.cell_styles:
        ...
        # need to overwrite openpyxl_ defaults in case workbook has different ones
        ...
        warn("Workbook contains no stylesheet, using openpyxl_'s defaults")

    if not wb._named_styles:
        normal = styles['Normal']
        wb.add_named_style(normal)
        warn("Workbook contains no default style, apply openpyxl_'s default")
