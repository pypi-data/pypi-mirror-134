...
## Still confused?
Try calling the `get_funcs()` method:
``` python
from scalecast.Forecaster import Forecaster
>>> f = Forecaster()
>>> f.get_funcs('adder') # which functions add regressors to the object?
>>> f.get_funcs('setter') # which attributes can be set?
>>> f.get_funcs('getter') # which attributes can be got?
>>> f.get_funcs('exporter') # which methods export what information?
>>> f.get_funcs('plotter') # which methods plot and what?

>>> print(f.get_funcs('plotter'))
['plot_acf', 'plot_pacf', 'plot_periodogram', 'seasonal_decompose', 'plot', 'plot_test_set', 'plot_fitted']
