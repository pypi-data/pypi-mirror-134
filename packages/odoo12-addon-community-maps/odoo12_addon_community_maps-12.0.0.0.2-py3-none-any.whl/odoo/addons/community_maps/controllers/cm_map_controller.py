...
class CmMapController(main.RestController):
  ...
  @route([
    _root_path + 'map/<string:_map_slug>/places',
    _root_path + 'map/<string:_map_slug>/places/<string:_place_slug>'
  ], methods=['GET'], auth="api_key", csrf=False)
  def places(self, _map_slug, _place_slug=None, **params):
    return self._process_method('map', 'places', _map_slug, _place_slug)