...
Author: Pascal Kuthe        | Pascal.Kuthe@pm.me
...
class McParameter(object):
    ...
    def __init__(
        ...
        if value_type in (int, np.int):
            ...
            raise NotImplementedError(
                "The type "
                + str(value_type)
                + " of parameter value is not known! Allowed: int, np.int, float, np.float."
            )
        ...
        self.exclude = [self._val_type(val) for val in exclude]
    ...
    def __repr__(self):
        ...
        str_exclude = "None" if self.exclude is None else "{:g}".format(self.exclude)

        return (
            "McParameter("
            + self.name
            + ", value={:g}".format(self.value)
            + ", minval={:g}".format(self._min)
            + ", maxval={:g}".format(self._max)
            + ", value_type="
            + str_type
            + ", inc_min="
            + str(self.inc_min)
    ...
    def dict_json(self) -> Dict[str, Union[float, int, str, bool, List[Union[float, int]]]]:
    ...
    @staticmethod
    def load_json(
        name: str, dict_parameter: Dict[str, Union[float, int, str, bool, List[Union[float, int]]]]
    ) -> McParameter:
        try:
    ...
    def check_bounds(self, value: Union[float, int]):
        ...
        if self.val_type == np.int:
            ...
            value, (int, float, np.int, np.float)
        ...
        if value_too_large:
            raise ValueTooLargeError(
                "Value of {0:s} above its maximum! Given: {1:e}! Maximum boundary : {2:e}!".format(
                    self, value, self.max
                )
            )
        ...
        if value_too_small:
            raise ValueTooSmallError(
                "Value of {0:s} below its minimum! Given: {1:e}! Minimum boundary : {2:e}!".format(
                    self, value, self.min
                )
            )
        ...
        if value_excluded:
            raise ValueExcludedError(
                "Value of {0:s} is excluded! Given: {1:e}! Excluded : {2:e}!".format(
                    self, value, self.exclude
                )
            )
...
class McParameterComposition(object):
    ...
    def json_dict(
        self,
    ) -> Dict[str, Dict[str, Union[float, int, str, bool, List[Union[float, int]]]]]:
        """
        Returns a dict with serializeable content for json.

        Returns
        -------

        A dictionary ready for serialization

        """
        return {
            para.name: para.dict_json()
    ...
    def dump_json(self, file_path):
        ...
        file_path.write_text(json.dumps(self.json_dict()))

    @staticmethod
    def load_paras_from_json(
        paras: Dict[str, Dict[str, Union[float, int, str, bool, List[Union[float, int]]]]]
    ) -> List[McParameter]:
        """Creates a McParameterComposition from a dictionary obtained by a json.load."""

        return [McParameter.load_json(name, param) for name, param in paras.values()]

    @classmethod
    def load_json(cls, file_path):
        """Loads the json file and then creates the McParameter (or subclass) object from the file."""

        # You rarely want to use this because this currently is not under semver and only saves a dict of parameters
        # In most cases you want to serialize a modelcard instead

        if not isinstance(file_path, Path):
            file_path = Path(file_path)

        with file_path.open("r") as file_json:
            content = json.load(file_json)

        composition = McParameterComposition()
        composition.paras = McParameterComposition.load_paras_from_json(content)
    ...
    def get(self, parameters):
        ...
        try:
            ...
            raise KeyError(
                "The parameter {0:s} is not part of this parameter collection!".format(parameters)
            ) from err
        ...
        for name, value in dict_parameters.items():
            try:
                ...
                if force:
                    ...
                    raise KeyError(
                        "The parameter {0:s} is not part of this parameter collection!".format(name)
                    ) from err
    ...
    def print_to_file(self, path_to_file, line_break=""):
        ...
        path_to_file.write_text(self.print_parameters(line_break=line_break) + "\n")
    ...
    def remove(self, parameters):
        ...
        try:
            ...
            raise KeyError(
                "The parameter {0:s} is not part of this parameter collection and can not be removed!".format(
                    parameters
                )
            ) from err
        ...
        if isinstance(paras, McParameter):
            if paras.name in self.name:
                raise ParaExistsError(
                    "Tried to set parameter {:s}, which was already defined.".format(paras)
                )
            else:
                if index is None:
