...
from DMT.external import build_tex, build_svg, clean_tex_files, build_png
...
class Plot(object):
    ...
    def add_data_set(self, x, y, label=None, style=None, **kwargs):
        ...
        """
        self.data.append({"x": x, "y": y, "label": label, "style": style, "kwargs": kwargs})
    ...
    def plot_pyqtgraph(
        ...
        if self.y_axis_scale == "log":
            y_min = np.log10(np.abs(y_min))  # also doing this in case of log for the data itself
            y_max = np.log10(np.abs(y_max))
        try:
            self.pw_pg.setYRange(np.real(y_min), np.real(y_max), padding=padding)
        except Exception:
            print("Error plotting PyQtGraph plot with name " + self.name + ".")
...
class SmithPlot(Plot):
    ...
    def save_pdf(self, directory, width):
        ...
        # prepare folder
        file_name = self.slugify(self.num) + ".pdf"
