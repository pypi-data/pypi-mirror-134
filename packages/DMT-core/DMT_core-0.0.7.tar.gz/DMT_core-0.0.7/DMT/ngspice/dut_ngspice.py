...
class DutNgspice(DutCircuit):
    ...
    def get_hash(self):
        ...
        if self._list_va_file_contents:
            list_va = [content for (file_name, content) in self._list_va_file_contents]
        else:
            list_va = []
    ...
    def create_inp_header(self, inp_circuit):
        ...
        if isinstance(inp_circuit, MCard) or isinstance(inp_circuit, McParameterComposition):
            ...
            inp_circuit = inp_circuit.get_circuit()
        ...
        for va_file in list_va_files:
            if self._copy_va_files:
                with open(va_file) as my_va_file:
                    va_file_content = my_va_file.read()
                va_file = os.path.basename(va_file)
                self._list_va_file_contents.append((va_file, va_file_content))
            else:
