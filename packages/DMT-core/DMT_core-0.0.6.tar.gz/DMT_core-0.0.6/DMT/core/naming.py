...
class SpecifierStr(str):
    ...
    def get_tex_unit(self, scale=1, add=""):
        ...
        if sub_specifiers.PHASE.sub_specifiers[0] in self.sub_specifiers:
            ...
            # get unit from pint
            try:
                unit = siunitx_format_unit(unit)
            except TypeError:
                unit = siunitx_format_unit(
                    unit._units, unit_registry
                )  # new version has other interface
            return r"\si{" + unit_prefix + unit + add + r"}"

    def to_label(self, scale=1, negative=False, divide_by_unit=False):
    ...
    def to_legend_with_value(self, value, scale=1, **kwargs):
    ...
    @classmethod
    def string_from_load(cls, string):
...
class GlobalObj(object):
    ...
    def __iter__(self):
        if self._MEMBERS is None:  # only if None or always???
            self._set_members()
