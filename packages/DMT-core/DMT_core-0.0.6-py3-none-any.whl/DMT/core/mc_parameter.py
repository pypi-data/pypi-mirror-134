...
class McParameter(object):
    ...
    def __repr__(self):
        ...
        str_exclude = "None" if self.exclude is None else f"{self.exclude:g}"
    ...
    def load_json(cls, dict_parameter):
        ...
        if "__McParameter__" in dict_parameter and dict_parameter["__McParameter__"] == 1.0:
            try:
                value_type = {"int": int, "float": float}[dict_parameter["type"]]
            except KeyError:
    ...
    def check_bounds(self, value):
        ...
        if value_excluded:
            raise ValueExcludedError(
                f"Value of {self:s} is excluded! Given: {value:e}! Excluded : {self.exclude:e}!"
            )
...
class McParameterComposition(object):
    ...
    def info_json(self):
        ...
        if hasattr(self, "__McParameterComposition__"):
            return {
                "__McParameterComposition__": self.__McParameterComposition__
            }  # make versions, so we can introduce compatibility here!
        else:
            return {
                "__McParameterComposition__": 1.0
            }  # make versions, so we can introduce compatibility here!
    ...
    def loads_json(cls, dict_parameter):
        ...
        if "__McParameterComposition__" in dict_parameter:
            ...
            raise IOError(
                "DMT->McParameterComposition: The given dict has no __McParameterComposition__ key!"
            )
    ...
    def print_parameters(self, paras=None, line_break=""):
        ...
        try:
            ...
            for para in sorted(paras, key=lambda x: x.name):
                temp_str += f"  {para:<12s} = {para:10.5e} {line_break}"
    ...
    def print_tex(self):
        ...
        try:
            ...
            # clean_mcard.add(self.get(["tau_dci"]))
        except (
            AttributeError,
            KeyError,
            ValueError,
            ...
            with doc.create(
                ...
                for para in sorted(clean_mcard, key=lambda x: (x.group, x.name)):
                    if group != para.group:
                        ...
                        group = para.group
                        if hasattr(self, "possible_groups"):
                            try:
                                group_desc = next(
                                    description
