""" Step to fit a COOS TCAD model to measurement data.
...
from DMT.COOS import DutCOOS


class XFitCOOS(XStep):
    """This XStep can fit measurement data to a TCAD model implemented in COOS.
    ...
    get_inp : callable
        for a given line and modelcard, return a COOS device definition.
    """
    ...
    def get_description(self):
        ...
        doc.append(
            NoEscape(
                ...
                + r"$ at different $"
                + self.outer_sweep_voltage.to_tex()
                + r"$ from measurements to COOS TCAD simulations based on augmented drift-diffusion transport."
            )
        )
        return doc
