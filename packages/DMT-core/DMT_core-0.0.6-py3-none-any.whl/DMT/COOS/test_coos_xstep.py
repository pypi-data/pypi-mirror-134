# this script tests the usage of DMT Xsteps using a DUT COOS
from DMT.COOS import DutCOOS, sige_hbt, coos_iv_fallback, XFitCOOS
...
dmt.append_simulation(dut_coos, sweep)
dmt.run_and_read(force=False)

# clean data
for key in dut_coos.data.keys():
    if "iv" in key:
        fallback = coos_iv_fallback
        dut_coos.data[key] = dut_coos.data[key].clean_data(
        ...
        dut_coos.data[key] = dut_coos.data[key].deembed_DC(DataFrame(), mres=mres)
...
### Extraction
extraction = Xtraction("coos_verify", mc, os.path.join("test", "tmp", "extractions", "coos"), lib)
