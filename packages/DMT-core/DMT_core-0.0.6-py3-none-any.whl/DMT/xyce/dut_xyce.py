...
class DutXyce(DutCircuit):
    ...
    def create_inp_header(self, inp_circuit):
        ...
        netlist : str
        """
        if isinstance(inp_circuit, MCard):
            ...
            self._inp_circuit = inp_circuit.get_circuit()
        elif not isinstance(inp_circuit, Circuit):
            raise OSError(
        ...
        list_va_files = set()  # so we have each element only once
        for element in self._inp_circuit.netlist:
            try:
                list_va_files.add(element.parameters.va_file)
        ...
        for va_file in list_va_files:
            ...
            if self._copy_va_files:
                va_file_content = va_file.read_text()
                self._list_va_file_contents.append((va_file.name, va_file_content))
            ...
            if va_file.is_file():
                ...
                # hash file content
                va_hash = create_md5_hash(va_file_content)
                ...
                if not path_va_hash.is_file():
                    ...
                    self._va_plugins_to_compile.append((path_va_hash, va_file_content))

                va_plugins.append(path_va_hash)
            else:
                raise IOError(
                    "DutXyce: Was not able to locate the Verilog-AMS file. The given path was \n%s",
                    va_file,
                )
    ...
    def make_input(self, sweep):
        ...
        if self._va_plugins_to_compile:
            ...
            processes = []
            for path_plugin, va_content in self._va_plugins_to_compile:
                # write to location
                path_plugin.parent.mkdir(exist_ok=True)
                path_va = path_plugin.with_suffix(".va")
                path_va.write_text(va_content)

                # call "buildxyceplugin"
                log_file = open(path_plugin.parent / (path_plugin.stem + "_compile.log"), "w")
                processes.append(
                    subprocess.Popen(
                        ["buildxyceplugin", "-o", path_plugin.stem, path_va.name, "."],
                        shell=False,
    ...
    def _import_xyce_results(self, *filepaths):
        ...
        if col_ac_switch in df_main.columns:
            ...
            for i_row_1, i_row_2 in row_pairs:
                ...
                df_main_new = df_main_new.append(
                    series_dc.append([series_ac_forward, series_ac_backward]), ignore_index=True
                )

            df_main = df_main_new.sort_values(
                by=cols_filter,
            )
