...
def upload_local_file_callback(ctx: click.Context, param: click.Parameter, value: List[str]) -> List[str]:
    if value:
        for dir_spec in value:
            ...
            if os.path.isdir(local_path):
                ...
                if directory_size >= 3 * 1024 * 1024 * 1024:
                    click.confirm(f"Try to upload [{os.path.abspath(local_path)}] "
                                  f"but it's too large ({sizeof_fmt(directory_size)}). "
                                  f"Process anyway?", abort=True)
...
def logs(
    ...
    while True:
        if read_experiment(name).status not in ["pending", "running"] and experiment_finished_dt is None:
            experiment_finished_dt = time.time()

        if experiment_finished_dt is not None and experiment_finished_dt - time.time() > 3:
            break

        logs = list_experiment_logs(
            experiment_name=name, after=after, worker_numer=worker_number
        )
        print_logs(logs)
        if len(logs) > 0:
