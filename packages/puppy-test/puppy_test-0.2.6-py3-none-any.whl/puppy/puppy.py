...
def upgrade():
    ...
    print("框架升级完成后不会自动关闭cmd窗口，请主动关闭！\n升级成功后请再次执行puppy update继续更新工程版本！")
...
def other(is_help=False):
    ...
    if now_version is not None and _puppy.to_int(now_version) < _puppy.to_int(_puppy.get_version()):
        desc += "        当前工程的版本与框架版本不一致，请使用puppy update命令更新项目！\n"
    ...
    print('''{}
        1、puppy init：    初始化工程
        2、puppy update：   更新当前工程脚本版本
        3、puppy upgrade：  升级puppy_test框架（仅内网可用）
        4、puppy clean：  清理当前工程无用的文件（如test_case下未使用的py)
        5、puppy generate-test-case -i 接口文件 -f 字段名 -r 构建规则：  自动生成测试案例（该功能开发中）'''.format(desc))
