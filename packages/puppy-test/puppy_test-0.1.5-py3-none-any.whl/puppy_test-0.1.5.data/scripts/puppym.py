...
import unittest
import ctypes, sys
from puppy.core.function.string.string_eplace_by_domains import StringReplaceByDomains
    ...
    for root, dirs, files in os.walk(jet_brains_path):
        for dir in dirs:
            if "PyCharm" in dir:
                ...
                version = dir[7:]
                # 删除.
                int_type_of_version = puppy.to_int(version)
...
def init(_type):
    ...
    print("注意：您正在执行puppym的初始化命令，此命令会执行以下操作：")
...
def update():
    ...
    if puppy.to_int(now_version) > puppy.to_int(puppy.get_version()):
        print("请先使用puppym upgrade升级puppy_test框架，再执行puppym update命令，如果已经是最新的puppy_test框架，说明代码的版本信息错误")
        return -1
        ...
        with open(runner_path, "r", encoding="utf-8") as file:
            for line in file.readlines():
                if "__version__" in line:
                    ...
                    now_version = number[0]
                    break
    desc = "当前puppy框架版本为：{}\n".format(puppy.get_version())
    if now_version is not None and puppy.to_int(now_version) < puppy.to_int(puppy.get_version()):
        desc += "    当前项目的版本与框架版本不一致，请使用puppym update更新项目！\n"
    if server_version is not None and puppy.to_int(server_version) > puppy.to_int(puppy.get_version()):
        desc += "    服务器有新的框架版本，请升级！服务器的版本为：{}\n".format(server_version)
    print('''命令输入错误：
        {}
        1、puppym init：    初始化工程
        2、puppym update：   更新当前工程脚本版本
        3、puppym upgrade：  升级puppy_test框架（仅内网可用）
        4、puppym clean：  清理当前工程无用的文件（如test_case下未使用的py)
        5、puppym generate-test-case -i 接口文件 -f 字段名 -r 构建规则：  自动生成测试案例（该功能开发中）'''.format(desc))
...
if __name__ == '__main__':
    ...
    if command == "init-tunit" or command == "init":
        ...
        upgrade()
        print("请继续执行puppym update命令更新工程脚本版本")
    elif command == "clean":
        clean()
