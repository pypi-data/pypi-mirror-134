...
def find_package_data(package_path: str) -> PackageDataType:
    ...
    :param package_path:        absolute path of the package directory (project root for app projects and the package
                                subdir project_root/namespace/portion for namespace projects).
    :return:                    setuptools package_data dict, where the key is an empty string (to be included for all
                                sub-packages) and the dict item is a list of the found resource files with a relative
                                path to the :paramref:`.package_path` directory).
    """
    files = []

    docs_path = os.path.join(package_path, DOCS_FOLDER)
    for file in glob.glob(os.path.join(docs_path, "**", "*"), recursive=True):
        if os.path.relpath(file, docs_path)[0] != "_":  # exclude _build folders
            files.append(os.path.relpath(file, package_path))
...
def namespace_guess(project_path: str) -> str:
    ...
    for part in package_name.split("_"):
        ...
        *ns_path_parts, portion_name = portion_name.split("_", maxsplit=1)
        namespace_name += "." + part
