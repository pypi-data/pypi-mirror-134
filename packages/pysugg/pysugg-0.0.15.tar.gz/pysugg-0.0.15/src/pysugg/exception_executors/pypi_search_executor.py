...
class PypiSearchExecutor(ExceptionExecutor):
    def execute(self, exception_type, value, tb):
        missing_name = re.findall(r"(?<=\')\w+(?=\')", str(value))[0]
        search_url = f"https://pypi.org/search/?q={missing_name}"
        print(f"Module name '{missing_name}' is not found, now searching the available modules.")
