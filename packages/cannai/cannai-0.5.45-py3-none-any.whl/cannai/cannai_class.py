...
class Cmodel(Cannaibase):
    ...
    def combine_models(self, e_score,e_score_key=None, kaisu=500,kaisu2=20, addition_len=10,rand_rate=0.1, save_higher=False):
        if e_score_key == None:
        ...
        totyu_len = addition_len * 3 + 70
        kaisu0 = kaisu * 3

        for ii0 in range(sml_len):
            kari_weight = []
            sum_ka_wi = 0.0
            for ism in range(sml_len):
                if ii0 == ism:
                    kari_weight.append(1.0)
                else:
                    kari_weight.append(0.0)

            mod_val = all_mod_data[ii0]
            if e_score_key == None:
            ...
            new_mod_group = self.model_list[ii0]
            jyuni = 0
            if len_mod_lak > 0:
        ...
        for round_ in range(kaisu2):
            l_mlak02 = len(model_list_additional_kari_weight)
            for ii in range(l_mlak02):
                kari_weight = model_list_additional_kari_weight[ii]
                for ism in range(totyu_len):
                    if random.random() < rand_rate:
                        kka_wi = random.uniform(-0.3, 0.3)
                        kari_weight[ism] += kka_wi

                mod_val = all_mod_data[0] * kari_weight[0]

                for ism in range(1, sml_len):
                    sss_k2 = all_mod_data[ism] * kari_weight[ism]
                    mod_val += sss_k2
                dat = self.set_date()
                if e_score_key == None:
                    e_out_l = self.get_line(mod_val, 0)
