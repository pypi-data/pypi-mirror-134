...
class Cmodel(Cannaibase):
    ...
    def combine_models(self, e_score,e_score_key=None, kaisu=500,kaisu2=1000,rand_rate=0.1, addition_len=10, save_higher=False):
        if e_score_key == None:
        ...
        for ii in range(kaisu0):
            print("\rround1: " + str(ii + 1) + "/" + str(sml_len+1), end="")
            kari_weight = []
            sum_ka_wi = 0.0
            for ism in range(sml_len):
        ...
        for round_ in range(kaisu2):
            ...
            for ii in range(l_modlak):
                ...
                while (abs(run_again) <= 0.01 or ag_flag):
                    ag_flag = False
                    for ism in range(sml_len):
                        if random.random() < rand_rate:
                            chang_val = random.uniform(-0.3, 0.3)
                            kari_weightb[ism] += chang_val
                            run_again += chang_val
                    for rag_ii in range(3):
                        nlnml3 = np.linalg.norm(model_list_additional_kari_weight[rag_ii] - kari_weightb)
                        print(nlnml3)
                        if nlnml3 < 0.001:
                            ag_flag = True
                            break
                ...
                if e_score_key == None:
                    ...
                    e_out_l = self.get_line(mod_val, e_score_key)
                comv_score = self.Cal_s.cal_score_single_sub(e_score, e_out_l, e_ans_l)
                new_mod_group = ["combine" + str(round_) +"_" + str(ii), mod_val, dat, "No_model", None, None, None]
        ...
        self.model_list_additional_weight = copy.deepcopy(model_list_additional_kari_weight)
