...
class NWApproval:
    ...
    def to_yaml(self) -> Dict[str, Any]:
        return {
            self.name: {
                "type": "approval",
                "requires": self.service.job_names_for_task_refs(self.requires)
            }
        }
...
class NWService:
    ...
    def job_names_for_task_refs(self, task_refs: List[str]) -> List[str]:
        """List of job names for all given task references"""
        required: List[str] = []
        for id in task_refs:
    ...
    async def check_changes_for_task_refs(self, task_refs: List[str]) -> Tuple[bool, Dict[str, Any]]:
        ...
        task_refs = list(filter(lambda x: not is_approval(x), task_refs)
                        )

        tasks = [to_result(t, reference)
                 for reference in task_refs for t in tasks_for(reference)]
    ...
    async def workflow_jobs_async(self) -> List[Dict[str, Any]]:
        ...
        def workflow_job(dependency_ids: List[str], name: str, custom_dependencies: List[str] = []) -> Dict[str, Any]:
            ...
            required = self.job_names_for_task_refs(
                dependency_ids)
            if len(required) > 0:
...
class NWTask:
    ...
    def __init__(self, name: str, service: "NWService", members: Dict[str, Any]):
        ...
        self.requires: List[str] = list(members["requires"])
        """tasks references we consider to determine a changeset; also used to wait for completion in circleci workflow jobs"""
        ...
        self.not_changed_command: Optional[Dict[str,
                                                Any]] = members["not_changed_command"]
        self.dependency_refs: List[str] = list(members["dependency-refs"])
        """tasks references we consider to determine a changeset"""
    ...
    async def is_changed_async(self) -> bool:
        ...
        async with self._lock:
            if self._is_changed is None:
                ...
                required_dependencies_result = await self.service.check_changes_for_task_refs(self.requires)
                self.change_info |= required_dependencies_result[1]
                self._is_changed = self._is_changed or required_dependencies_result[0]

                 # check depending external tasks by the dependency-refs attribute:
                required_dependency_refs_result = await self.service.check_changes_for_task_refs(self.dependency_refs)
                self.change_info |= required_dependency_refs_result[1]
                self._is_changed = self._is_changed or required_dependency_refs_result[0]
