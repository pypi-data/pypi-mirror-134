        ...
        *,
        ...
        try:
            ...
            try:
                ...
                log.warning("timeout detected during kernel-runner status check")
            ...
            try:
                ...
                log.warning("timeout detected during service app query")
            if self.local_config['debug']['log-kernel-config']:
                log.debug('service ports:\n{!r}', pretty(service_ports))
        except asyncio.CancelledError:
            log.exception("cancelled waiting of container startup (k:{})", kernel_id)
            raise RuntimeError("cancelled waiting of container startup due to "
                               "initialization failure or agent shutdown")
        except zmq.error.ZMQError:
            log.exception("error while waiting of container startup (k:{})", kernel_id)
            raise RuntimeError("cancelled waiting of container startup due to "
                               "initialization failure or agent shutdown")
        finally:
