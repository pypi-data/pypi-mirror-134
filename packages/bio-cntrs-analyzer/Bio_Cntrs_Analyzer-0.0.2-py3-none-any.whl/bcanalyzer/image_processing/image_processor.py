...
def process_image(img_path, params, largest_cnt_only=False, force_q_th=False):
    ...
    result = sliding_window(mid, (win_size, win_size),
                            mean_window, dx=dxy, dy=dxy) // 255

    th = None
    if use_abs_threshhold and threshhold_abs is not None and not force_q_th:
        th = threshhold_abs
        th_perc = stats.percentileofscore(result.flatten(), th)
    else:
        th = np.percentile(result, threshhold)
        th_perc = threshhold
    canny_thresh = np.where(result > th, 0, 1).astype(np.uint8)

    canny_thresh = tile_array(canny_thresh, dxy, dxy)

    h_pad = img_np.shape[0] - canny_thresh.shape[0]
    w_pad = img_np.shape[1] - canny_thresh.shape[1]
    canny_thresh = np.pad(canny_thresh, ((
        h_pad//2, h_pad//2+img_np.shape[0] % 2), (w_pad//2, w_pad//2+img_np.shape[1] % 2)), 'edge')

    canny_thresh = canny_thresh[..., np.newaxis]
    #canny_thresh = cv2.bitwise_not(canny_thresh)
    if largest_cnt_only:
        canny_thresh = getLargestContourThresh(canny_thresh)

    preview = invertByMask(img_np, canny_thresh)

    res_meta = {}
    res_meta["thre_abs"] = th
    res_meta["thre"] = th_perc

    return preview, canny_thresh, res_meta
