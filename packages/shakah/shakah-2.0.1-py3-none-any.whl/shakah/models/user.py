...
class UserTest(ShakahModel, table=True):
    ...
    token: Optional['OAuthTokenTest'] = Relationship()

class OAuthTokenTest(ShakahModel, table=True):
    __find_excludes__ = {"refresh_token", "expires_at", "expires_in", "access_token"}
    name: str
    ...
    user: Optional[UserTest] = Relationship(back_populates="token")
