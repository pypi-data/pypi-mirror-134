...
def random_matrix(dim, struct='dense'):
    ...
    if struct=="dense-unif":
        transition_matrices= random_matrices_dense_unif(dim)
...
class RestlessBandit:
    ...
    def gittins_indices(self, discount, number_of_updates=5):
        ...
        return whittle.compute_whittle_indices(self.transition_matrices[:, 0, :], self.transition_matrices[:, 1, :],
                self.reward_vector[:, 0], self.reward_vector[:, 1],
                check_indexability=False, beta=discount, number_of_updates=number_of_updates)[1]

    def is_indexable(self, discount=1):
        return whittle.compute_whittle_indices(self.transition_matrices[:, 0, :], self.transition_matrices[:, 1, :],
                self.reward_vector[:, 0], self.reward_vector[:, 1],
                check_indexability=True, beta=discount, number_of_updates=5)[0]
