...
class ptmethods:
    ...
    def get_response(self, url, method, proxy=None):
        if proxy is None:
            proxy = self.proxy
        r = requests.request(method, url, allow_redirects=self.redirects, headers=self.headers, proxies=proxy, verify=False, timeout=8)
        return r
    ...
    def print_result(self, options, methods, proxy_method, connect_method, localhost_proxy_test, localhost_connect_test):
        ...
        ptmisclib.ptprint(ptmisclib.out_ifnot(f" ", "", self.use_json))
        if proxy_method:
            ptmisclib.ptprint(ptmisclib.out_ifnot(f"Proxy mode is allowed", "VULN", self.use_json))
            ptmisclib.ptprint(ptmisclib.out_ifnot(f"Title of localhost via proxy: {localhost_proxy_test}", "VULN", self.use_json))
        else:
            ptmisclib.ptprint(ptmisclib.out_ifnot(f"Proxy mode is not allowed", "NOTVULN", self.use_json))
        if connect_method:
            ptmisclib.ptprint(ptmisclib.out_ifnot(f"CONNECT method at port {self.port} is allowed", "VULN", self.use_json))
            ptmisclib.ptprint(ptmisclib.out_ifnot(f"Title of localhost via CONNECT method: {localhost_connect_test}", "VULN", self.use_json))
        else:
            ptmisclib.ptprint(ptmisclib.out_ifnot(f"CONNECT method at port {self.port} is not allowed", "NOTVULN", self.use_json))
