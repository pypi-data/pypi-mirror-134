...
class SimpleBaseMiddleware:
    ...
    async def get_body_params(self, request):
        ...
        if hasattr(self, "_body_params"):
            return self._body_params

        body = await self.get_body(request)
        try:
            self._body_params = json.loads(body)
        except:
            assert (
            ...
            if content_type == b"multipart/form-data":
                multipart_parser = MultiPartParser(request.headers, body)
                form_data = await multipart_parser.parse()
            elif content_type == b"application/x-www-form-urlencoded":
                form_parser = FormParser(request.headers, body)
                form_data = await form_parser.parse()
            else:
                form_data = FormData()
            self._body_params = dict(form_data)
