...
class SecurityRateThrottle(throttling.SimpleRateThrottle):
    ...
    @property
    def rate(self):
        if getattr(self, '_rate', None) < 0 or getattr(self, '_rate', None) is None:
    ...
    def set_rate(self, request, view):
        if request.user.is_authenticated:
            ...
            self._rate = view_rate if view_rate is not None else self._user_rate
        else:
            view_rate = getattr(view, self.security_throttle_anon, None)
            self._rate = view_rate if view_rate is not None else self._anon_rate
...
class CustomUserRateThrottle(throttling.UserRateThrottle):
    ...
    def allow_request(self, request, view):
        ...
        if getattr(view, SecurityRateThrottle.security_throttle_user, None) == 0:
            return True
...
class CustomAnonRateThrottle(throttling.AnonRateThrottle):
    ...
    def allow_request(self, request, view):
        ...
        if getattr(view, SecurityRateThrottle.security_throttle_anon, None) == 0:
            return True
