...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return bpkvS if e else bpkvf
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->bpkvY:
...
@click.option("--revert",is_flag=bpkvS,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:bpkvs):
...
def _cpvcs_initialized(pod_name:bpkvc)->bpkvs:
...
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
def cmd_cpvcs_init(name:bpkvc):
...
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=bpkvS,default=bpkvf)
def cmd_cpvcs_delete(name:bpkvc,remote:bpkvs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
def cmd_cpvcs_register_remote(name:bpkvc):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=bpkvS)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=bpkvS)
def cmd_cpvcs_rename(name:bpkvc,new_name:bpkvc):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
def cmd_cpvcs_commit(message:bpkvc,name:bpkvc):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=bpkvS,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=bpkvS,default=bpkvf,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
def cmd_cpvcs_push(squash:bpkvs,message:bpkvc,name:bpkvc,three_way:bpkvs):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
@click.option("-v","--version",type=bpkvz)
@click.option("-m","--message",required=bpkvf)
def cmd_cpvcs_push_overwrite(version:bpkvz,message:bpkvc,name:bpkvc):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=bpkvz,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=bpkvS,default=bpkvf,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
def cmd_cpvcs_inject(version:bpkvz,reset:bpkvs,name:bpkvc):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=bpkvS,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=bpkvS,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
@click.option("--lazy/--eager",default=bpkvS,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:bpkvc,inject:bpkvs,reset:bpkvs,lazy:bpkvs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=bpkvS,default=bpkvf)
def cmd_cpvcs_list_pods(remote:bpkvs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
def cmd_cpvcs_versions(name:bpkvc):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=bpkvS,type=bpkvz)
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
def cmd_cpvcs_version_info(version:bpkvz,name:bpkvc):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=bpkvz,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
def cmd_cpvcs_version_metamodel(version:bpkvz,name:bpkvc):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=bpkvS,type=bpkvz,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=bpkvS,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=bpkvS,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=bpkvf,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
def cmd_cpvcs_set_version(version:bpkvz,inject:bpkvs,reset:bpkvs,commit_before:bpkvs,name:bpkvc):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
def cmd_cpvcs_commits(version:bpkvz,name:bpkvc):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=bpkvS)
@click.option("--commit","-c",required=bpkvS)
@click.option("-n","--name",help="Name of the cloud pod",required=bpkvS)
def cmd_cpvcs_commit_diff(version:bpkvz,commit:bpkvz,name:bpkvc):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(bpkvY)
  status.stop()
...
def print_table(columns:Dict[bpkvc,bpkvc],rows:List[Dict[bpkvc,Any]],formatters:Dict[bpkvc,Callable[[Any],bpkvc]]=bpkvY):
 from rich.table import Table
 if formatters is bpkvY:
  formatters=bpkvW()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  bpkvx(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->bpkvY:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->bpkvY:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->bpkvY:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->bpkvY:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
