...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or mTuBN
 @mTuBL
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return mTuBa(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @mTuBg
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if mTuBd(origins,mTuBQ)else origins.get("Items")
  origins_to_create=mTuBJ([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if mTuBd(aliases,mTuBQ)else aliases.get("Items")
  aliases_to_create=mTuBJ(aliases)
  for distr in distrs:
   origins=mTuBJ([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=mTuBJ(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",mTuBj)
   config["Enabled"]=mTuBe(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if mTuBd(custom_errors,mTuBU)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=mTuBe(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":mTuBj}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=mTuBe(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=mTuBN):
    parent=config if parent is mTuBN else parent
    items=parent.get(key)or[]
    if mTuBd(items,mTuBU)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":mTuBI(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if mTuBd(cache_behavs,mTuBU)else cache_behavs)
