...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=PMqaL,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if PMqal(perm["IpProtocol"])!=PMqal(rp["IpProtocol"]):
    continue
 ...
 @PMqaN
 def get_vpc_and_name_for_security_group(cls,group_id:PMqal)->Tuple[PMqal,PMqal]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or PMqaL
  group_name=groups and groups[0]["GroupName"]or PMqaL
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",PMqaL)
   vpc_id=PMqaL
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",PMqaL)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=PMqal(result.get("IpProtocol"))
   description=result.pop("Description",PMqaL)
   cidr_ipv6=result.pop("CidrIpv6",PMqaL)
