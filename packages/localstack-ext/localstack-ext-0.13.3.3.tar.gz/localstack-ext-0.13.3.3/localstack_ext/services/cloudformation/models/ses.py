...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[yoKbr])[0]
 def get_physical_resource_id(self,attribute=yoKbr,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if yoKbT(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return yoKbD(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @yoKbB
 def add_defaults(resource,stack_name:yoKbL):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return yoKbD(SESReceiptRule,self).get_cfn_attribute(attribute)
 @yoKbw
