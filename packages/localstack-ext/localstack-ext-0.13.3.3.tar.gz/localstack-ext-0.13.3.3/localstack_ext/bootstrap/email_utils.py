...
SENT_EMAILS=[]
EMAIL_BLACKLIST=DckHK()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",DckHE(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=DckHs,from_name=DckHs,smtp_host=DckHs,smtp_user=DckHs,smtp_pass=DckHs,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if DckHf(recipients,DckHv)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=DckHs):
 ...
 result["From"]=formataddr((DckHE(Header(from_name,"utf-8")),from_email))
 return result
