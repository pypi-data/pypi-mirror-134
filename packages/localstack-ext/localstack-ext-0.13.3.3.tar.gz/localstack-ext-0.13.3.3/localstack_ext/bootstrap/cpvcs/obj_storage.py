...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=VpvRC
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:VpvRT)->VpvRT:
  pass
 @VpvRf
 def get(cls,requested_file_locator:VpvRT):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=VpvRC
 @VpvRf
 def get(cls,state_file_locator:VpvRT,requested_storage:VpvRT,serializers:Dict[VpvRT,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[VpvRT,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:VpvRT)->VpvRT:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:VpvRT)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:VpvRT)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:VpvRT)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:VpvRT)->VpvRT:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[VpvRT]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:VpvRT,new_key:VpvRT,referenced_by_version:VpvRT=VpvRC):
  pass
 @abc.abstractmethod
 def version_exists(self,key:VpvRT)->VpvRF:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:VpvRT,key:VpvRT):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:VpvRT,new_key:VpvRT,base_path:VpvRT)->VpvRF:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[VpvRT]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[VpvRT,VpvRT],remote_location:VpvRT=VpvRC)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[VpvRT]:
  ...
  for cpvcs_obj in args:
   if VpvRr(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif VpvRr(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[VpvRT,VpvRT],remote_location:VpvRT=VpvRC)->List[Union[Revision,Version]]:
