...
def install_package(packages:Union[List[OqEVx],OqEVx],api_name:OqEVx):
 if not(is_debian()and in_docker()):
  ...
  return
 if OqEVe(packages,OqEVx):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=OqEVn)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=OqEVn)
def check_or_install(command:OqEVx,packages:Union[List[OqEVx],OqEVx],api:OqEVx):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:OqEVx,target_file:OqEVx,timeout:int=OqEVW):
 def do_download(url,print_error=OqEVn):
  try:
