...
class DecryptionHandler:
 decryption_key:QlTcM
 def __init__(self,decryption_key:QlTcM):
  self.decryption_key=decryption_key
 def decrypt(self,content)->QlTcM:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=QlTcI):
  if path and not QlTcF(path,QlTce):
   path=QlTce(QlTct(path,"_path",[]))
  if not path:
   return QlTcI
...
def patch_traceback_lines():
 if QlTct(traceback.FrameSummary,"_ls_patch_applied",QlTcI):
  return
 @QlTcX
 def line(self):
  try:
 ...
 QlTcA(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=QlTcL
def patch_inspect_findsource():
 if QlTct(inspect,"_ls_patch_applied",QlTcI):
  return
 ...
 inspect._ls_patch_applied=QlTcL
