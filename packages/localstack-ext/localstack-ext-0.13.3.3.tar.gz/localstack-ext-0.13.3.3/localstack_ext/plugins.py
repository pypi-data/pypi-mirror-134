...
def api_key_configured()->fvAmd:
 return(fvAml if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else fvAmb)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if fvAmM([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
