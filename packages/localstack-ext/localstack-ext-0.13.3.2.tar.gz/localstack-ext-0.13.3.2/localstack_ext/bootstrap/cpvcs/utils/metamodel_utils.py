...
CAPTURE_METAMODEL_SEPARATELY=zcwCI
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=zcwCl(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=zcwCl(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->zcwCQ:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[zcwCQ,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,zcwCd)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(zcwCd,state_files_to)
...
class CommitMetamodelUtils:
 @zcwCk
 def create_metadata_archive(cls,version:Version,delete_reference:bool=zcwCI,overwrite:bool=zcwCI,metamodels_file:zcwCQ=zcwCd):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @zcwCk
 def create_delta_metamodel_file(cls,version:zcwCh,revision:zcwCh,store_to_zip=zcwCI)->Tuple[Dict,Dict]:
 ...
 @zcwCk
 def reconstruct_metamodel(cls,version:zcwCh,revision:zcwCh=zcwCd)->Dict:
  result={}
  for rev in zcwCf(1,revision+1):
  ...
  return result
 @zcwCk
 def get_version_metamodel(cls,version:zcwCh,revision:zcwCh=zcwCd)->Dict:
 ...
 @zcwCk
 def get_commit_diff(cls,version_no:zcwCh,commit_no:zcwCh)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->zcwCv:
 if zcwCS(clazz,zcwCO):
  return{}
 constructor=zcwCs(clazz,"__init__",zcwCd)
...
def _create_metamodel_helper(obj,width=25,visited:set=zcwCd):
 if obj is zcwCd:
  return obj
