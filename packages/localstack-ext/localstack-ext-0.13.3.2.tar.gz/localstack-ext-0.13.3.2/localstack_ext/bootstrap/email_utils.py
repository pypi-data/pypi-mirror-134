...
SENT_EMAILS=[]
EMAIL_BLACKLIST=Ysrnv()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",YsrnX(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=YsrnW,from_name=YsrnW,smtp_host=YsrnW,smtp_user=YsrnW,smtp_pass=YsrnW,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if YsrnH(recipients,YsrnQ)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=YsrnW):
 ...
 result["From"]=formataddr((YsrnX(Header(from_name,"utf-8")),from_email))
 return result
