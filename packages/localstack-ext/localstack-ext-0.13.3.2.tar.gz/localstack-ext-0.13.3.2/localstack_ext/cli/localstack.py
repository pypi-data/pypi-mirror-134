...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return QtSyG if e else QtSyM
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->QtSyD:
...
@click.option("--revert",is_flag=QtSyG,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:QtSya):
...
def _cpvcs_initialized(pod_name:QtSyW)->QtSya:
...
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
def cmd_cpvcs_init(name:QtSyW):
...
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=QtSyG,default=QtSyM)
def cmd_cpvcs_delete(name:QtSyW,remote:QtSya):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
def cmd_cpvcs_register_remote(name:QtSyW):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=QtSyG)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=QtSyG)
def cmd_cpvcs_rename(name:QtSyW,new_name:QtSyW):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
def cmd_cpvcs_commit(message:QtSyW,name:QtSyW):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=QtSyG,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=QtSyG,default=QtSyM,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
def cmd_cpvcs_push(squash:QtSya,message:QtSyW,name:QtSyW,three_way:QtSya):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
@click.option("-v","--version",type=QtSye)
@click.option("-m","--message",required=QtSyM)
def cmd_cpvcs_push_overwrite(version:QtSye,message:QtSyW,name:QtSyW):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=QtSye,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=QtSyG,default=QtSyM,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
def cmd_cpvcs_inject(version:QtSye,reset:QtSya,name:QtSyW):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=QtSyG,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=QtSyG,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
@click.option("--lazy/--eager",default=QtSyG,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:QtSyW,inject:QtSya,reset:QtSya,lazy:QtSya):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=QtSyG,default=QtSyM)
def cmd_cpvcs_list_pods(remote:QtSya):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
def cmd_cpvcs_versions(name:QtSyW):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=QtSyG,type=QtSye)
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
def cmd_cpvcs_version_info(version:QtSye,name:QtSyW):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=QtSye,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
def cmd_cpvcs_version_metamodel(version:QtSye,name:QtSyW):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=QtSyG,type=QtSye,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=QtSyG,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=QtSyG,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=QtSyM,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
def cmd_cpvcs_set_version(version:QtSye,inject:QtSya,reset:QtSya,commit_before:QtSya,name:QtSyW):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
def cmd_cpvcs_commits(version:QtSye,name:QtSyW):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=QtSyG)
@click.option("--commit","-c",required=QtSyG)
@click.option("-n","--name",help="Name of the cloud pod",required=QtSyG)
def cmd_cpvcs_commit_diff(version:QtSye,commit:QtSye,name:QtSyW):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(QtSyD)
  status.stop()
...
def print_table(columns:Dict[QtSyW,QtSyW],rows:List[Dict[QtSyW,Any]],formatters:Dict[QtSyW,Callable[[Any],QtSyW]]=QtSyD):
 from rich.table import Table
 if formatters is QtSyD:
  formatters=QtSyC()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  QtSyz(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->QtSyD:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->QtSyD:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->QtSyD:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->QtSyD:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
