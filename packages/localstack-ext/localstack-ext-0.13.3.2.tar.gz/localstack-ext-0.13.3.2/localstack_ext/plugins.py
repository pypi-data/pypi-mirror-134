...
def api_key_configured()->IdnhS:
 return(Idnhq if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else Idnhw)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if IdnhC([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
