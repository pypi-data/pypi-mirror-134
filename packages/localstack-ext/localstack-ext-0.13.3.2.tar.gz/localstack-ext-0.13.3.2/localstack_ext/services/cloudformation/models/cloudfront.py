...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or QGnwq
 @QGnwW
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return QGnwX(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @QGnwS
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if QGnwJ(origins,QGnws)else origins.get("Items")
  origins_to_create=QGnwO([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if QGnwJ(aliases,QGnws)else aliases.get("Items")
  aliases_to_create=QGnwO(aliases)
  for distr in distrs:
   origins=QGnwO([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=QGnwO(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",QGnwp)
   config["Enabled"]=QGnwE(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if QGnwJ(custom_errors,QGnwL)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=QGnwE(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":QGnwp}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=QGnwE(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=QGnwq):
    parent=config if parent is QGnwq else parent
    items=parent.get(key)or[]
    if QGnwJ(items,QGnwL)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":QGnwz(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if QGnwJ(cache_behavs,QGnwL)else cache_behavs)
