...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=tKCfm,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if tKCfv(perm["IpProtocol"])!=tKCfv(rp["IpProtocol"]):
    continue
 ...
 @tKCfb
 def get_vpc_and_name_for_security_group(cls,group_id:tKCfv)->Tuple[tKCfv,tKCfv]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or tKCfm
  group_name=groups and groups[0]["GroupName"]or tKCfm
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",tKCfm)
   vpc_id=tKCfm
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",tKCfm)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=tKCfv(result.get("IpProtocol"))
   description=result.pop("Description",tKCfm)
   cidr_ipv6=result.pop("CidrIpv6",tKCfm)
