...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[uSgsQ])[0]
 def get_physical_resource_id(self,attribute=uSgsQ,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if uSgsd(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return uSgsr(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @uSgsG
 def add_defaults(resource,stack_name:uSgsh):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return uSgsr(SESReceiptRule,self).get_cfn_attribute(attribute)
 @uSgsc
