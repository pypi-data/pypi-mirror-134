...
def selectbox_v1(label,
  ...
  if sidebar:
    ret = st.sidebar.selectbox(label=label, options=options_keys, index=index)
  else:
    ret = st.selectbox(label=label, options=options_keys, index=index)

  ret = options_dict[ret]

  logging.getLogger('st').info(f"{label}={ret}")
  if sidebar:
    st.sidebar.write(f"{ret}")
  else:
    st.write(f"{label}={ret}")
  print(f"{label}={ret}")
  return ret
