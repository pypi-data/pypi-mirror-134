...
def Modification_ann(ann_path, M_Sim, matching_acc, output_dir=None, img_size=None):
    ...
    if output_dir == None:
        ...
        ann_new_path = ann_path.replace('.json', '_aligned.json')
    else:
        ann_name = os.path.basename(ann_path)
        ann_new_path = os.path.join(output_dir, ann_name.replace('.json', '_aligned.json'))
    
    ori_shape = (load_dict['imageWidth'], load_dict['imageHeight'])

    load_dict['imagePath'] = load_dict['imagePath'].replace('.jpg', '_aligned.jpg')
    ...
    load_dict['flags'] = {
        'type': 'Matched and cropped transformation.',
    ...
    shapes_new = copy.deepcopy(shapes)
