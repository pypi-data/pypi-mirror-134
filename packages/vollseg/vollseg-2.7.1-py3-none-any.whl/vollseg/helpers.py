...
def VollSeg_unet(image, model, n_tiles = (2,2), axes = 'YX', noise_model = None, RGB = False, iou_threshold = 0, slice_merge = False):
    ...
    if ndim == 3 and slice_merge:
        ...
        Finalimage = match_labels(Binary, iou_threshold = iou_threshold)           
    else:
...
n_tiles = (1,2,2), UseProbability = True, globalthreshold = 1.0E-5, extent = 0, dounet = True, seedpool = True, save_dir = None, Name = 'Result',  startZ = 0, slice_merge = False):
    ...
    if dounet:
        ...
        if unet_model is not None:
            ...
            Mask = UNETPrediction3D(image, unet_model, n_tiles, 'ZYX', iou_threshold = nms_thresh, slice_merge = slice_merge )
    ...
    smart_seeds = RemoveLabels(smart_seeds) 
    ...
    Skeleton = SmartSkel(Sizedsmart_seeds, Sizedproabability_map) 
    if save_dir is not None:
        print('Saving Results and Done')
        imwrite((stardist_results + Name+ '.tif' ) , star_labels.astype('uint16'))
