...
def prepare_tagged_df_for_training(
    ...
    remove_columns(df, [const.ID, *const.REQUIRED_COLS])
    reorder_cols(df, [const.ID, *const.REQUIRED_COLS])

    input_file_dir, input_file_name = os.path.split(df_path)
    input_file_name, input_file_ext = os.path.splitext(input_file_name)
    _, update_path = tempfile.mkstemp(
        dir=input_file_dir,
        prefix=f"{input_file_name}-update-{dataset_type}",
