...
class _LinearLabelSimulate(_BaseSimulator[_LinearLabelModel]):
    ...
    def get_label_position(self, compound: str, position: int) -> Optional[np.ndarray]:
        ...
        if res is None:
            return None
        return cast(Dict[str, np.ndarray], res)[self.model.isotopomers[compound][position]]

    def get_label_distribution(self, compound: str) -> Optional[np.ndarray]:
        ...
        return cast(np.ndarray, res.loc[:, compounds].values)
