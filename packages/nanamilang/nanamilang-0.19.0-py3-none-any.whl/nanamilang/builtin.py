...
def meta(data: dict):
    """
    NanamiLang, apply meta data to a handle function
    ...
    return wrapped  # <- register builtin function/macro handle to access it inside of the NanamiLang
...
class BuiltinMacros:
    ...
    def if_macro(tree: list, local_env: dict, _, eval_function) -> list or Token:
        ...
        return then_branch if evaluated.truthy() is True else else_branch  # <- return responding branch
    ...
    def cond_macro(tree: list, local_env: dict, _, eval_function) -> list or Token:
        ...
        for [cond, expr] in plain2partitioned(tree):
            ...
            if evaluated.truthy():
                return expr  # <- return expression if responding condition evaluated is truthy
    ...
    def fn_macro(tree: list, local_env: dict, _, eval_function) -> list or Token:
        ...
        local_env.update(payload)  # <- update local environment, this AST will be aware of our function
    ...
    def defn_macro(tree: list, local_env: dict, module_env, eval_function) -> list or Token:
        ...
        handle = lambda args: fni.handle(tuple(args))  # <- due to some limits we can not use fni.handle directly
        ...
        for [idn_token_or_form, dtype_token_or_form] in NML_M_ITERATE_AS_PAIRS(bindings_form, 2):
            ...
            if isinstance(idn_token_or_form, list):
                if NML_M_FORM_IS_A_VECTOR(idn_token_or_form):
                    ...
                    NML_M_ASSERT_COLL_FORM_ITEMS_COUNT_IS(
                        idn_token_or_form,
                        evaluated.count().reference(), 'let: lengths of left and right-sides vectors mismatching')
                    ...
                    NML_M_ASSERT_FORM_IS_A_VALID_HASHMAP(idn_token_or_form, 'let: cannot destruct invalid HashMap')
                    assert isinstance(evaluated, datatypes.HashMap), 'let: right-side needs to be a HashMap struct'
                    ...
                    NML_M_ASSERT_FORM_IS_A_VECTOR(_[1], message='let: the vector we was expected is not a Vector!')
                    ...
                    if _[0].dt().reference() == 'strs':
                        ...
                        raise AssertionError('let: invalid key was specified: supported now are: :keys, and :strs')
                else:
                    raise AssertionError('let: given form is invalid, or we does not support destructuring on it,')
            else:
                assert idn_token_or_form.type() == Token.Identifier,    'let: the left-side needs to be Identifier'
