...
def find_all_by_name(name, case_sensitive= False, inspy_logger_device= None, on_the_dl=False, colorful_logging=False):
    ...
    for proc in psutil.process_iter():
        try:
            ...
            proc_info = proc.as_dict(attrs=['pid', 'name', 'create_time', 'username'])
            log.debug(proc_info)
            ...
            if name in proc_name:
                ...
                log.debug(f'{colors.green}Added {proc_info["pid"]} to found process list')
    ...
    for proc in kill_list:
        ...
        try:
            ...
            if missing >= 20:
                ...
                break
        log.debug(f'{colors.green}Kill signal sent to {colors.yellow}{proc["pid"]}')
...
def kill_all_by_name(name, case_sensitive=False, inspy_logger_device=None, on_the_dl=False, colorful_logging=False):
    ...
    procs = find_all_by_name(name, case_sensitive, inspy_logger_device, on_the_dl, colorful_logging)
