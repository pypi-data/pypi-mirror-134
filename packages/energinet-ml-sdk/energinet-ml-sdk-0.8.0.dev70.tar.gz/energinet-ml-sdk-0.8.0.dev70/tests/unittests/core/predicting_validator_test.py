...
class TestPredictionController:
    def test__predictions_returned_in_correct_order(self):
        ...
        assert result.validations[0].get("state") == ValidatorStatus.NOT_VALID
        assert result.validations[1].get("state") == ValidatorStatus.VALID
        assert result.validations[-1].get("state") == ValidatorStatus.NOT_VALID
