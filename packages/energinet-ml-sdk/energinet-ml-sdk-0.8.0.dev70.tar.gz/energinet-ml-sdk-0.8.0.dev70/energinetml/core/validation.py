...
class Validator:
    ...
    def get_features(self) -> Dict[str, Tuple["FeatureType", ...]]:
        ...
        """
        return {
            feature.name: (feature.type.value, ...) for feature in self.feature_models
        }
...
class Feature(ABC):
    ...
    def check_meta_data_attr(self) -> None:
        """An internal method called before returning the validator."""
        if self.meta_data is None:
            raise FeatureMetaDataMissing(
                name=self.name,
                message="Not possible to return a validator for 'meta_data = None'.",
            )
...
class IntegerFeature(Feature):
    ...
    type: "FeatureType" = FeatureType.INT

    def describe_func(
        self, x: Union[pd.Series, np.ndarray], x_percentile: float = 0.95
    ) -> None:
        """This method is used to update the meta_data dictionary for the
        default integer feature validation.

        Args:
            x (pd.Series): A pandas series with integer (or float) values.
        ...
        warn_low, warn_high = np.percentile(
            x, q=[((1 - x_percentile) / 2) * 100, (1 - (1 - x_percentile) / 2) * 100]
        )
...
class FloatFeature(IntegerFeature):
    ...
    type: FeatureType = FeatureType.FLOAT
...
class CategoricalFeature(StringFeature):
    ...
    ordered: bool = False
    type: FeatureType = FeatureType.CATEGORY
...
class DatetimeFeature(Feature):
    ...
    type: FeatureType = FeatureType.DATETIME
