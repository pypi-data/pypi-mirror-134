...
from kameleoon.exceptions import ConfigurationNotFoundException, CredentialsNotFoundException, \
    ExperimentConfigurationNotFound, NotTargeted, NotActivated, VariationConfigurationNotFound,\
...
class KameleoonClient:
    ...
    def _fetch_graphql(self) -> None:
        ...
        self.obtain_feature_flags_graphql(self.site_code)
    ...
    def obtain_feature_flags_graphql(self, site_code: str, per_page=-1) -> Optional[List[Dict[str, Any]]]:
        """
        ...
        feature_flags = self._fetch_all_graphql(f"{self.automation_base_url}v1/graphql?perPage={per_page}",
                                                data=query_ql_feature_flags(site_code),
                                                headers=self._get_headers())
