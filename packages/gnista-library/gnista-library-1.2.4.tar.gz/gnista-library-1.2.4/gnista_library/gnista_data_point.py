...
class GnistaDataPoint:
    ...
    def create_new(cls: Type[T], connection: GnistaConnection, name: str, tags: List[str]) -> Union[T, None]:
        ...
        client = AuthenticatedClient(base_url=connection.datapoint_base_url, token=token, verify_ssl=False)
        ...
        return cls(connection=connection, data_point_id=str(new_id), name=name)
    ...
    def set_data_point_data(
        ...
        token = self.connection.get_access_token()
        client = AuthenticatedClient(base_url=self.connection.datapoint_base_url, token=token, verify_ssl=False)
