"""
Create Python setup.py file from pyproject.toml file
"""
import toml

pyproject = toml.load("pyproject.toml")

if isinstance(pyproject["tool"]["poetry"], dict):
    endpoint = pyproject["tool"]["poetry"]
...
# This is the name of your project. The first time you publish this package,
# this name will be registered for you. It will determine how users can install this project, e.g.:
#
...
version = endpoint["version"]  # Required
...
#
# Optional if long_description is written in reStructuredText (rst) but required for plain-text or Markdown;
# if unspecified, "applications should attempt to render [the long_description] as text/x-rst;
# charset=UTF-8 and fall back to text/plain if it is not valid rst" (see link below)
#
...
#
# Note that this is a list of additional keywords, separated by commas,
# to be used to assist searching for the distribution in a larger catalog.
...
#
# Alternatively, if you just want to distribute a single Python file, use the `py_modules` argument instead as follows,
# which will expect a file called `my_module.py` to exist:
#
#   py_modules=["my_module"],
#
packages = (find_packages(where="src"),)  # Required

# Specify which Python versions you support. In contrast to the 'Programming Language' classifiers above,
...
# Although 'package_data' is the preferred approach, in some case you may need to place data files outside of your packages.
...
#
# Examples listed include a pattern for specifying where the package tracks issues,
# where the source is hosted, where to say thanks to the package maintainers,
# and where to support the project financially. The key is what's used to render the link text on PyPI.
project_urls = (
    {  # Optional
...
script = endpoint["scripts"]
command = [*script]

dependencies = endpoint["dependencies"]
devdependencies = endpoint["dev-dependencies"]
