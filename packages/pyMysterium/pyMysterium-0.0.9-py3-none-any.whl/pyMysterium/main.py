...
class Mysterium:
    ...
    def create_new_order(self, lightning_network: bool = False, myst_amount: float = 0, pay_currency: str = "LTC") -> dict:
        minimum_amount = self.payment_order_options()[0]['order_options']['minimum']
        if myst_amount <= minimum_amount:
            raise MinimumAmountError(
                f"Amount must be greater than {minimum_amount}")
        data = {
    ...
    def __set_channel_address(self):
        previous_orders = self.get_all_orderes()
        if len(previous_orders) < 1:
            minimum = self.payment_order_options()[0]['order_options']['minimum']
            response = self.create_new_order(myst_amount=minimum*1.2)
            # self.channel_id = response['channel_address']
            self.channel_id = 'Myst did a shit job here..!'
        else:
            self.channel_id = previous_orders[0]['channel_address']
