...
def load_data(df: None, args):
  ...
  if data_smpl_mode == True:
    ...
    df = df.sample(n = data_smpl_pop, random_state = data_smpl_ran)
  return df
...
def detect_sample(df, args):
  ...
  #------------------------------------#
  thrs_per = args["mode_pres_detc_thrs_per"]
  thrs_min = args["mode_pres_detc_thrs_min"]
  lens_alls = len(df)
  lens_thrd = int(lens_alls * thrs_per)
  #------------------------------------#
  if thrs_per == -1: #Manual Mode - Complete Dataset
    sample = {"smpl_mode": False, "smpl_pops": 0 }
  elif thrs_per == 0:
...
def detect_types(df, args):
  df_types = pd.DataFrame(df.dtypes).reset_index().rename(columns={"index": "feature_name", 0: "feature_type"})

  type_num = ["int32","int64","float32","float64"]
  type_str = ["string", "object"]
  type_cat = ["bool"]

  def transform(feature_name, feature_type):
    if str.lower(str(feature_type)) in type_num:
      df_types_thes = args['mode_pres_detc_thrs_typ']
      if (1.*df[feature_name].nunique()/df[feature_name].count() < df_types_thes): #or some other threshold
        ...
        return "Numeric"
    elif str.lower(str(feature_type)) in type_str:
      df_types_thes = args['mode_pres_detc_thrs_typ']
      if (1.*df[feature_name].nunique()/df[feature_name].count() < df_types_thes): #or some other threshold
        return "Categorical"
      elif (1.*df[feature_name].nunique()/df[feature_name].count() >= df_types_thes): #or some other threshold
        return "String"
    elif str.lower(str(feature_type)) in type_cat:
       return "Categorical"
  df_types['feature_type'] = df_types.apply(lambda x: transform(x.feature_name, x.feature_type), axis=1)
  return df_types
...
def detect_features(df, args):
  ...
  df_types_nums_temp = df_types[(df_types["feature_name"] != df_class) & (df_types["feature_type"] == "Numeric")]
  if df_types_nums_temp.empty == False:
    df_types_nums = df_types_nums_temp["feature_name"].tolist()
  df_types_strs_temp = df_types[(df_types["feature_name"] != df_class) & (df_types["feature_type"] == "String")]
  if df_types_strs_temp.empty == False:
    df_types_strs = df_types_strs_temp["feature_name"].tolist()
  df_types_cats_temp = df_types[(df_types["feature_name"] != df_class) & (df_types["feature_type"] == "Categorical")]
...
def detect_area(df, args):
  df_types = detect_types(df, args)
  df_class = args["mode_pres_cols_clas"]
  df_types_clas = df_types[(df_types["feature_name"] == df_class)]["feature_type"].iloc[0]
...
def data_impute(df, dataset_columns_nums, dataset_columns_strs, dataset_columns_cats, mode_type, mode_labl):
  dataset_columns_alls = dataset_columns_nums + dataset_columns_strs + dataset_columns_cats
  filled_cols = []
  missed_cols = []
  ...
  ####################################################
  impute_cols = set(dataset_columns_xxxs)-set(missed_cols)
  for feature in impute_cols:
    print("Copying feature: {0}".format(feature))
    df_det_xxxs[feature] = df[feature]
  ####################################################
  impute_cols = set(dataset_columns_xxxs)-set(filled_cols)
  for feature in impute_cols:
    print("Imputing feature: {0}".format(feature))
    df_det_xxxs[feature] = df[feature + "_imp"]

    parameters_nums = list(set([feature + '_imp' for feature in dataset_columns_nums]) - {feature + '_imp'})
    parameters_strs = list(set([feature + '_imp' for feature in dataset_columns_strs]) - {feature + '_imp'})
    parameters_cats = list(set([feature + '_imp' for feature in dataset_columns_cats]) - {feature + '_imp'})
  ...
  if mode_intr_meth == "trai":
    ...
    if args['intr_keep_cols'] == 0:
      if mode_pres_cols == "choose":
        selected_columns_nums = list(set(args["mode_pres_cols_nums_chs"]))
        selected_columns_strs = list(set(args["mode_pres_cols_strs_chs"]))
        selected_columns_cats = list(set(args["mode_pres_cols_cats_chs"]))
        selected_columns_clas = args["mode_pres_cols_clas"]
      elif mode_pres_cols == "substruct":
        selected_columns_nums = list(set(args["mode_pres_cols_nums_all"]) - set(args["mode_pres_cols_nums_sub"]))
        selected_columns_strs = list(set(args["mode_pres_cols_strs_all"]) - set(args["mode_pres_cols_strs_sub"]))
        selected_columns_cats = list(set(args["mode_pres_cols_cats_all"]) - set(args["mode_pres_cols_cats_sub"]))
        selected_columns_clas = args["mode_pres_cols_clas"]
      elif mode_pres_cols == "complete":
        selected_columns_nums = list(set(args["mode_pres_cols_nums_all"]))
        selected_columns_strs = list(set(args["mode_pres_cols_strs_all"]))
        selected_columns_cats = list(set(args["mode_pres_cols_cats_all"]))
        selected_columns_clas = args["mode_pres_cols_clas"]
        ...
        df_cols_info = detect_cols_missing(df)
        print("Cols Information:")
        print(df_cols_info)
        print("-----------------------------------")
        thes = args["mode_pres_detc_thrs_mis"]
        drop_cols = df_cols_info[df_cols_info["Ratio"] > thes].index.tolist()
    ...
    #Mode of handling missing values
    print("====================== Rows Selected ======================")
    ...
    if mode_pres_rows == "remove":
      ...
      mode_pres_rows_labl = args['mode_pres_rows_labl']
      df_nums = data_impute(df, selected_columns_nums, selected_columns_strs, selected_columns_cats, 'nums', mode_pres_rows_labl)
      df_strs = data_impute(df, selected_columns_nums, selected_columns_strs, selected_columns_cats, 'strs', mode_pres_rows_labl)
      df_cats = data_impute(df, selected_columns_nums, selected_columns_strs, selected_columns_cats, 'cats', mode_pres_rows_labl)
      df_clas = df[selected_columns_clas]
...
def preprocessing(df_nums, df_strs, df_cats, df_clas, args, encos):
  ...
  if mode_prep_bala_method == "smote":
    if mode_intr_meth == "trai":
      oversample = SMOTE()
