...
class DenMune():
    ...
    def __init__ (self, train_data, test_data=None, labels_true=None, file_2d ='_temp_2d', k_nearest=1, rgn_tsne=False, prop_step=0, delimiter=',', verpose=True):     
        ...
        self.analyzer["n_points"]["size"] = data.shape[0] # this will changed in preplot when we plot only train or test data
        ...
        if data.shape[1] != 2 and file_2d == '_temp_2d':
            ...
            data = genfromtxt(file_2d , delimiter=self.delimiter)
    ...
    def fit_predict(self, validate=True ):
        ...
        labels_dic = {}
        self.labels_train = pred_list [:self.train_sz]
        self.labels_test = pred_list[self.train_sz:]
        labels_dic ['train'] = self.labels_train
        labels_dic ['test'] = self.labels_test

        if validate:
          validity_scores = self.validate_Clusters()

        return labels_dic, validity_scores
        
       

    def match_Labels(self, labels_true):
       
        labels_pred = self.labels_train
    ...
    def validate_Clusters(self ):
        ...
        labels_pred = self.match_Labels(labels_true)
    ...
    def preplot_Clusters(self, data_type=None):
        ...
        if data_type == 'test':
          ...
          self.analyzer["n_points"]["size"] = self.test_sz
        elif data_type == 'train':
          labels = self.labels_train
          self.analyzer["n_points"]["size"] = self.train_sz
        elif data_type == 'augmented':
          labels = list(itertools.chain(self.labels_train, self.labels_test))
    ...
    def plot_clusters(self, data_type='train', show_noise=False, ground=False):
        data2 = []
        colors2 = []
