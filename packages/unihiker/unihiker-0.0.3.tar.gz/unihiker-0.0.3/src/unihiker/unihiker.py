...
class GUI():
    ...
    def __init__(self):
        def mainloopThread():
            ...
            style.configure('TButton', font=('HYQiHei', 11))
            GUI.masterGlobal = style.master
            GUI.masterGlobal.geometry('240x320')
            GUI.masterGlobal.resizable(0, 0)
            GUI.masterGlobal.mainloop()
        thd = Thread(target=mainloopThread)   # gui thread
        ...
        self.canvas = ttk.Canvas(self.frame, bd=0, highlightthickness=0)
