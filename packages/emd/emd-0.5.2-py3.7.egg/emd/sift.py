...
from . import spectra
from .logger import get_level, sift_logger, wrap_verbose
...
def ensemble_sift(X, nensembles=4, ensemble_noise=.2, noise_mode='single',
    ...
    if nprocesses > 1:
        with initialise_parallel(n_jobs=nprocesses, verbose=get_level()) as parallel:
            res = parallel(delayed(pfunc)() for ii in range(nensembles))
    else:
        res = [pfunc() for ii in range(nensembles)]
...
def complete_ensemble_sift(X, nensembles=4, ensemble_noise=.2,
    ...
    with initialise_parallel(n_jobs=nprocesses, verbose=get_level()) as parallel:
        ...
        res = parallel(delayed(pfunc)(noise=noise[:, ii, np.newaxis]) for ii in range(nensembles))
    ...
    while continue_sift:
        ...
        with initialise_parallel(n_jobs=nprocesses, verbose=get_level()) as parallel:
            ...
            pfunc = functools.partial(sift, sift_thresh=sift_thresh,
                                      ...
                                      envelope_opts=envelope_opts,
                                      extrema_opts=extrema_opts)
            res = parallel(delayed(pfunc)(noise[:, ii, np.newaxis]) for ii in range(nensembles))
...
def get_next_imf_mask(X, z, amp, nphases=4, nprocesses=1,
    ...
    with initialise_parallel(n_jobs=nprocesses, verbose=get_level()) as parallel:
        res = parallel(delayed(pfunc)(X+m[:, ii, np.newaxis]) for ii in range(nphases))
...
def initialise_parallel(n_jobs=1, verbose=None):
    """Set up and return a joblib Parallel instance."""
    # EMDs verbose has levels 10, 15, 20, 30, 40, 50 in decreasing verbosity
    # Parallel also uses 0 to 50 but the frequency of the messages increases with level
    # We set a sensible Parallel verbose level depending on the current emd logging level
    emd_log_level = get_level()

    if emd_log_level < 20:
        # EMD is VERBOSE or DEBUG - Set Parallel to 10
        parallel_log_level = 10
    elif emd_log_level < 30:
        # EMD is INFO - set Parallel to 5
        parallel_log_level = 5
    else:
        # EMD is WARNING, ERROR or CRITICAL - set Parallel to silent
        parallel_log_level = 0

    return Parallel(n_jobs=n_jobs, verbose=parallel_log_level)
