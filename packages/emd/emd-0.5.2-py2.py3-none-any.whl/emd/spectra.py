...
def quadrature_transform(X, fix_zerocrossings=False):
    ...
    """
    nX = utils.amplitude_normalise(X.copy(), clip=False)
...
def hilberthuang(IF, IA, edges=None,
    ...
    Notes
    -----
    Run a HHT with an automatically genereated set of histogram bins:
    ...
    reccommended for very large HHTs
...
def holospectrum(IF, IF2, IA2,
    ...
    Notes
    ------
    Run a Holospectrum with an automatically genereated set of histogram bins:
    ...
    result in a memory efficient sparse array - this is strongly reccommended
    for very large HHTs
...
def hilbertmarginal(IF, IA, edges=None, bins2=None, edges2=None, order=2,
                    sample_rate=1, return_sparse=False):
    ...
    edges, bins = _histogram_bin_relay(edges, IF.flatten())
    edges2, bins2 = _histogram_bin_relay(edges2, IA.flatten())

    hima = _higher_order_spectra(IF,
                                 IA[:, :, None],
                                 np.ones_like(IA[:, :,  None]),
                                 edges, edges2)

    # Create PDF histogram
    hima = hima.sum(axis=(2, 3))
    hima = hima / IF.size

    dA = np.diff(bins2)[0]
    A_values = bins2[None, :, None, None]**order
    hima = hima * A_values * dA

    hima = _post_process_spectra(hima, 2,
                                 sum_time=False,
                                 sum_imfs=False,
                                 mode=None,
