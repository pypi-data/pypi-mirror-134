...
class mvp(Technical_Indicator):
    class w:
        stop_trading_already = 'stop trading already'

    def __init__(self, symbol='', date='', period_seconds=10, volume_burst=1000.0, volume_lot=1000.0, volume_moving=1000.0, yesterday_close=0.0, volume_burst_30s=1000.0, volume_lot_30s=1000.0, rule=1):
        cprint(f'MVP: symbol: {symbol}, date: {date}, period second: {period_seconds}, volume burst: {volume_burst}, volume burst_30s: {volume_burst_30s}, volume lot: {volume_lot}, volume moving: {volume_moving}', 'yellow')
    ...
    def add_mvp_index(self):
        ...
        if self.mvp_index.shape[0] == 0:
            ...
            if volume > self.volume_burst_30s:
                if close < self.price_yesterday_close:
                    ...
                    burst = 1
            self.mvp_index = pd.DataFrame({'ts': [self.mvp_tmp['ts'][0]], 'volume': [volume], 'high': [high], 'low': [low], 'close': [close], 'score': [0], 'ratio': [ratio], 'mvp_ratio': [ratio * 2], 'burst': [burst], 'lot': [lot]})
            return
        ...
        df_tmp = pd.DataFrame({'ts': [self.mvp_tmp['ts'][0]], 'volume': [volume], 'high': [high], 'low': [low], 'close': [close], 'score': [score], 'ratio': [ratio], 'mvp_ratio': [mvp_ratio], 'burst': [burst], 'lot': [lot]})
    ...
    def draw(self, symbol, date, filename):
        ...
        plt.savefig(f'{folder}/{burst_count}-{symbol}-{date.replace("/", "-")}.png')
        fig.clf()
        plt.close('all')
