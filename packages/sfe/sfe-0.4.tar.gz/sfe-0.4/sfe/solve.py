...
class solve(object):
    def __init__(self):
        pass
    def solve_static_elastic(self,c):
    ...
    def solve_dynamic_eigen(self,c):
    ...
    def solve_newmark_beta(self,c,omega,pxi1=0.02,pxi2=0.04,n1=256,delta=0.5,beta=0.25):
    ...
    def moca(self,K,M,Omega,n,n_0,freq1=253.52,freq2=1582.96,rho=7850,b=0.06, h=0.007,L=0.15,R=0.35,y1=0.04,y2=0.04,Amp_ex=500,kjq=8e6,miu=0.2,alfa=0.25,beta=0.5):
        '''
        L:长度；l：单元长度；R：轮盘半径；b：宽度；h：厚度；A：截面面积；rho：密度；
        niu：泊松比；k：剪切矫正因子；n：单元个数；sdof：自由度个数；
        I：截面惯性矩；G：剪切模量；E：弹性模量；kjq：接触面剪切刚度；
        miu：榫头榫槽接触面摩擦系数；
        sunalpha/sunbeta：接触面与竖直方向夹角
        Omega：叶片转速；n_0：前排静止叶片数；
        Amp_ex：外激励幅值；
        y1、y2：前两阶固有频率得阻尼系数
        '''
