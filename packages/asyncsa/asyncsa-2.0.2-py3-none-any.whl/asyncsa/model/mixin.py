...
class Mixin:
    ...
    def return_key(cls):
        if not cls._return_key:
            for k in cls.__table__.columns:
                if hasattr(k, "primary_key") and getattr(k, "primary_key"):
    ...
    def to_dict(self):
        res = {k.name: getattr(self, k.name) for k in self.__table__.columns}
        return res

    async def save(self):
        await self.manager().instance(self, model=self.__class__, q_type="update")
        return self
    ...
    def get_d_values(cls):
        if not cls.d_values:
            cls.d_values = {
                k.name: cls.handle_default_value(k.default.arg)
                for k in cls.__table__.columns
    ...
    async def select(cls, **kwargs):
        result = await cls.manager().select_by_param(params=kwargs, model=cls)
        return cls.instancefy(result)

    @classmethod
    def build_query(cls, **kwargs):
        result = cls.manager().query(params=kwargs, model=cls)
        print("build query", result, type(result))
        return result

    @classmethod
    async def execute_query(cls, query=None):
        result = await cls.manager().select(query=query)
        return cls.instancefy(result)

    async def delete(self, *args, **kwargs):
        await self.manager().instance(self, model=self.__class__, q_type="delete")
