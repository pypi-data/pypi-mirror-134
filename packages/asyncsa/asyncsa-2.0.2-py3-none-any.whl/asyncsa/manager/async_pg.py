...
class PostgresManager(BaseManager):
    ...
    async def parse_compare(self, k, v, model):
        if "__" in k:
    ...
    async def gen_model_query(self, params=None, model=None):
        query_judge = None
        for k, v in params.items():
            if hasattr(model.__table__.c, k):
                if query_judge is not None:
                    query_judge = query_judge & (getattr(model.__table__.c, k) == v)
                else:
                    query_judge = getattr(model.__table__.c, k) == v
        query = model.__table__.select().where(query_judge)
        return query

    async def get_by_param(self, params=None, model=None):
        query = await self.gen_model_query(params=params, model=model)
