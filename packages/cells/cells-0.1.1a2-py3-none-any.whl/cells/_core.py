...
class Cells(BaseCells):
    def __init__(self, unicode_version: str = "9.0.0", cache_size: int = 4096) -> None:
        """
        Args:
        ...
        if unicode_version not in SUPPORTED_UNICODE_VERSIONS:
            raise UnsupportedUnicodeVersion(f"Unicode version {unicode_version} not supported. "
                                            f"Supported Unicode versions: {SUPPORTED_UNICODE_VERSIONS}")
        self.unicode_version: str = unicode_version

        database_module = (
            f".databases.unicode_{unicode_version.replace('.', '')}"
        )
    ...
    def measure(self, text: str) -> int:
        ...
        if _is_single_cell_widths(text):
            return len(text)
        cached_result: int = self._cache.get(text)
