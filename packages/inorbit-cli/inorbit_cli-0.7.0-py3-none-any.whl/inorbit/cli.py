...
def delete(ctx, filename):
    ...
    logger.debug(f"Configuration read: {inorbit_config}")
    required_keys = ["kind", "apiVersion", "metadata"]
    inorbit_config = {k: inorbit_config[k] for k in required_keys}

    logger.debug(f"Object required data: {inorbit_config}")

    try:
        logger.debug("Clearing configuration")
        _ = client.http_post("/configuration/clear", post_data=inorbit_config)
...
def apply_config(filename):
    ...
    try:
        ...
        _ = client.http_post("/configuration/apply", post_data=inorbit_config)
    except inorbit.exceptions.InOrbitError as ex:
        raise click.ClickException(ex)
...
@click.option("--scope", "scope", type=str, default="company", help="Configuration object scope.")
@click.option("--dump", "dump", default=None, help="Output file name.")
...
def get_config(kind, scope, dump, config_id):
    ...
    if dump:
        click.echo(f"Creating dump file '{dump}'")
        # Responses with multiple items are dump as separate json documents
        # For queries with a single response this generate a valid json document
        with open(dump, "w") as fd:
            for r in response["items"]:
                fd.write(json.dumps(r, indent=3))
        click.echo(f"File '{dump}' created.")
        return
...
def delete_config(config_id, kind, scope):
    """Clear configuration with the provided id. The default `company` scope
