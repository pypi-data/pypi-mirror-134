...
def _poly_hand_granade(_obj):
    ...
    if isinstance(_obj.series.index, (bluebelt.core.index.DatetimeMultiIndex, bluebelt.core.index.IsoDatetimeMultiIndex, bluebelt.core.index.DayOfYearMultiIndex)):
        index = np.array([d.timestamp() for d in _obj.series.index.to_datetimeindex()], dtype='int')
        index = index - index.min()
        index = index / np.gcd.reduce(index)

        # drop nan values
        _index = np.array([d.timestamp() for d in _obj.series.index.to_datetimeindex().dropna()], dtype='int')
        _index = _index - _index.min()
        _index = _index / np.gcd.reduce(_index)

    elif isinstance(_obj.series.index, pd.DatetimeIndex) and _obj.series.index.freqstr == "D":
        index = np.array([d.timestamp() for d in _obj.series.index], dtype='int')
        index = index - index.min()
        index = index / np.gcd.reduce(index)

        # drop nan values
        _index = np.array([d.timestamp() for d in _obj.series.dropna().index], dtype='int')
        _index = _index - _index.min()
        _index = _index / np.gcd.reduce(_index)

    else:
        index = np.arange(0, _obj.series.size)
        _index = np.arange(0, _obj.series.dropna().size)
...
def _pattern_plot(_obj, **kwargs):
    ...
    if residuals:
        ...
        # residuals histogram
        bluebelt.core.graph.hist(_obj.residuals.values, ax=ax2)
        ax2.set_yticks([])
        ...
        bluebelt.core.graph.dotted_line(pdf_x, stats.norm.pdf(pdf_x, norm_mu, norm_std), ax=ax2)
    ...
    # pattern
    bluebelt.core.graph.line(_obj.result.index, _obj.result.values, ax=ax1, zorder=95, label='pattern')
    
    if bounds:
        # observations
        bluebelt.core.graph.observations(_obj.within_bounds.index, _obj.within_bounds.values, ax=ax1, zorder=99, label='observations')

        # bounds
        bluebelt.core.graph.fill_between(_obj.series.index, _obj.lower.values, _obj.upper.values, ax=ax1, zorder=90, label=f'{(_obj.confidence * 100):1.0f}% bounds')
        bluebelt.core.graph.dotted_line(_obj.series.index, _obj.lower.values, ax=ax1, zorder=92, label=None)
        bluebelt.core.graph.dotted_line(_obj.series.index, _obj.upper.values, ax=ax1, zorder=92, label=None)
        
        # out of bounds
        bluebelt.core.graph.out_of_bounds(_obj.out_of_bounds.index, _obj.out_of_bounds.values, ax=ax1, zorder=85, label='out of bounds')
    else:
        # observations
        bluebelt.core.graph.observations(_obj.within_bounds.index, _obj.within_bounds.values, ax=ax1, zorder=99, label='observations')

        # out of bounds
        bluebelt.core.graph.observations(_obj.out_of_bounds.index, _obj.out_of_bounds.values, ax=ax1, zorder=85, label='out of bounds')

    # outliers
    bluebelt.core.graph.outliers(_obj.outliers.index, _obj.outliers.values, ax=ax1, zorder=80, label='outliers')
    ...
    if isinstance(_obj.series.index, pd.MultiIndex):
        bluebelt.core.helpers._get_multiindex_labels(ax1, _obj.series, style)
