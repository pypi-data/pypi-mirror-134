...
def get_page_count_from_server(server_url,page_size,uploader):
...
def download_data_from_server(server_url,page_size,save_csv_path="",save_rawdata_folder="",func_process=None,uploader=""):

    api_url = f"{server_url}/api/Data"
    list_model = []
    fields = []
    page_count=get_page_count_from_server(server_url,page_size,uploader)
    ...
    for page_index in tqdm(range(1,page_count+1)):# page count
        ...
        for row in page["DataTable"]:
            ...
            if save_rawdata_folder!="":
                ...
                html_str=quick_html_page(download_fulltext_url)
