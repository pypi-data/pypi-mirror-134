...
def submit_page_list(server_url, user_id,csv_file,save_html_folder, use_md5url_as_id=False, driver_path="browsers/chromedriver.exe",tag="",language=""):
    ...
    for idx,model in enumerate(list_model):
        ...
        url = model["real_url"]
        try:
            upload_to_server(server_url,user_id, url,use_md5url_as_id=use_md5url_as_id, driver_path=driver_path,tag=tag,language=language,save_folder=save_html_folder)
            print()
        except:
            print("Error in calling API!")
            print()

def submit_url_list(server_url, user_id,list_url,use_md5url_as_id=False, driver_path="",tag="",language="",save_html_folder="html_data"):
