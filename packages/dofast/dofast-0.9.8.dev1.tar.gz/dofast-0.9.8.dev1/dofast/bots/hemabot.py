...
class PcloudProcessor(TextProcessor):
    ...
    def match(self, text: str) -> bool:
        file_formats = 'mp4/mp3/jpg/png/bin/u3m8/gif/pdf/docx/doc/ppt/pptx/xls/xlsx/txt/mkv/avi/mpg/mpeg/mov'
    ...
    def run(self, text: str, update: Update) -> str:
        ...
        while TIMEOUT >= 0:
            if task.status in EXCEPTION_STATES:
                if MAX_FAILURE <= 0:
                    return 'FAILED'
                msg = 'FAILED. Task {} failed with status {}. Retrying ...'.format(
                    text, task.status)
...
class Psycho:
    ...
    def text_handler(self, update: Update, context: CallbackContext) -> None:
        ...
        import threading
        threading.Thread(target=TextClient.run_in_order, args=(__processors, text, update, ),
                         daemon=True).start()
