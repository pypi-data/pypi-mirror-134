...
from DKCommon.request_extensions import get_session, HeaderAuth
...
class DKApiHelper:
    ...
    def refresh_token(self):
        ...
        with self.session.get(f"{self.url}/v2/validatetoken", auth=self.auth) as r:
            if r.status_code == 200:
                new_auth = HeaderAuth(r.json()["token"])
    ...
    def valid_token(url: str, token: str, request_timeout=_DEFAULT_TIMEOUT) -> Union[bool, str]:
        ...
        try:
            response = session.get(endpoint, auth=HeaderAuth(token))
        except Exception:
