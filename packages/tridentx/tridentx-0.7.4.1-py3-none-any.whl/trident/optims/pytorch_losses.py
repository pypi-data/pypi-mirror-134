...
class ActiveContourLoss(_ClassificationLoss):
    ...
    def region(self,output, target, u=1):
        B,C,H,W=output.shape
        label = target.to(output.dtype)
        c_in = torch.ones_like(output).to(output.dtype)
        c_out = torch.zeros_like(output).to(output.dtype)
        region_in = torch.abs(torch.sum(output * ((label - c_in) ** 2),dim=(2,3)))
        region_out = torch.abs(torch.sum((1 - output) * ((label - c_out) ** 2),dim=(2,3)))
    ...
    def elastica(self,input, a=1, b=1):
        ...
        elastica = torch.sum((a + b * (curvature ** 2)) * torch.abs(length),dim=(2,3))
        return elastica/(H*W)
    ...
    def calculate_loss(self, output, target, **kwargs):
        ...
        if self.is_logsoftmax:
            output=exp(output)
        length=self.elastica(output, a=1, b=1)
        region=self.region(output, target, u=1)
        loss=length+self.lambdaP*region
