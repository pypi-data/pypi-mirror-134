...
from banner.utils.neware import calculate_neware_columns, calculate_dq_dv, merge_cache
...
@__cache_query
def neware_query(device: int, unit: int, channel: int, test: int, connection: Union[Connection, str] = None, cache_connection=None, ttl=None, raw=False, dqdv=False, condition: str = '1', temperature: bool = True) -> pd.DataFrame:
    '''
    ...
    try:
        if not raw:
            data = calculate_neware_columns(data, temperature=not aux_data.empty)
            
        if dqdv:
            data['dqdv'] = calculate_dq_dv(data)
...
@__cache_query
def neware_query_by_test(table: str, cell: int, test: int, connection: Union[Connection, str] = None, cache_connection=None, ttl=None, raw=False, dqdv=False, condition: str = '1', temperature: bool = True) -> pd.DataFrame:
    '''
        Queries a given Connection(ip)/str of a known connection (or first known) return result as DataFrame
    ...
    data = neware_query(*neware_keys, raw=raw, dqdv=dqdv, condition=condition, temperature=temperature)
