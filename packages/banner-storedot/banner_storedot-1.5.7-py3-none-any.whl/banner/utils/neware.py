...
def calculate_neware_columns(df: pd.DataFrame, temperature=True):
    ''' 
    ...
    df[NW_TIMESTAMP] = calculate_neware_timestamp(df)
...
def calculate_neware_timestamp(df: pd.DataFrame):
    ''' 
...
def calculate_dq_dv(df: pd.DataFrame, raw=False):
    ''' 
