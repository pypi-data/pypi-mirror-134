...
class PatternMiner:
    ...
    def update_statistics(self, df_data: pd.DataFrame = None):
        ...
        for level in range(len(self.df_data.index.names)):
            self.df_data[
                self.df_data.index.names[level]
            ] = self.df_data.index.get_level_values(level=level)
    ...
    def correct_data(self, *args, **kwargs):
        ...
        df_results["correct_value"] = df_results["pattern_def"].apply(
            lambda x: get_value(expression=x, num=2)
        )
        df_results = df_results.loc[df_results["result_type"] == False]

        # Get the Q column names
        colq = get_value(expression=df_results["pattern_def"].iloc[0], num=2, col=1)
    ...
    def convert_labels(self, df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
        """
        Converts the column names of a pattern dataframe
        """
        return to_dataframe(patterns=convert_columns(self.df_patterns, df1, df2))

    def convert_columns_to_time(
        self, name_col: str, year, extra: list = [], tqd: bool = False
    ) -> pd.DataFrame:
        """
    ...
    def convert_to_time(
        self, to_index, year, to_del: list = [], set_year: bool = True
    ) -> pd.DataFrame:
        """
        ...
        for metapattern in self.metapatterns:
                ...
                for i in clusters:
                    ...
                    if "min_confidence" in parameters:
                        if (
                            ...
                            patterns = get_highest_conf(df=patterns)
                            logger.info(
            ...
            # if no cluster
                ...
                df_patterns = df_patterns.append(
                    patterns, ignore_index=True, sort=False
                )
...
def get_value(expression: str, num: int = 1, col: int = 3):
    ...
    if conditional_expression is None:
        ...
        if col == 1:  # get the col name
            for match in re.finditer(r"{(.*?)}", conditional_expression.group(num)):
                item3 = match.group(1)[1:-1]
                values.append(item3)
        else:
            # Find repeating expression of conditions
            for match in re.finditer(
                r"(.*?)[&|\||\^](\s*\({.*)", conditional_expression.group(num)
            ):
            ...
            item2 = re.search(
                r"(.*)([>|<|!=|<=|>=|=])(.*)", conditional_expression.group(num)
            )
            item3 = re.search(r'"(.*)"', item2.group(col))
...
def get_highest_conf(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    """
    df["P_val"] = df["pattern_def"].apply(lambda x: get_value(expression=x, num=1))
    df = df.sort_values(SUPPORT, ascending=False)  # Sort values
    _, idx = np.unique(
        df["P_val"].astype(str).values, return_index=True
    )  # Drop duplicate rows

    df = df.iloc[idx].sort_index()  # Only get dataframe rows from these indices
    df = df.drop(["P_val"], 1)

    return df


def derive_patterns_from_template_expression(
    metapattern: dict = None, dataframe: pd.DataFrame = None
):
    """
    ...
    express = parameters.get("expres", False)

    if re.search(r"IF(.*)THEN(.*)", expression) or express:  # conditional statement
        patterns = derive_conditional_patterns_from_expression(
            expression=expression, metapattern=metapattern, dataframe=dataframe
        )
    else:
...
def derive_quantitative_pattern_from_expression(
    expression: str, metapattern: dict, dataframe: pd.DataFrame
):
    """
...
def get_possible_columns(
    amount: int, expression: str, dataframe: pd.DataFrame, quant: bool = False
):
    """
...
def get_possible_values(
    amount: int, possible_expressions: list, dataframe: pd.DataFrame
):
...
def add_qoutation(possible_expressions: list) -> list:
    """
...
def derive_conditional_patterns_from_expression(
    expression: str="", metapattern: list=None, dataframe: pd.DataFrame=None
):
    """
...
def evaluate_metrics(
    ...
    encodings: dict,
    eval_dict,
    metapattern: dict = None,
    df_patterns: pd.DataFrame = None,
):
    ...
    supp_x = expression_metrics["X"] / (
        expression_metrics["X"] + expression_metrics["~X"]
    )
    supp_nx = expression_metrics["~X"] / (
        expression_metrics["X"] + expression_metrics["~X"]
    )

    if "Y" in expression_metrics.keys():
        supp_y = expression_metrics["Y"] / (
            expression_metrics["Y"] + expression_metrics["~Y"]
        )
        supp_ny = expression_metrics["~Y"] / (
            expression_metrics["Y"] + expression_metrics["~Y"]
        )
    else:
        supp_y = 0
...
def derive_patterns_from_code(metapattern: dict = None, dataframe: pd.DataFrame = None):
...
def derive_conditional_pattern(
    dataframe: pd.DataFrame = None, metapattern: dict = None
):
    """
    ...
    if (dataframe is not None) and (df_patterns is not None):
        ...
        for idx in tqdm(
            ...
            try:
                ...
                colq = get_value(
                    expression=df_patterns.loc[idx, "pattern_def"], num=2, col=1
                )
                colp = get_value(
                    expression=df_patterns.loc[idx, "pattern_def"], num=1, col=1
                )
