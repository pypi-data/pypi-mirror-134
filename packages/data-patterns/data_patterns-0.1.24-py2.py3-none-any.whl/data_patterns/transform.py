...
class PatternDataFrame(pd.DataFrame):
    ...
    def to_excel2(self, filename, *args, **kwargs):
        ...
        if not self.empty:
            ...
            df.to_excel(
                writer, sheet_name=sheet_name, merge_cells=False, *args, **kwargs
            )
        else:
        ...
        if not self.empty:
            for pattern_id in self[PATTERN_ID].unique().sort():
                ...
                if not co.empty:
                    co.to_excel(
                        writer,
                        sheet_name=pattern_id + SHEET_NAME_POST_CO,
                        merge_cells=False,
                    )
                if not ex.empty:
                    ex.to_excel(
                        writer,
                        sheet_name=pattern_id + SHEET_NAME_POST_EX,
                        merge_cells=False,
                    )
        for name in writer.sheets:
            worksheet = writer.sheets[name]
