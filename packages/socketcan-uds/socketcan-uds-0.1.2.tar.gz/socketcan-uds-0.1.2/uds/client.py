...
from uds.common import parse_response, ServiceId, UdsTimeoutError, DiagnosticSession, ResetType, \
    ...
    concat_control_dtc_setting_request, DtcSettingType, concat_write_data_by_id_request, \
    concat_request_download_request, concat_request_upload_request, CompressionMethod, EncryptionMethod, \
    concat_transfer_data_request, concat_request_transfer_exit_request, concat_clear_diagnostic_information_request, \
...
class UdsClient:
    ...
    def download_block(self,
        ...
        """
        A convenience function to download a whole block of data.
        It executes all necessary services.
        :param addr: The address of the upload. Hardcoded to 32bit for now.
        :param size: The size of the upload. Hardcoded to 32bit for now.
        :param data: The data to be transferred.
        :param compression_method: The method of compression.
        ...
        self.request_download(addr=addr,
                              size=size,
                              compression_method=compression_method,
                              encryption_method=encryption_method)
        ...
        for chunk_idx, chunk_bytes in enumerate(
            ...
            LOGGER.debug("Download Block - Transfer Data Block {0} Size {1}".format(chunk_idx + 1, len(chunk_bytes)))
            self.transfer_data(block_sequence_counter=chunk_idx + 1,
                               data=chunk_bytes)
        LOGGER.debug("Download Block - Request Transfer Exit")
        self.request_transfer_exit(transfer_request_parameters=transfer_request_parameters)
