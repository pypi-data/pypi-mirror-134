...
LUNAR_DATA_BATCH_DATA_API_URL = "/v1/data/batch"
...
class DataClient(LunarClient):
    ...
    def __init__(self, config: Config):
        ...
        self.batch_api_url = (
            LUNAR_DATA_BATCH_DATA_API_URL if config.ENV == "LOCAL" else f"/api{LUNAR_DATA_BATCH_DATA_API_URL}"
        )
    ...
    async def get_batch_data_async(
        ...
        body = await self._request(method="POST", url=self.batch_api_url, data=data, headers=headers)
