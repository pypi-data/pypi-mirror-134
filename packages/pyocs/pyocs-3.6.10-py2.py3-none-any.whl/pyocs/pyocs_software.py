...
class PyocsSoftware:
    ...
    # 无依赖OCS
    def get_download_link_by_ocs(self, ocs_number, use_cache=True):
        ...
        sw_list = faas_api.get_software_list(ocs_number=ocs_number, exclude_disable=False, use_cache=use_cache)
        if not sw_list:
    ...
    # 无依赖OCS
    def get_software_download_link(self, ocs_number, sw_name, use_cache=True):
        """根据软件全名获取软件外网下载链接和链接更新日期
        Args:
            ocs_number:订单号
            sw_name: 软件的全名
        ...
        Returns:
            返回软件下载链接和下载链接有效期
        """
        sdl_list = self.get_download_link_by_ocs(ocs_number=ocs_number, use_cache=use_cache)
        for sdl in sdl_list:
            if sw_name == sdl.name:
                return sdl
        return None
