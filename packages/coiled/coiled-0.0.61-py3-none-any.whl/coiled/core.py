...
class Cloud(Generic[IsAsynchronous]):
    ...
    async def _create_cluster_declarative(
        ...
        *,
        ...
        # TODO: for now this has the same signature as the old _create_cluster, and is called by the same
        # (not private) create_cluster method. That may need to change.

        # TODO: raise exception for args we don't like

        # TODO: support passing the region
        # (Question: looks like in declarative we support swapping regions at cluster creation time, not just setup...
        # ...is that really true?)

        # TODO: pass more options through, including:
        # software: Optional[str] (maybe?? why is this a separate arg from "config"?)

        # environ: Optional[Dict[str, str]]
        # options: Optional[Union[AWSRuntimeOptionsAPISchema, GCPRuntimeOptionsAPISchema]]
    ...
    def create_cluster(
        ...
        *,
        ...
        if use_declarative():
            _create_cluster = self._create_cluster_declarative
        else:
            _create_cluster = self._create_cluster
