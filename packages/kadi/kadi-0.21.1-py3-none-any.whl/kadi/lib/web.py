...
def download_stream(data, download_name, mimetype=None, as_attachment=True):
    """Send a binary stream to a client as a file.

    :param data: The binary stream to send, usually an in-memory bytes buffer.
    ...
    :param as_attachment: (optional) Flag indicating whether the file should be
        displayed in the browser instead of being downloaded. Should only be used for
        trusted data or data that is safe for displaying.
...
def download_string(data, download_name, mimetype=None, as_attachment=True):
    """Send a string to a client as an UTF-8 encoded text file.

    Uses :func:`download_stream`.
