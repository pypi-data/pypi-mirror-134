...
def _get_lib_path(name):
    if os.name == u"posix" and sys.platform == u"darwin":
        prefix, ext = u"lib", u".dylib"
    elif sys.platform == u"win32":
        prefix, ext = u"lib", u".dll"
    else:
        prefix, ext = u"lib", u".so"
    fn = None
    meipass = getattr(sys, u"_MEIPASS", None)
    if meipass is not None:
        fn = os.path.join(meipass, prefix + name + ext)
    if fn is None and pkg_resources is not None:
        try:
            fn = pkg_resources.resource_filename(u"knight_v8", prefix + name + ext)
        except Exception:
            pass
    if fn is None:
...
class Knight:
    ...
    def eval(self, script):
        ...
        if ret == u"undefined": return None
        try:
