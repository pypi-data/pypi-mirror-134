...
class ExcelWriter:
    ...
    def generate(self, data):
        ...
        steps = [
            ...
            ['System Details 2', self._systemDetails2(),],
            ...
            ['LPAR CPU/RAM Summary 2', self._lparCpuRam2(),],
    ...
    def _header(self):
        ...
        worksheet.write('A1', 'Here we will say something about the scanner and the its parameters')
    ...
    def _systemDetails(self):
        ...
        for data in self.data:
            ms_names += sorted(data['managed_system'].keys())
            for ms, value in data['managed_system'].items():
                num_ms += 1
                vars = vars.union([k for k, v in value.items() if type(v) in [int, float, str]])
        vars = sorted(vars)
        vars.remove('_HMC') # we do not it!
        vars.remove('name') # we do not it!

        # Define columns structure for Table creation setting only column names
        columns = [{'header': 'Variable'}] + [{'header': ms}
                                              for ms in ms_names]

        # Add table
        worksheet.add_table(0, 0, 1+len(vars), num_ms, {'columns': columns, 'autofilter': False})
        ...
        worksheet.write(1, 0, 'HMC', self.bold)
        for n, v in enumerate(vars):
            worksheet.write(2+n, 0, v, self.bold)

        # Write Managed System data, by columns
        col = 1
        for data in self.data:
            for ms in sorted(data['managed_system'].keys()):
                worksheet.write(1, col, data['managed_system'][ms]['_HMC'], self.normal)
                for n, v in enumerate(vars):
                    worksheet.write(2+n, col, 
                                    data['managed_system'][ms].get(v, 'N/A'), self.normal)
                col += 1

        # Static column width
        worksheet.set_column(0, 0, 48)      
        worksheet.set_column(1, len(ms_names), 18)
        worksheet.set_column(len(ms_names)+1, len(ms_names)+10, 2)


    def _systemDetails2(self):
        worksheet = self.workbook.add_worksheet('System Details2')
        worksheet.freeze_panes(1, 1)
    ...
    def _lparCpuRam(self):
        ...
        # Add table
        worksheet.add_table(0, 0, sum([ len(ms['lpar'].keys()) for ms in managedSystem.values() ]),
        ...
        for msName in sorted(managedSystem.keys()):
            ms = managedSystem[msName]
            for lparName in sorted(ms['lpar'].keys()):
                lpar = ms['lpar'][lparName]
                data = [
                    ms['_HMC'],
                    msName,
                    lparName,
                ...
                #if  pool != 'DefaultPool':
                #    worksheet.write_comment(row, 14, f'Pool size = {}')
    ...
    def _lparCpuRam2(self):
        worksheet = self.workbook.add_worksheet('LPAR CPU RAM 2')
    ...
    def _LPAR_vadapter_details(self):
        worksheet = self.workbook.add_worksheet('LPAR CPU vadapter details')
