...
__version__ = '0.3'             # Used for PyPi versioning
...
def main():
    ...
    grp2.add_argument('--parallel', action='store', type=int, default=DEF_TASKS, 
                        help=f'Parallel scans when using YAML on multiple HMCs. Dafault is {DEF_TASKS}')
    ...
    argstring = [str(k)+'='+str(v) if k not in ['pwd', 'ssh_key', 'dir']
                 else str(k)+':<HIDDEN>' for k, v in vars(args).items()]
    logger.debug("Args: " +  " ".join(argstring))
    argstring = []
    for elem in configuration:
        s = '{' + ' '.join([str(k)+'='+str(v) if k not in ['password', 'ssh-key', 'dir']
...
def get_data(config):
    ...
    dc = DataCollector(config['ip'], config['user'], Path(config['dir']) / config['ip'],
                       ssh_key=config.get('ssh-key', None), password=config.get('password', None), connect=config['connect'], tqdmMgr=config['tqdmMgr'])
