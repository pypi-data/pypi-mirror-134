...
import baseten
from baseten.baseten_deployed_model import (BasetenArtifact,
                                            BasetenDeployedModel)
from baseten.common import api
from baseten.common.core import (ARTIFACT_FILENAME, MODEL_FILENAME,
...
def deploy_model(serialized_model: IO[Any],
    ...
    model_key = _upload_file(serialized_model, file_ext, MODEL_FILENAME)
...
def create_artifact(
    ...
    logger.info('Making contact with BaseTen 👋 👽')
    s3_key = _upload_file(artifact_tgz, 'tgz', ARTIFACT_FILENAME)
    ...
    logger.info(f'Successfully stored {Fore.BLUE}{name}{Style.RESET_ALL} artifact with id {artifact_resp["id"]}.')
...
def deploy_baseten_scaffold(
    ...
    logger.info('Making contact with BaseTen 👋 👽')
    s3_key = _upload_file(temp_file, 'tgz', MODEL_FILENAME)
    if not model_info:
...
def _upload_file(serialize_file: IO, file_ext: str, file_name: str) -> str:
    ...
    form_fields['file'] = (model_file_name, serialize_file)
