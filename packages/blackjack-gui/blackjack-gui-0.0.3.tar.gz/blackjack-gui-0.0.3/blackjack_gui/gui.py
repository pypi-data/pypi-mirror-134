...
def get_image(card: Card = None, width: int = 100, height: int = 130):
    if card is None:
        filename = 'blackjack_gui/images/back.png'
    else:
        prefix = {
            'A': 'ace',
            'J': 'jack',
        ...
        if card.label in prefix.keys():
            ...
            fix = str(card.value)
        filename = f'blackjack_gui/images/{fix}_of_{card.suit}.png'
...
def get_chip_image(color: str = 'red'):
    size = 50
    filename = f'blackjack_gui/images/{color}-chip.png'
    image = Image.open(filename).resize((size, size-15), Image.ANTIALIAS)
    return ImageTk.PhotoImage(image)


def get_finger_image():
    filename = f'blackjack_gui/images/finger2.png'
