...
class XlfParser:
    ...
    def from_xml(xls_content: str) -> 'XlfParser':
        ...
        for trans_units_element in trans_units_elements:
            ...
            source_text = source.text + b''.join(etree.tostring(e) for e in source).decode('UTF-8')

            target = trans_units_element.find('target', nsmap)
            if target is not None:
                target_text = target.text + b''.join(etree.tostring(e) for e in target).decode('UTF-8')
            else:
                target_text = ''
