...
class server:
    def __init__(self ,version):
        ...
        File('https://launchermeta.mojang.com/mc/game/version_manifest.json' ,self.basepat h +'\\versions.json')
        self.aa = '<html><body>Sorry, invalid request</body></html>'

        with open(self.basepat h +'\\versions.json') as f:
            self.content = f.read()

        with open(self.basepat h +'\\versions.json' ,'w') as f:
            f.write(self.content.replace(self.aa ,''))
    ...
    def download_server_core_jar(self ,dir):
        ...
        for i in range(10):
            ...
            with open(dir + '\\server.json' ,'w') as f:
                f.write(self.content.replace(self.aa ,''))
        with open(dir + '\\server.json', 'w') as f:
            f.write(self.content.replace('true', 'True'))
        with open(dir + '\\server.json', 'w') as f:
            f.write(self.content.replace(self.aa, ''))
        ...
        while True:
            ...
            try:
                ...
                File(self.ser['downloads']['server']['url'] ,di r +'\\server.jar')
                break
            except BaseException as e:
                print(e)
                continue
    ...
    def setting_default(self ,di):
        with open(d i +'\\eula.txt' ,'w') as f:
            f.write('eula=true')
        with open(d i +'\\server.properties' ,'a') as f:
            f.write('')
    ...
    def setting_add(self ,dir ,a ,b):
        with open(di r +'\\server.properties') as f:
            self.d = f.read()
        ...
        with open(di r +'\\server.properties' ,'w') as f:
            f.write(final)
