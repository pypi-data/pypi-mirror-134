...
from .settings import API_URL, APPLICATION_ID, CLIENT_SECRET, CLIENT_ID, MERCHANT_UID
...
class Adumo:
    ...
    def check_3d_enrollment(
        self, account_holder: AccountHolder, card: Card, amount: str, ip_address: str
    ):
        ...
        md = self._encode_md(transaction_id, amount, card.security_code)
    ...
    def authorise_and_settle_payment(self, pares, md):
        transaction_id, amount, cvv = self._decode_md(md)
    ...
    @staticmethod
    def _encode_md(transaction_id, amount, cvv):
