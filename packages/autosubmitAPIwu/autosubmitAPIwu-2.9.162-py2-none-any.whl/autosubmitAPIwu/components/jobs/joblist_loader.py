...
class JobListLoader(object):
  ...
  def assign_existing_graph_coordinates(self):
    valid_coordinates = self._get_graph_drawing_data()
    if valid_coordinates:
      for job in self._jobs:
        job.x_coordinate, job.y_coordinate = valid_coordinates[job.name]
      return True
    return False
  ...
  def _get_graph_drawing_data(self):
    ...
    return ExperimentGraphDrawing(self.expid).get_validated_data(self._jobs)
