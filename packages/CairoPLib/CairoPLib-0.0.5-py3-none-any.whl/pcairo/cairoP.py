...
class Drawing:
    def __init__(self, name: str, width: int, height: int, typefile: str):
        ...
        self.context.set_source_rgba(0.9, 0.9, 0.9, 1)
    ...
    def background(self, red: int, green: int, blue: int, alpha: int = 255):
        self.fill(red, green, blue, alpha)
        self.strokeWeight(0)
    ...
    def color_font(self, red: int, green: int, blue: int, alpha: int = 255):
        self.context.set_source_rgba(red/255, green/255, blue/255, alpha/255)
    ...
    def putImage(self, image, x: float, y: float, width: float = -1, height: float = -1):
        self.ims = cairo.ImageSurface.create_from_png(image)

        if width == -1 and height == -1:
            pass
        elif width == -1:
            scaleX = self.ims.get_height()/height
            scaleY = scaleX
        elif height == -1:
            scaleY = self.ims.get_width()/width
            scaleX = scaleY
        else:
            scaleX = self.ims.get_height()/height
            scaleY = self.ims.get_width()/width

        self.ims.set_device_scale(scaleX, scaleY)
        #print(self.ims.get_width(), self.ims.get_height())
        self.context.set_source_surface(self.ims, x, y)
        self.context.paint()
