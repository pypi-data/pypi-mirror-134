...
class Field:
    ...
    originalDataType: Optional[str] = None
...
class Query:
    ...
    def _where_spell_out(self, where):
        ...
        for where_condition in where:
            if isinstance(where_condition, Condition):
                ...
                elif where_condition.field.originalDataType is not None:
                    formatted_cond = where_condition.field.column + " (" + where_condition.field.originalDataType + ") " + operator + " " + formatted_value
                else:
                    formatted_cond = where_condition.field.column + " " + operator + " " + formatted_value
    ...
    def spell_out(self):
        ...
        fields = []
        for field in self.fields:
            if field.internalDataType is not None:
            ...
            elif field.originalDataType is not None:
                if field.alias is None:
                    if field.aggregation is not None and isinstance(field.aggregation, str):
                        string = '"' + field.aggregation + " " + field.column + " (" + field.originalDataType + ")" + '"'
                    elif field.aggregation is not None and isinstance(field.aggregation, SQLFunction):
                        string = '"' + field.aggregation.name + " " + field.column + " (" + field.originalDataType + ")" + '"'
                    else:
                        string = '"' + field.column + " (" + field.originalDataType + ")" + '"'
                else:
                    if field.aggregation is not None and isinstance(field.aggregation, str):
                        string = '"' + field.aggregation + " " + field.column + " (" + field.originalDataType + ") AS " + field.alias + '"'
                    elif field.aggregation is not None and isinstance(field.aggregation, SQLFunction):
                        string = '"' + field.aggregation.name + " " + field.column + " (" + field.originalDataType + ") AS " + field.alias + '"'
                    else:
                        string = '"' + field.column + " (" + field.originalDataType + ") AS " + field.alias + '"'
            else:
                if field.alias is None:
                    if field.aggregation is not None and isinstance(field.aggregation, str):
