...
def grab_version(new_version=None):
	output = None
	with finput(__file__,inplace=True) as foil:
		for line in foil:
			if line.startswith('VERSION = '):
				output = line.strip().replace('VERSION = ','').replace('"','')
				if new_version is not None:
					print(f"VERSION = \"{'.'.join(new_version)}\"")
				else:
					print(line,end='')
			else:
				print(line,end='')
...
if selfArg('install'):
	...
	typle = tuple(grab_version().split('.'))
	grab_version((typle[0],typle[1], str(int(typle[-1])+1)  ))
