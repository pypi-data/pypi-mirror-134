...
class Local:
    ...
    def edit_profile(self, nickname: str = None, content: str = None, icon: BinaryIO = None, backgroundColor: str = None, backgroundImage: str = None, defaultBubbleId: str = None, media: BinaryIO = None):
        data = {
            "address": None,
            "latitude": 0,
            "longitude": 0,
        ...
        if media: data["mediaList"] = [[100, self.upload_media(media, "image"), None, None, None, {"fileName": str(media.name)}]]
    ...
    def ban(self, userId: str, reason: str, banType: int = None):
        data = json.dumps({"reasonType": banType, "note": {"content": reason}, "timestamp": int(timestamp() * 1000)})
    ...
    def unban(self, userId: str, reason: str = "هذا العضو كان شاطر اخر كم يوم"):
        data = json.dumps({"note": {"content": reason}, "timestamp": int(timestamp() * 1000)})
    ...
    def activate_status(self, status: int = 1):
        data = json.dumps({"onlineStatus": status, "duration": 99999999, "timestamp": int(timestamp() * 1000)})
    ...
    def submit_wiki(self, wikiId: str, message: str = None):
        data = {"message": message, "itemId": wikiId, "timestamp": int(timestamp() * 1000)}
        data = json.dumps(data)
    ...
    def edit_blog(self, title: str, content: str, blogId: str = None, wikiId: str = None, fansOnly: bool = False, backgroundColor: str = None):
        data = {"title": title, "content": content, "timestamp": int(timestamp() * 1000)}
