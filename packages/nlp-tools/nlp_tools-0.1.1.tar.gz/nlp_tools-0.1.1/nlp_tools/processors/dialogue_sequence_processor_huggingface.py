    ...
    def transform(self,samples: TextSamplesVar,seq_length: int = None) :
        ...
        for seq in samples:
            ...
            for index, sub_seqn in enumerate(seq):
                ...
                if index == 0:
                    new_seq += self.text_tokenizer.tokenizer.cls_token + sub_seqn + self.text_tokenizer.tokenizer.sep_token
                    start_position += len(sub_seqn) + 2
                else:
                    new_seq += sub_seqn + self.text_tokenizer.tokenizer.sep_token
                    start_position += len(sub_seqn) + 1
            ner_masking.append(sub_ner_masking)
            clean_seqs.append(new_seq)

        tokenizers_result = self.text_tokenizer.encode(clean_seqs,add_special_tokens=False,maxlen=seq_length)
