...
class NmfModel:
    ...
    def get_aic(self) -> float:
        if not self.is_ok():
            self.test_model()
        freeparams = self._ncomp
        nelem = self._nobj
        m_proxy = self.get_m_proxy()
        m_delta = self._mat - m_proxy
        err = self.get_precision(relative=False)
        res = 2 * (freeparams + math.log(err / self._nobj))
        return res
...
def generate_scree_plot(mat: Union[np.ndarray, pd.DataFrame], ncmin: int, ncmax: int, do_entropies=True):
    columns = []
    ncomp = ncmin
...
def add_miniscree_plots_from_scree_plot(df_examples: pd.DataFrame,
    ...
    if dropna:
        df_examples.dropna(axis=0, how='any', inplace=True)
    return df_examples
