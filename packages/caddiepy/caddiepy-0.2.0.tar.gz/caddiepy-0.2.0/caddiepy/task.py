...
class Task:
    ...
    def set_parameter(self, parameter, value):
        if parameter == 'gene_interaction_datasets':
            setattr(self.config.parameters, parameter, [settings.GeneInteractionDataset(x) for x in value])
        elif parameter == 'drug_interaction_datasets':
            setattr(self.config.parameters, parameter, [settings.DrugInteractionDataset(x) for x in value])
