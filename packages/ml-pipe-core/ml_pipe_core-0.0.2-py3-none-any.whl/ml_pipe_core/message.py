...
class Message:
    ...
    def _deserialize(class_type: 'Message',
        ...
        merged_message_value = ''
        for message in messages:
            merged_message_value += message.decode('utf-8') if type(message) == bytes else message.value().decode(
                'utf-8')

        message_dict = json.loads(merged_message_value)
