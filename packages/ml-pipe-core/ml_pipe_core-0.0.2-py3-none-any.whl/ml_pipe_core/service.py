...
class Service(ABC):
    ...
    def post_init(self):
        ...
        self.alive_thread = TimerThread(0.05, self._alive)
        self.alive_thread.start()

    def _alive(self):
        self.producer.sync_produce(topic=IS_ALIVE, value=AliveMessage(info=self.info(), type=self.type, status=self.status).serialize(), headers=Headers(self.name))
