...
class KafkaPipeClient():
    ...
    def get_observation(self, package_id, timeout=30, poll_time=0.05):
        res = requests.get(f'http://{OBSERVER_URL}/find/{package_id}')
        if res.status_code == 200:
            return res.json()
        time_counter = 0.0
        while(res.status_code == 202):
            ...
            if time_counter >= timeout:
                _logger.error(f"timeout reached for package_id: {package_id}.")
                return None
