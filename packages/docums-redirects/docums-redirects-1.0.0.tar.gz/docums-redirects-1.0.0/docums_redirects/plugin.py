...
log = logging.getLogger('docums.plugin.redirects')
...
def get_html_path(path, use_directory_urls):
    ...
    # Directory URLs require some different logic. This mirrors docums' internal logic.
...
class RedirectPlugin(BasePlugin):
    ...
    def on_files(self, files, config, **kwargs):
        ...
        if config.get('redirects'):
            log.warn("The root-level 'redirects:' setting is not valid and has been changed in version 1.0! "
                     "The plugin-level 'redirect-map' must be used instead.")
        ...
        self.doc_pages = {}
        for page in files.documentation_pages():  # object type: docums.structure.files.File
    ...
    def on_post_build(self, config, **kwargs):
        ...
        for page_old, page_new in self.redirects.items():
            ...
            # Note: we use 'warn' here specifically; docums treats warnings specially when in strict mode
            else:
