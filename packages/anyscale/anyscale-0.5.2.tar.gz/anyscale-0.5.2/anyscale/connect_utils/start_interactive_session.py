...
class StartInteractiveSessionBlock:
    ...
    def __init__(
        ...
        self._log_interactive_session_info(
            cluster.id, job_config.metadata.get("job_name")
        )

    def _log_interactive_session_info(
        self, cluster_id: str, interactive_session_name: str
    ) -> None:
        retry = 0
        while retry < 10:
        ...
        if not len(jobs_resp):
            raise RuntimeError(
                f"Unable to get interactive session with job name {interactive_session_name}."
            )
        job = jobs_resp[0]
