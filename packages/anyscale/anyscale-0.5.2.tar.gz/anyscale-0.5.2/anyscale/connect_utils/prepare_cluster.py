...
class PrepareClusterBlock:
        ...
        if needs_start:
            ...
            if self.cluster_env_name or existing_terminated_cluster is None:
                build_id = self._get_cluster_build(
                    project_id, self.cluster_env_name, self.cluster_env_revision
                ).id
            else:
    ...
    def _create_or_update_session_data(
        ...
        start_required = True
        cluster_list = self.api_client.list_sessions_api_v2_sessions_get(
    ...
    def _get_cluster_build(
        ...
        if cluster_env_name:
            build = self._get_cluster_env_build(
                project_id, cluster_env_name, cluster_env_revision
            )
        else:
            try:
    ...
    def _get_cluster_env_build(
        ...
        cluster_environments = list_entities(
            ...
            filters={"name_contains": cluster_env_name, "project_id": project_id},
        )
