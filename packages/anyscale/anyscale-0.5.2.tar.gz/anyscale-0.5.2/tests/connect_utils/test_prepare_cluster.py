...
def test_create_or_update_session_data(
    ...
    mock_prepare_cluster_block.api_client.list_sessions_api_v2_sessions_get = Mock(
    ...
    mock_prepare_cluster_block.api_client.list_sessions_api_v2_sessions_get.assert_called_once_with(
        project_id="test_project_id", active_only=False, name="test_cluster_name"
    )
...
def test_get_cluster_build(mock_prepare_cluster_block, cluster_env_name: Optional[str]):
    ...
    assert (
        mock_prepare_cluster_block._get_cluster_build(
            "test_project_id", cluster_env_name, None
        )
        == mock_build
    )
    ...
    if clust_env_revision == 1:
        assert (
            mock_prepare_cluster_block._get_cluster_env_build(
                "test_project_id", "test_cluster_env_name", clust_env_revision
            )
            == mock_build1
        )
    else:
...
def test_get_cluster_compute_id(
    mock_prepare_cluster_block, cluster_compute_name: Optional[str]
):
    ...
    observed_result = mock_prepare_cluster_block._get_cluster_compute_id(
        "test_project_id", cluster_compute_name, None, "test_cloud_name"
    )
    ...
    if cluster_compute_name:
        ...
        mock_prepare_cluster_block._get_cloud_id.assert_called_once_with(
            "test_project_id", "test_cloud_name"
        )
...
@pytest.mark.parametrize("cluster_needs_start", [True])
@pytest.mark.parametrize("cluster_name", ["test_cluster_name", None])
...
def test_init(
    ...
    mock_build_cluster_env_if_needed = Mock(return_value="mock_cluster_env_name")
    ...
    with patch.multiple(
        ...
        prepare_cluster_block = PrepareClusterBlock(
            ...
            cluster_compute_dict="test_cluster_compute_dict",
            cloud_name="test_cloud_name",
            build_pr=None,
            force_rebuild=None,
            build_commit=None,
            cluster_env_name="test_cluster_env_name",
            cluster_env_dict="test_cluster_env_dict",
            cluster_env_revision="test_cluster_env_revision",
            ray=Mock(),
        )
    ...
    if cluster_needs_start:
        mock_build_cluster_env_if_needed.assert_called_once_with(
            ...
            None,
            None,
            "test_cluster_env_dict",
            "test_cluster_env_name",
            None,
        )
        mock_get_cluster_build.assert_called_once_with(
            "test_project_id", "mock_cluster_env_name", "test_cluster_env_revision"
        )
        ...
        if cluster_compute_name:
            mock_get_cluster_compute_id.assert_called_once_with(
                ...
                "test_cluster_compute_dict",
                "test_cloud_name",
            )
        mock_wait_for_app_build.assert_called_once_with(
            "test_project_id", "mock_build_id"
        )
