...
class MyZipFile(zipfile.ZipFile):
    ...
    def _extract_member(self, member, path=None, pwd=None):
        ...
        try:
           ret_val = zipfile.ZipFile._extract_member(self, member, path, pwd)
        ...
        if platform.system() == "Linux":
                ...
                raise Exception("Binary packages are not yet supported on "+platform.system())
...
def installPlugin(name, tgtpath="./"):
        ...
        print(" - installing '"+str(name)+"' (destination path "+str(tgtpath)+")") 
...
def printUsage():
        print("USAGE: sofa-spm <cmd> <cmd-args> <--dblocation path>")
        ...
        print("  --dblocation path        specify a specific path where recipieces will be downloaded.")
...
if sys.argv[1] == "search":
        ...
        if len(sys.argv) <3:
                ...
                print("USAGE: spm token <list|add> <--dblocation path>")
                sys.exit(0)

        if sys.argv[2] == "list":
                if len(sys.argv) <3:
                        print("Invalid (token list) command line.")
                        print("USAGE: spm token list <--dblocation path>")
                        sys.exit(0)
                tokens = loadTokens()
                ...
                if len(sys.argv) != 5:
                        ...
                        print("USAGE: spm token add <name> <token> <--dblocation path>")
                        sys.exit(0)
