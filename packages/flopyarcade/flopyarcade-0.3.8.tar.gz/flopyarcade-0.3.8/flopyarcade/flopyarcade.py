...
class FloPyEnv(gym.Env):
    ...
    # def __init__(self, env_config={}, initWithSolution=True):
        ...
        # ENVTYPE='1s-d'
        # ENVTYPE='6s-c'
        ENVTYPE=env_config['ENVTYPE'] if 'ENVTYPE' in env_config.keys() else '1s-d'
        PATHMF2005=env_config['PATHMF2005'] if 'PATHMF2005' in env_config.keys() else None
        PATHMP6=env_config['PATHMP6'] if 'PATHMP6' in env_config.keys() else None
        MODELNAME=env_config['MODELNAME'] if 'MODELNAME' in env_config.keys() else 'FloPyArcade'
        ANIMATIONFOLDER=env_config['ANIMATIONFOLDER'] if 'ANIMATIONFOLDER' in env_config.keys() else 'FloPyArcade'
        _seed=env_config['_seed'] if '_seed' in env_config.keys() else None
        flagSavePlot=env_config['flagSavePlot'] if 'flagSavePlot' in env_config.keys() else False
        flagManualControl=env_config['flagManualControl'] if 'flagManualControl' in env_config.keys() else False
        manualControlTime=env_config['manualControlTime'] if 'manualControlTime' in env_config.keys() else 0.1
        flagRender=env_config['flagRender'] if 'flagRender' in env_config.keys() else False
        NAGENTSTEPS=env_config['NAGENTSTEPS'] if 'NAGENTSTEPS' in env_config.keys() else 200
        nLay=env_config['nLay'] if 'nLay' in env_config.keys() else 1
        nRow=env_config['nRow'] if 'nRow' in env_config.keys() else 100
        nCol=env_config['nCol'] if 'nCol' in env_config.keys() else 100
        OBSPREP=env_config['OBSPREP'] if 'OBSPREP' in env_config.keys() else 'perceptron' # 'perceptron'
        initWithSolution=env_config['initWithSolution'] if 'initWithSolution' in env_config.keys() else True
        PATHMF6DLL=env_config['PATHMF6DLL'] if 'PATHMF6DLL' in env_config.keys() else None
