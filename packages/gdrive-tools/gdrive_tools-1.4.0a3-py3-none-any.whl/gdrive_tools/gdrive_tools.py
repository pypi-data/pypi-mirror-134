...
class GDriveTools():
  ...
  def fillSheetFromDataframe(self, sheetId: str, dataframe: pd.DataFrame, sheetTableName='', startCell='A1', withColumnnames=True, valueInputOption='RAW'):
    """
    ...
    self.fillSheet(sheetId, parsed, sheetTableName, startCell, withColumnnames, valueInputOption)

  def fillSheet(self, sheetId: str, data: List[dict], sheetTableName='', startCell='A1', withColumnnames=True, valueInputOption='RAW'):
    """
    ...
    fullA1Range = f"'{sheetTableName}'!{startCell}" if sheetTableName else f'{startCell}'
