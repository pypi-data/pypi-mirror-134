...
@LastEditTime: 2022-01-14 16:47:27
...
class ShakeshopNotifyHandler(FrameBaseHandler):
    ...
    def post_async(self):
        ...
        reward_order_status_list = invoke_result_data.data["reward_order_status_list"] if invoke_result_data.data.__contains__("reward_order_status_list") else [2]
        try:
            ...
            if result:
                ...
                if tag == '101':  # 订单支付成功，根据推送的消息参数进行必要的业务处理，5秒内返回success
                    ...
                    if order_detail_result and order_detail_result["code"] == 10000:
                        ...
                        user_info_dict = user_info_model.get_cache_dict("open_id=%s", params=[open_id], order_by="id desc")
                        if user_info_dict:
