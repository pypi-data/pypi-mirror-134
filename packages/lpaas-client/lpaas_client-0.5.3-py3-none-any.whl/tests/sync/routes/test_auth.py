...
from lpaas_client import TheoryName
...
class TestAuthAutomaticAuth(TestCase):
    ...
    @respx.mock
    def test_create_theory_with_request_reject(self):
        theory_name = TheoryName('theory_name')
        theory_str = 'a.\np(a).\np(a, b).\n'
        theory = parse_theory(theory_str)

        request = respx.post(base_url.join('/lpaas/theories')) % 401
        auth_request = respx.post(base_url.join('/lpaas/auth')).mock(
                return_value=Response(status_code=200, json={'token': 'jwt_token'}))

        with self.assertRaises(Unauthorized):
            self.client.add_new_theory(theory_name, theory)
        self.assertTrue(auth_request.called)
        self.assertTrue(request.called)
