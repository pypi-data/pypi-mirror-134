...
class TestAuthClient(asynctest.TestCase):
    client: AuthClient
    ...
    async def test_get_token(self):
        ...
        await self.client.authenticate(AuthData('User', 'pass'))
        self.assertEqual(self.client.get_token(), token)

    @respx.mock
    async def test_get_or_create_token(self):
        token = 'jwt token value'
        respx.post(base_url.join('/lpaas/auth')).mock(
                return_value=Response(status_code=200, json={'token': token}))
        self.assertEqual(await self.client.get_or_create_token(AuthData('User', 'pass')), token)

    @respx.mock
    async def test_add_jwt_token(self):
        token = 'jwt token value'
        respx.post(base_url.join('/lpaas/auth')).mock(
                return_value=Response(status_code=200, json={'token': token}))
        await self.client.authenticate(AuthData('User', 'pass'))
    ...
    @respx.mock
    async def test_raise_exception_with_no_user_data(self):
        with self.assertRaises(ValueError):
            await self.client.get_or_create_token()
    ...
    @respx.mock
    async def test_raise_exception_with_failed_auth(self):
