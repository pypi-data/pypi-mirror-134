...
from lpaas_client.sync.routes.goals import GoalClient
...
class TestGoalClient(TestCase):
    ...
    def test_get_goal_list(self):
        ...
        goal_list_response = self.client.get_goal_list(goal_name)
    ...
    def test_get_sub_goal(self):
        ...
        response = self.client.get_sub_goal(goal_name, index)
        self.assertTrue(requests.called)
        self.assertEqual(response, parse_struct('goal_a'))

    @respx.mock
    def test_remove_goal_list(self):
        requests = respx.delete(base_url.join(f'/lpaas/goals/{goal_name}')) % 204

        self.client.remove_goal_list(goal_name)

        self.assertTrue(requests.called)

    @respx.mock
    def test_replace_goal_list(self):
        requests = respx.put(base_url.join(f'/lpaas/goals/{goal_name}')) % 204

        self.client.replace_goal_list(GoalName(goal_name), goal_list)
    ...
    @respx.mock
    def test_add_new_sub_goal(self):
        requests = respx.post(base_url.join(f'/lpaas/goals/{goal_name}')) % 200

        self.client.add_sub_goal(goal_name, parse_struct('term'))

        self.assertTrue(requests.called)

    @respx.mock
    def test_multiple_call_in_with_statement(self):
        request = respx.post(base_url.join(f'/lpaas/goals')).mock(
                return_value=Response(status_code=200, content=f'/goals/{goal_name}'))
