...
class NetworkGroup(Collection):
    ...
    def to_network(self, method: str = "mean") -> Network:
        ...
        if isinstance(self.graph, nx.MultiDiGraph):
            GraphConstructor = nx.MultiDiGraph
            directionality = "directed"
        elif isinstance(self.graph, nx.MultiGraph):
            directionality = "undirected"
            GraphConstructor = nx.MultiGraph
        else:
            raise ValueError("Unknown graph type")
        emetadata = dict()
        cmetadata = dict()
        for context in self.contexts:
            emetadata.update(context["emetadata"])
            cmetadata.update(context["cmetadata"])
        metadata = {
            **emetadata,
        ...
        graph = GraphConstructor(**metadata)
        # Step2: Converge the nodes and links
        # TODO: Nodes can be added directly as long as there is at least one link from/to it
        # TODO: Look through the graphs and add them to `graph`
        # The above is inefficient, we would ideally want to use adj_matrix
        # then look up the appropriate edges in all the graphs and add to `graph`
        return Network.load_graph(graph)
