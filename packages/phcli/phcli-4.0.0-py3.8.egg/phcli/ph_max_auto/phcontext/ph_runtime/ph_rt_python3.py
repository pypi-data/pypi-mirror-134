...
class PhRTPython3(PhRTBase):
    ...
    def put_items2dy(self, config):
        table_name = "dagconf"
        config.load_yaml()
        inputs = {}
        outputs = {}
        for input in config.spec.containers.args:
            inputs.update({input.key: input.value})
        for output in config.spec.containers.outputs:
            outputs.update({output.key: output.value})
        ...
        print(type(labels))
        data={}
        data.update({"project_Id": "max"})
        data.update({"flow_version": "developer2"})
        data.update({"job_name": self.name})
        data.update({"job_version": "20211113"})
        data.update({"timeout": int(timeout)})
        data.update({"type": runtime})
        data.update({"inputs": json.dumps(inputs)})
        data.update({"outputs": json.dumps(outputs)})
        data.update({"labels": json.dumps(labels)})
        self.phdy.put_data(table_name, data)
