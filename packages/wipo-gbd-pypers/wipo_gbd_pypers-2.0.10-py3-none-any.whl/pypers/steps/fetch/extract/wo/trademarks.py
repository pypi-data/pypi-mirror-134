    ...
    def add_xml_file(self, _, xml_file):
        ...
        for event, elem in context:
            ...
            if tag == 'TradeMark':
                ...
                img_match = self.img_map.get(re.sub(r'[A-Z]*$', '', appnum), None)
                if img_match:
                    img_name, ext = os.path.splitext(os.path.basename(img_match))
                    if not img_name == appnum:
                        path = os.path.dirname(img_match)
                        new_image = os.path.join(path, '%s%s' % (appnum, ext))
                        shutil.copyfile(img_match, new_image)
                        img_match = new_image
