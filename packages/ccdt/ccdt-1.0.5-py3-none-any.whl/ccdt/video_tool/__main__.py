...
def main(argument):
    if argument.function == 'split':
        videos_path, file_names = ccdt.Split.get_videos_path(argument.input_dir, argument.output_dir, argument.file_types)
        for i in range(len(file_names)):
            ccdt.Split.save_frame(videos_path[i], argument.interval, file_names[i])
