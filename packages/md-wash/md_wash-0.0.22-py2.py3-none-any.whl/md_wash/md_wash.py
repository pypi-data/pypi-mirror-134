...
def task(abspath,output,assetname,ifuuid,copy,assets_relative_path,ifDeletedOriginFile):
...
def main(args):
    ...
    copy                 = args.copy
    output               = args.output
    ifDeletedOriginFile  = args.delete
    ...
    if not os.path.isdir(inputsrc):
        ...
        task(filename,output,assetname,ifuuid,copy,assets_relative_path,ifDeletedOriginFile)
    else:
            ...
            for (dirpath, dirnames, filenames) in os.walk(inputsrc):
                ...
                if args.recursive:
                    for filename in filenames:
                        ...
                        f = t.submit(task,abspath,output,assetname,ifuuid,copy,assets_relative_path,ifDeletedOriginFile)
                        # will block
                        if f.result():
...
def createParse():
    ...
    parser.add_argument("-d",'--delete',action='store_true', default=False,required=False, help='是否删除原始文件')
