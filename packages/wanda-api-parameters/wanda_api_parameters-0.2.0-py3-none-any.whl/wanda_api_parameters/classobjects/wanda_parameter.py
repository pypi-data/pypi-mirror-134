...
from wanda_api_parameters.parameters_api import (WKEY_GENERAL, WKEY_MIN, WKEY_MAX, WKEY_SERIES)
...
class WandaParameter:
    ...
    def get_properties(self, model: object):
        ...
        if self.wanda_component.lower() == WKEY_GENERAL.lower():
            ...
            # for comp in comps:
            #     if comp.has_keyword(self.wanda_component):
            #         properties.append(self.get_wanda_property(comp))
            #     elif comp.get_complete_name_spec() == self.wanda_component:
            #         properties.append(self.get_wanda_property(comp))
            # # Set nodes
            # nodes = model.get_all_nodes()
            # for node in nodes:
            #     if node.has_keyword(self.wanda_component):
            #         properties.append(self.get_wanda_property(node))
            #     elif comp.get_complete_name_spec() == self.wanda_component:
            #         properties.append(self.get_wanda_property(node))
