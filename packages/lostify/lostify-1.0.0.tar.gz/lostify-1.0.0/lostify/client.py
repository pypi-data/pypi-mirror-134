...
""" A Spotify API wrapper dedicated to people with a brain """
...
class Spotify(object):
  ...
  async def send_request(self, method, endpoint, payload=None, **kwargs):
    ...
    if payload:
      for item in payload:
        json_data[item] = payload[item]
    if len(kwargs) != 0:
      for kwarg in kwargs:
        json_data[kwarg] = kwargs[kwarg]
    ...
    response = await self.session.request(method, endpoint, headers=authorization)
