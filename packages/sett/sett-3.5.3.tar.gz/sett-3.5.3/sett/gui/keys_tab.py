...
class KeysTab(QtWidgets.QWidget):
        ...
        def offline_action(icon: str, tip: str, selection: bool = True) -> QAction:
            ...
            if self.app_data.config.offline:
                ...
                if selection:
                    ...
                    action = QAction(QtGui.QIcon(icon_obj), tip, self)
            return action

        # Create a dict of actions (functions) to associate to each
        # action button of the GUI.
        # The reason to create a dict object before looping over it is so that
        # mypy gets the correct typing information (for some reason
    ...
    def get_selected_keys(self) -> Tuple[gpg.Key, ...]:
        ...
        selected in the GUI.
        """
        # NOTE: cast() is needed here as selectedIndexes() wrongly hints that
        #       it returns List[int] when in fact it returns
        #       Sequence[QtCore.QModelIndex].
        selected_indexes = (
            cast(QtCore.QModelIndex, x) for x in self.pub_keys_view.selectedIndexes()
        )
        selected_keys = (index.model().get_value(index) for index in selected_indexes)
...
class KeyDownloadDialog(QtWidgets.QDialog):
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self.search_string.textChanged.connect(
    ...
    def download_selected(self) -> None:
        ...
        #       Sequence[QtCore.QModelIndex].
        selected_indexes = (
            cast(QtCore.QModelIndex, x) for x in self.key_list_view.selectedIndexes()
        )
        keys_to_download = [index.model().data(index) for index in selected_indexes]
        if keys_to_download:
