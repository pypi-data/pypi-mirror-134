...
class FileSelectionWidget(QtWidgets.QGroupBox):
    ...
    def _create_remove_selected_action(self) -> QAction:
        def clear_selected() -> None:
            ...
            # NOTE: cast() is needed here as selectedIndexes() wrongly hints
            #       that it returns List[int] when in fact it returns
            #       Sequence[QtCore.QModelIndex].
            selected_indexes = (
                cast(QtCore.QModelIndex, x)
                for x in self.file_list_view.selectedIndexes()
            )
