...
class KeycloakEnvironment():
    ...
    def __init__(self,
        ...
        if self.keycloak_enabled:
            self.kc = KeycloakAPI(auth_keycloak_url=self.keycloak_url,
    ...
    def authenticateByServiceAccount(self, client_id=None, client_realm=None):
        ...
        if self.keycloak_enabled:
            keycloak_auth_client_secret_value = self.get_client_secret(client_id=client, realm=realm)
    ...
    def authenticateByUsernamePassword(self):
        headers = {}
        if self.keycloak_enabled:
            keycloak_auth_client_secret_value = self.get_client_secret(client_id=self.keycloak_auth_client_id, realm=self.keycloak_auth_realm)
            headers = get_token(
                base_url=self.keycloak_url,
                validate_certs=self.validate_certs,
                auth_realm=self.keycloak_auth_realm,
    ...
    def get_token_url(self):
        return self.kc.get_token_url(realm=self.keycloak_auth_realm)
