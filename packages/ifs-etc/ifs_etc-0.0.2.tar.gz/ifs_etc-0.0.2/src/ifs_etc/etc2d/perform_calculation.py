...
from . import cube
from .cube import filter_mag, read_filter
...
def flux2electron(config, wavearr, fluxarr, throughtputwave, throughtputvalue, fluxunit=''):
    ...
    for j in range(qvalue.shape[1]):
        for i in range(qvalue.shape[2]):
            qvalue[:, j, i] = np.interp(wavearr[:, j, i], throughtputwave, throughtputvalue)
...
class DetectorSignal(cube.ModelCube):
...
class DetectorNoise(object):
    ...
    def __init__(self, config, signal):
        ...
        nskycube = np.zeros(shape=signal.source_counts.shape, dtype=float)
        for j in range(nskycube.shape[1]):
            for i in range(nskycube.shape[2]):
                nskycube[:, j, i] = get_nsky(signal.ccdspec_wave)        # e/s/specsampling
                # todo: 简化这个循环
...
class calculation_snr(object):
    ...
    def __init__(self, config, straylight=False):
        ...
        if straylight == False:
            ...
            nstraylight = np.zeros(shape=self.source.shape, dtype=float)
            for j in range(nstraylight.shape[1]):
                for i in range(nstraylight.shape[2]):
                    nstraylight[:, j, i] = self.source[:, ind_ycen[0], ind_xcen[0]] * 0.02
                    # todo: 简化这个循环
...
def perform_calculation(config, calculation_mode='et2sn'):

    if calculation_mode == 'et2sn':

        report = calculation_snr(config)

    elif calculation_mode == 'sn2et':

        report = calculation_exptime(config)

    elif calculation_mode == 'sn2mag':

        report = calculation_limitmag(config)

    else:

        print('calculation_mode need to be "| et2sn | sn2et | sn2mag |"')
        pdb.set_trace()
