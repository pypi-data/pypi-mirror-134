...
class Table(ReprJsonable):
    ...
    def __post_init__(self) -> None:
        ...
        for col in self._java_api.get_table_schema(self._name):
            self._columns[col.name] = Column(
                name=col.name, data_type=col.data_type, _table=self
            )
    ...
    def _load_arrow(
        ...
        from ._arrow_utils import arrow_to_temporary_file

        tmp_file = arrow_to_temporary_file(arrow_table, prefix=self.name)
        ArrowDataSource(self._java_api).load_arrow_into_table(table=self, path=tmp_file)

    def load_numpy(
        self,
        # See https://stackoverflow.com/q/69779744/5558141.
    ...
    def _identity(self) -> Identity:
        identity: Identity = (self._name, self._scenario)
