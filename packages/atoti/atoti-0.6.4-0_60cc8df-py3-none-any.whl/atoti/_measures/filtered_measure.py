...
class WhereMeasure(MeasureDescription):
    ...
    def _do_distil(
        ...
        if underlying_else_name is not None:
            java_api.refresh()
            full_measures = java_api.get_full_measures(cube_name)
            self._validate_type_compatibility(
                true_value_type=full_measures[underlying_name].underlying_type,
                false_value_type=full_measures[underlying_else_name].underlying_type,
            )
    ...
    def _do_distil(
        ...
        for condition in self._conditions:
            if isinstance(condition, LevelCondition):
                conditions.append(
                    {
                        "level": get_java_coordinates(condition.level_coordinates),
                        "type": "other",
