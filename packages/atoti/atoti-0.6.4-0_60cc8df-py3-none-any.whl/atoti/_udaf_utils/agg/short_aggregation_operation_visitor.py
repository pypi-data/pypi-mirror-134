...
from atoti_core import is_numeric_array_type, is_numeric_type
...
class ShortAggregationOperationVisitor(AggregationOperationVisitor):
    ...
    def _get_contribute_source_code(operation_element: JavaOperationElement) -> str:
        numeric_code = (
            ...
            if is_numeric_type(operation_element.output_type.java_type)
            else None
        )
        array_code = dedent(
            """\
    ...
    def _get_merge_source_code(operation_element: JavaOperationElement) -> str:
        ...
        if is_numeric_array_type(operation_element.output_type.java_type):
            body = dedent(
                """\
