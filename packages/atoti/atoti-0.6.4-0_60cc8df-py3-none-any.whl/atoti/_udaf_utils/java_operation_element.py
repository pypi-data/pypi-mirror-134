...
from atoti_core import is_numeric_array_type, is_numeric_type, keyword_only_dataclass
...
class BasicJavaOperationElement(JavaOperationElement):
    ...
    def get_java_source_code(
        ...
        if is_numeric_type(self._output_type.java_type) and numeric_code is not None:
            return numeric_code.format(java_source_code=self.java_source_code)
        if (
            is_numeric_array_type(self._output_type.java_type)
            and array_code is not None
        ):
...
class TernaryJavaOperationElement(JavaOperationElement):
    ...
    def output_type(self) -> DataType:
        ...
        if is_numeric_type(
            self.true_statement_java_operation.output_type.java_type
        ) and is_numeric_type(
            self.false_statement_java_operation.output_type.java_type
        ):
            return self.true_statement_java_operation.output_type
