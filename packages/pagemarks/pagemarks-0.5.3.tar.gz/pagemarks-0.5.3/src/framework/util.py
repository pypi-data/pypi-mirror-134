...
def locate_repo(config: CmdLine, use_env: bool = True) -> str:
...
def prompt(question: str, default_answer=PromptAnswer.NO) -> PromptAnswer:
    result = default_answer
    try:
        while True:
            answer = input(question + ' [y/N/a/q] ')
            if len(answer.strip()) == 0:
                break
            try:
                result = PromptAnswer(answer.lower())
                break
            except ValueError:
                continue
