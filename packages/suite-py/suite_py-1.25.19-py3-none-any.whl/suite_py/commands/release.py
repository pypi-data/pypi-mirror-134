...
class Release:
    ...
    def _deploy(self, version=""):
        ...
        if not version:
            current = self._get_current_version(countries)
            if current:
                releases = self._get_releases_since(current)
            else:
    ...
    def _get_current_version(self, country):
        ...
        if not current_build:
            logger.error(f"Unable to determine current version for country {country}")
            sys.exit(1)
