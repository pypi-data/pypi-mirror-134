...
class AttentionMask(ABC):
    ...
    def _make_global_layout(self) -> None:
        ...
        if not self.is_head_specific:
            ...
            m = np.ones(
                [self.n_head, self.n_query_block, self.n_key_block],
                dtype=np.bool,
            )
            r = product(*[range(n) for n in m.shape])
...
class DenseAttentionMask(AttentionMask):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        self.global_layout = np.ones(
            [self.n_query_block, self.n_key_block],
            dtype=np.bool,
        )
...
def make_full_layout(d: AttentionMask) -> np.ndarray:
    ...
    if not d.is_head_specific:
        ...
        if len(d.global_layout.shape) == 2:
            ...
            u = np.reshape(
                d.global_layout,
                [d.n_head, d.n_query_block, d.n_key_block, 1, 1],
            )
        else:
            raise RuntimeError()
        ...
        v = np.reshape(
            v,
            [d.n_head, d.n_query_block, d.n_key_block, d.block_size, d.block_size],
        )
