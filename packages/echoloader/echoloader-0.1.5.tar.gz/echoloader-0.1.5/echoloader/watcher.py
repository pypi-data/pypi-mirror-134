        ...
        with self.auth_mutex:
            if self.check_token():
                try:
                    ...
                    logger.warning(f"Failed to renew access token due to {exc}")
...
def main():
    ...
    parser.add_argument(
        "--csv-out", type=lambda p: csv.writer(open(p, 'w', newline='')),
    ...
    if args.csv_out:
        args.csv_out.writerow(['customer', 'trial', 'patientID', 'visit', 'filename'])
