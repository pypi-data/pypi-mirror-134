...
def relaunch(func):
    def wrapper(self, *args, **kwargs):
        try:
            ...
            self.d = H5Driver(Qrunner.serial_no)
    return wrapper
...
class H5Driver(object):
    ...
    def __init__(self, serial_no=None, pkg_name=None):
        ...
        self.d = webdriver.Chrome(
            executable_path=ChromeDriverManager(version=Qrunner.webview_version).install(),
            options=options
        )
    ...
    def switch_window(self, old_windows):
        ...
        newest_window = [window for window in current_windows
                         if window not in old_windows][0]
