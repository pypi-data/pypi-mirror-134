...
class OnlineSimulator():
    ...
    def get_optimal_ranking(self, config, dataObj):
        ...
        if config.optimal_ranker == 'naive_relevance':
            ...
            n_users = self.user_features.shape[0]
            optimal_rankings, optimal_scores = np.zeros((n_users, config.list_size), dtype=int), np.zeros((n_users, config.list_size), dataObj.feature_data['train_item_topical_features'].shape[1] + dataObj.feature_data['train_item_latent_features'].shape[1])
            for i in range(n_users):
                ...
                self.optimal_scores[:, dataObj.feature_data['test_item_topical_features'].shape[1]:] = dataObj.feature_data['train_item_latent_features'][ranking]
                return optimal_rankings, optimal_scores
    ...
    def compute_rewards(self, config, dataObj, batch_users, rankings):
        ...
        if config.optimal_ranker == 'naive_relevance':
            ...
            scores = np.zeros((n_users, config.list_size))
            for i in range(n_users):
                topic_coverage = self.__convert_to_topic_coverage(dataObj.feature_data['test_item_topical_features'][rankings])
                ...
                item_full_feature[:, dataObj.feature_data['test_item_topical_features'].shape[1]:] = dataObj.feature_data['train_item_latent_features'][rankings]
