...
class AbstractRanker():
    ...
    def get_features_for_optimal_ranker(config, dataObj):
        if config.full_feature:
            ...
            full_feature = np.zeros((dataObj.feature_data['train_item_latent_features'].shape[0], dataObj.feature_data['train_item_latent_features'].shape[1] + dataObj.feature_data['train_item_topical_features'].shape[1]))
            full_feature[:, :dataObj.feature_data['train_item_topical_features'].shape[1]] = dataObj.feature_data['train_item_topical_features']
            full_feature[:, dataObj.feature_data['train_item_topical_features'].shape[1]:] = dataObj.feature_data['train_item_latent_features']
            return np.concatenate((1 - float(config.lmbda)) * dataObj.feature_data['test_user_topical_features'], float(config.lmbda) * dataObj.feature_data['test_user_latent_features']), full_feature
