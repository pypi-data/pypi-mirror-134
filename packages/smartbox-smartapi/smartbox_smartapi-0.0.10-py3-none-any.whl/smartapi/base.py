...
class BaseRelayNode(relay.Node):
    dummy = True
...
class BaseType(DjangoObjectType):
    ...
    id_disambiguated = String()

    def resolve_node_description(parent, info):
        return parent.__str__()

    def resolve_id_disambiguated(parent, info):
        return disambiguate_id(parent.id)

    @classmethod
    def get_queryset(cls, queryset, info, **kwargso):
            ...
            for filter in filters:
                ...
                if filter.or_filters:
                    for condition in filter.or_filters:
                        ...
                        elif(condition.qs and (condition.field[-3:] == '_id' or condition.field == 'id')):
                            condition.qs = disambiguate_id(condition.qs);
