...
def noiseLine(size, draw):
    for item in range(int(random.random()*5)):
        h = int(random.random()*size[1])
        op = -1
        text_color = getColor()
        randomCount = int(random.random()*size[1]*2)
        for w in range(size[0]):
            if w % randomCount == 0:
                op *= -1

            h += op
            draw.point((w, h), fill=text_color)
...
def generateImage(text):
    try:
        ...
        image = Image.new(
            ...
            size=(Base.image_weight, Base.image_height)
        )
        ...
        for item in text:
            ...
            if Base.text_color == "random":
                text_color = getColor()
            draw.text(
                (x_position, getHeight()), item,
                font=font, fill=text_color
            )
            x_position += getWeight()

        noiseLine(image.size, draw)
