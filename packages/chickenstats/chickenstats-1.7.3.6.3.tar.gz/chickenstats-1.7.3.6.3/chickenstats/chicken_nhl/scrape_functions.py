...
def scrape_pbp(game_ids, _print = True):
    ...
    for game_id_idx, game_id in enumerate(game_ids):
        ...
        home_so_winner = pbp_df[np.logical_and(pbp_df.is_home == 1, pbp_df.shootout == 1)].event.value_counts()['GOAL'] > \
                            pbp_df[np.logical_and(pbp_df.is_home == 0, pbp_df.shootout == 1)].event.value_counts()['GOAL']

        away_so_winner = pbp_df[np.logical_and(pbp_df.is_home == 0, pbp_df.shootout == 1)].event.value_counts()['GOAL'] > \
                            pbp_df[np.logical_and(pbp_df.is_home == 1, pbp_df.shootout == 1)].event.value_counts()['GOAL']
