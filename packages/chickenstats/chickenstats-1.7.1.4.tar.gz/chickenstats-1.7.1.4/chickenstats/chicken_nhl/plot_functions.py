...
NHL_COLORS = {
		...
		'ARI': {'GOAL': '#8C2633', 'SHOT': '#111111', 'MISS': '#D3D3D3'},
		'BOS': {'GOAL': '#FFB81C', 'SHOT': '#000000', 'MISS': '#D3D3D3'},
		'BUF': {'GOAL': '#FCB514', 'SHOT': '#002654', 'MISS': '#D3D3D3'},
		'CAL': {'GOAL': '#F1BE48', 'SHOT': '#C8102E', 'MISS': '#D3D3D3'},
		...
		'DET': {'GOAL': '#CE1126', 'SHOT': '#FFFFFF', 'MISS': '#D3D3D3'},
		'EDM': {'GOAL': '#FF4C00', 'SHOT': '#041E42', 'MISS': '#D3D3D3'},
		'FLA': {'GOAL': '#C8102E', 'SHOT': '#041E42', 'MISS': '#D3D3D3'},
		'LAK': {'GOAL': '#111111', 'SHOT': '#A2AAAD', 'MISS': '#D3D3D3'},
		...
		'OTT': {'GOAL': '#C52032', 'SHOT': '#000000', 'MISS': '#D3D3D3'},
		...
		'TBL': {'GOAL': '#002868', 'SHOT': '#FFFFFF', 'MISS': '#D3D3D3'},
		'TOR': {'GOAL': '#00205B', 'SHOT': '#FFFFFF', 'MISS': '#D3D3D3'},
...
def game_rink_map(data, good_team = None, title = None, strengths = ['all', '5v5', 'PP', 'SH', 'EV', '3v3'],
                        colors = None, save_img = False, default_colors = NHL_COLORS):
    ...
    mask = np.logical_and(data.event.isin(FENWICK), data.shootout != 1)
    ...
    for strength in strengths:
        ...
        for team in teams:
            ...
            for result, color in colors_dict.items():
                ...
                event_df = plot_df[plot_df.event == result]

                if color == '#FFFFFF' or if color == '#A2AAAD':

                    edge_color = colors[team]['GOAL']

                else:

                    edge_color = None

                rink.scatter(event_df.norm_coords_x, event_df.norm_coords_y, c = color, label = result,
                           s = 250, alpha = 0.8, ax = ax, edgecolors = edge_color, is_constrained = True)
            ...
            if team == good_team:
                ...
                ax.legend(loc="lower center", ncol=3, title = f'{team}', fontsize = 14,
                            title_fontsize = 16, facecolor = 'white', framealpha = 1, edgecolor = 'gray')
            if team == bad_team:

                ax.legend(loc="upper center", ncol=3, title = f'{team}', fontsize = 14,
                            title_fontsize = 16, facecolor = 'white', framealpha = 1, edgecolor = 'gray')
        if save_img == True:
            ...
            fig.savefig(f'rink_plots/{good_team}_{bad_team}_{strength}.png', dpi = 750, bbox_inches = 'tight', facecolor = 'white')
