...
def scrape_api_events(game_ids, response_data = None, game_data = pd.DataFrame(), session = None, _print = True):
    ...
    for game_id_idx, game_id in enumerate(game_ids):
        ...
        events_df.event_team_name = events_df.event_team_name.str.replace({'PHOENIX COYOTES': 'ARIZONA COYOTES'})

        events_df.event_team_tri_code = events_df.event_team_tri_code.str.replace({'PHX': 'ARI'})
