...
class SemosPlugin(BasePlugin):
        ...
        for item in img.finditer(html):
            ...
            if src_match:
                ...
                if CONVERT_MODE == 'DEBUG':
                    html = html.replace(item.group(), f"<object data=\"{BPMN_ENDPOINT}/?{page.url}{image_file}\" type=\"image/svg+xml\" id=\"svg\" width=\"100%\" height=\"100%\"></object>")
                else:
    ...
    def on_post_build(self, config):
        ...
        if CONVERT_MODE != 'DEBUG':
            for bpmn in Path(build_dir).rglob('*.bpmn'):
                ...
                subprocess.call([CONVERT_TOOL, bpmn, f"{bpmn}.svg"])
    ...
    def update_database(self, file, new_file, data, db_file):
        ...
        if save:
            subprocess.call([CONVERT_TOOL, file, new_file])
