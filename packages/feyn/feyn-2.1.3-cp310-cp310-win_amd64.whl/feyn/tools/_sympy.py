import math
from feyn._import import assert_imports
...
def sympify_element(elm, symbolic_lr=False, symbolic_cat=True, include_weights=True):
    ...
    if elm.fname == "in:linear":
        ...
        if not symbolic_cat:
            ...
            sym_tuples = [(sympy.Symbol(f"{input_name}_{cat_name}"), weight) for cat_name, weight in cats]
            expr = b
...
@assert_imports
def sympify_model(m, signif=6, symbolic_lr=False, symbolic_cat=True, include_weights=True):
    exprs = [sympify_element(elm, symbolic_lr=symbolic_lr, symbolic_cat=symbolic_cat, include_weights=include_weights) for elm in m]
