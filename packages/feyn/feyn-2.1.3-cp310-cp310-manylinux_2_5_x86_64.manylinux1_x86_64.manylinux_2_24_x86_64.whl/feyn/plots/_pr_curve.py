...
def plot_pr_curve(
    ...
    **kwargs,
    ...
    random_performance = np.sum(np.where(y_true, np.ones(len(y_true)), np.zeros(len(y_true)))) / len(y_true)
    ...
    ax.plot([0, 1], [random_performance, random_performance], "--", c=Theme.color("dark"), lw=1.3)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
