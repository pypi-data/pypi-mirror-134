...
PARADIGMS_FOLDER = IDOCConfiguration().content["folders"]["paradigms"]["path"]
...
class Programmer(Settings, Root):
    ...
    def __init__(
        ...
        self._paradigms_dir = PARADIGMS_FOLDER
        self._barriers = {}
        self.table = []
        self._submodules = {}
    ...
    def list(self, dict_format=True):
        ...
        """
        files = sorted(os.listdir(self._paradigms_dir))
    ...
    def paradigm_path(self, paradigm_path):
        ...
        paradigm_filename = os.path.basename(paradigm_path)
        self._settings['paradigm_path'] = os.path.basename(paradigm_filename)

        if not self._load_table(paradigm_filename):
            raise IDOCException(f'The provided paradigm {paradigm_filename} could not be loaded. See more details below')
    ...
    def _load_table(self, paradigm_path):
        ...
        print(self.absolute_paradigm_path)
