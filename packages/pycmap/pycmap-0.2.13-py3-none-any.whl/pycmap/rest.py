...
class _REST(object):
    ...
    def datasets_with_ancillary(self):
        """
        Returns a dataframe containing the list of data sets with pre-computed colocalized ancillary (mostly environmental) variables.
        A growing number of Simons CMAP datasets are automatically colocalized with a large (100+) number of environmental parameters derived from satellite and numerical model products.
        This method returns the list of data sets that have been automatically colocalized. 
        """        
        return self.query("""
    ...
    def get_dataset_with_ancillary(self, tableName, CIP=False):
        """
        Returns the entire dataset plus pre-computed colocalized ancillary (mostly environmental) variables.
        It is not recommended to retrieve datasets with more than 500k rows using this method.
        Note that this method does not return the dataset metadata. 
        Use the 'get_dataset_metadata' method to get the dataset metadata.
        """
        datasetID = self.get_dataset_ID(tableName)
