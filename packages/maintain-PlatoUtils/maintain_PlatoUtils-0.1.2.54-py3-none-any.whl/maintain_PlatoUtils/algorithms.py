...
def graphEmbedding(gClient,subGraph=[{"head":{"type":"Company","keyAttr":"CompanyName"},
    ...
    if len(subGraph)>0:
        ...
        for triItem in subGraph:
            ...
            hNodeInfoList=[{"nodeName":htItem[0],"nodeType":headType,"nodeVec":[]}  for htItem in totalHtDfList]
            tNodeInfoList=[{"nodeName":htItem[1],"nodeType":tailType,"nodeVec":[]}  for htItem in totalHtDfList]
            nodeInfoList=hNodeInfoList+tNodeInfoList
        ...
        for nodeItem in totalG.node:
            totalG.node[nodeItem]["feature"]=np.random.random([vecSize,])
        if model=="DeepWalk":
            walk_number = 100
            walk_length = 5
            ...
            for nodeInfoI,_ in tqdm.tqdm(enumerate(nodeInfoList)):
                nodeInfoList[nodeInfoI]["nodeVec"]=nodeEmbedDict[nodeInfoList[nodeInfoI]["nodeName"]]
            
            return embedding_model,nodeInfoList
