...
class NodesTest(TestCase):
    ...
    def test_as_string(self):
        ...
        self.assertEqual(
            tree.as_string({"text": u"héhé"}), u'Any X WHERE X has_text "héhé"'
        )
        tree = parse(u"Any X WHERE X has_text %(text)s")
        self.assertEqual(
            tree.as_string({"text": u'hé"hé'}), u'Any X WHERE X has_text "hé\\"hé"'
        )
        tree = parse(u"Any X WHERE X has_text %(text)s")
        self.assertEqual(
            tree.as_string({"text": u"hé\"'hé"}), u'Any X WHERE X has_text "hé\\"\'hé"'
    ...
    def test_as_string_no_encoding(self):
        ...
        tree = parse(u"Any X WHERE X has_text %(text)s")
        self.assertEqual(
            tree.as_string(kwargs={"text": u"héhé"}), u'Any X WHERE X has_text "héhé"'
        )
