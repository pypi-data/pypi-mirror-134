...
class SmartDayTradeForexDynamicSingleSLTFEnv(gym.Env):
    ...
    def step(self,actions):
        ...
        if self.terminal:
            ...
            return_values = (self.data.close - close_values) * lots * 100000
            portfolio_return = sum(return_values)
            ...
            self.ticker_memory.append(self.data["tic"])
            self.sec_types_memory.append(self.data['sec'])
            self.trades_per_stock_memory.append(self.start_trade_per_stock)
            self.exchanges_memory.append(self.data['exchange'])
            self.date_memory_per_stock.append(self.data['date'])
            self.low_values_memory.append(self.data['low'])
            self.close_values_memory.append(self.data['close'])
    ...
    def get_volatality_per_stock(self):
        ...
        for tic in self.ticker_list:
            ...
            max_close = df_tic.loc[-22:,'close'].max()
            low = df_tic.iloc[-1,df_tic.columns.get_loc('low')].item()
            self.volatality_per_stock.append(max_close-low/max_close)
