...
class FeatureStore:
    ...
    def get_history_feature(self, label_table_info: dict,
        ...
        if not is_local:
            ...
            tables_name = map(self.get_table_name,
                              self.spark_session.catalog.listTables(result_table_name.split('.')[0]))
            table_exists = result_table_name.split('.')[1] in tables_name
