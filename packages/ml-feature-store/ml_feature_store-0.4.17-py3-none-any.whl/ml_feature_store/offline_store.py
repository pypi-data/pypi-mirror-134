...
class FeatureStore:
    ...
    def get_history_feature(self, label_table_info: dict,
        ...
        if not is_local:
            ...
            # r = requests.get(submit_job_request)
            req_data = json.dumps({'labelTable': label_table_path, 'lableFieldNames': ','.join(field_names),
                                  'uidName': id_name, 'timestamp': timestamp, 'features': ','.join(feature_refs),
                                  'dss': dts, 'user': user, 'resultTableName': result_table_name,
                                  'resultTableExists': table_exists})
            print(req_data)
            r = requests.post(url=self.feature_query_service_url + '/get_assemble_sql', data=req_data)
            if r.status_code == 200:
                content = json.loads(r.content)
                sqls = str(content['sql'])
                table_name = str(content['resultTableName'])
                print(sqls)
                sql_arr = sqls.strip().split(';')
                length = len(sql_arr) - 1
                i = 0
                while i < length:
                    print(sql_arr[i])
                    self.spark_session.sql(sql_arr[i])
                    i = i + 1
