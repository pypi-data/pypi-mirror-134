...
def datablockpath_selection(driver=None, state=None, roadtype=None, selector=1):
    ...
    if seldriver == '1' or seldriver == 'D1':
        if selstate == '1' or selstate == 'normal':
            if selroadtype == '1' or selroadtype == 'secondary':
                ...
                print("ERROR: No such entry type or misspelled entry type, try to write the entry correctly or remove the '', \n you can also use the entry numbers ")
    ...
    if (selected == '1' or selected == 'directory 1') and directory != None:
        ...
        return directory2
    elif (selected == '3' or selected == 'both') and (directory != None and directory2 != None):
        ...
        print('Note: for Driver 6, the driver have a unique session in the case of normal state and secondary road \n')
...
def datatype_selection(data=''):
    ...
    if datatype == 'GPS' or datatype == '1':
        ...
        print("ERROR: No such data type or misspelling, try to write it correctly or remove the '', \nyou can also use the data type numbers")
...
def path_generator(dataset_dir='', conditions_vector=[], datatype=''):
    ...
    if dataset_dir == '' and conditions_vector == [] and datatype == '':
        ...
        conditionspath = datablockpath_selection(
            conditions_vector[0], conditions_vector[1], conditions_vector[2], conditions_vector[3])
        if conditionspath == '':
            print('\nError in the conditions entry, empty path generated ')
            return path
...
def data_header_selector(selection=''):
    ...
    if header_type == 'GPS' or header_type == '1':
        header_type = 'GPS'
        data_headers = ['Timestamp (seconds)', 'Speed (km/h)', 'Latitude coordinate (degrees)',
                        'Longitude coordinate (degrees)', 'Altitude (meters)', 'Vertical accuracy (degrees)',
                        'Horizontal accuracy (degrees)', 'Course (degrees)', 'Difcourse: course variation (degrees)']
    elif header_type == 'Acceleromter' or header_type == '2':
        header_type = 'Acceleromter'
        data_headers = ['Timestamp (seconds)', 'Boolean of system activated (1 if >50km/h)',
                        'Acceleration in X (Gs)', 'Acceleration in Y (Gs)', 'Acceleration in Z (Gs)', 'Acceleration in X filtered by KF (Gs)', 'Acceleration in Y filtered by KF (Gs)', 'Acceleration in Z filtered by KF (Gs)', 'Roll (degrees)', 'Pitch (degrees)', 'Yaw (degrees)']
    elif header_type == 'lane detection' or header_type == '3':
        header_type = 'lane detection'
        data_headers = ['Timestamp (seconds)', 'X: car position relative to lane center (meters)',
                        'Phi: car angle relative to lane curvature (degrees)', 'W: road width (meters)',
        ...
        data_headers = ['Timestamp (seconds)', 'Distance to ahead vehicle in current lane (meters) [value -1 means no car is detected in front]',
                        'Time of impact to ahead vehicle (seconds) [distance related to own speed]', 'Number of detected vehicles in this frame (traffic)',
                        'GPS speed (km/h) [same as in RAW GPS]']
    elif header_type == 'open street map' or header_type == '5':
        header_type = 'open street map'
        data_headers = ['Timestamp (seconds)', 'Maximum allowed speed of current road (km/h)', 'Reliability of obtained maxspeed (0=unknown,1=reliable, 2=used previously obtained maxspeed,3=estimated by type of road)',
                        'Type of road (motorway, trunk, secondary...)', 'Number of lanes in current road', 'Estimated current lane (1=right lane, 2=first left lane, 3=second left lane, etc) [experimental]',
                        'GPS Latitude used to query OSM (degrees)', 'GPS Longitude used to query OSM (degrees)', 'OSM delay to answer query (seconds)',
                        'GPS speed (km/h) [same as in RAW GPS]']
    elif header_type == 'lane change events' or header_type == '6':
        header_type = 'lane change events'
        data_headers = ['Timestamp (seconds)', 'Type [+ indicates right and - left, 1 indicates normal lane change and 2 slow lane change]',
                        'GPS Latitude of the event (degrees)', 'GPS Longitude of the event (degrees)', 'Duration of the lane change (seconds) [measured since the car position is near the lane marks]',
        ...
        data_headers = ['Timestamp (seconds)', 'Type (1=braking, 2=turning, 3=acceleration)', 'Level (1=low, 2=medium, 3=high)', 'GPS Latitude of the event',
                        'GPS Longitude of the event ', 'Date of the event in YYYYMMDDhhmmss format']
    elif header_type == 'semantics final' or header_type == '8':
        header_type = 'semantics final'
        data_headers = ['Hour of route start', 'Minute of route start', 'Second of route start', 'Average speed during trip (km/h)', 'Maximum achieved speed during route (km/h)',
                        'Lanex score (internal value, related to lane drifting)', 'Driving time (in minutes)', 'Hour of route end', 'Minute of route end', 'Second of route end', 'Trip distance (km)',
                        'ScoreLongDist	(internal value, Score accelerations)', 'ScoreTranDist	(internal value, Score turnings)', 'ScoreSpeedDist (internal value, Score brakings)',
                        'ScoreGlobal (internal value, old score that is not used anymore)', 'Alerts Long (internal value)', 'Alerts Late (internal value)', 'Alerts Lanex (internal value)',
                        'Number of vehicle stops during route (experimental, related to fuel efficiency estimation)', 'Speed variability (experimental, related to fuel efficiency estimation)',
                        'Acceleration noise (experimental, related to fuel efficiency estimation)', 'Kinetic energy (experimental, related to fuel efficiency estimation)',
                        'Driving time (in seconds)', 'Number of curves in the route', 'Power exherted (experimental, related to fuel efficiency estimation)', 'Acceleration events (internal value)',
                        'Braking events (internal value)', 'Turning events (internal value)', 'Longitudinal-distraction Global Score (internal value, combines mean[31] and std[32])',
                        'Transversal-distraction Global Score (internal value, combines mean[33] and std[34])', 'Mean Long.-dist. score (internal value)', 'STD Long.-dist. score (internal value)',
                        'Average Trans.-dist. score (internal value)', 'STD Trans.-dist. score (internal value)', 'Lacc (number of low accelerations)', 'Macc (number of medium accelerations)',
                        'Hacc (number of high accelerations)', 'Lbra (number of low brakings)', 'Mbra (number of medium brakings)', 'Hbra (number of high brakings)', 'Ltur (number of low turnings)',
                        'Mtur (number of medium turnings)', 'Htur (number of high turnings)', 'Score total (base 100, direct mean of the other 7 scores [45-51])', 'Score accelerations (base 100)',
                        'Score brakings (base 100)', 'Score turnings (base 100)', 'Score lane-weaving (base 100)', 'Score lane-drifting (base 100)', 'Score overspeeding (base 100)',
                        'Score car-following (base 100)', 'Ratio normal (base 1)', 'Ratio drowsy (base 1)', 'Ratio aggressive (base 1)']
    elif header_type == 'semantics online' or header_type == '9':
        header_type = 'semantics online'
        data_headers = ['TimeStamp since route start (seconds)', 'GPS Latitude (degrees)', 'GPS Longitude (degrees)', 'Score total WINDOW (base 100, direct mean of the other 7 scores)',
                        'Score accelerations WINDOW (base 100)', 'Score brakings WINDOW (base 100)', 'Score turnings WINDOW (base 100)', 'Score weaving WINDOW (base 100)',
                        'Score drifting WINDOW (base 100)', 'Score overspeeding WINDOW (base 100)', 'Score car-following WINDOW (base 100)', 'Ratio normal WINDOW (base 1)',
                        'Ratio drowsy WINDOW (base 1)', 'Ratio aggressive WINDOW (base 1)', 'Ratio distracted WINDOW (1=distraction detected in last 2 seconds, 0=otherwise)',
                        'Score total (base 100, direct mean of the other 7 scores)', 'Score accelerations (base 100)', 'Score brakings (base 100)', 'Score turnings (base 100)',
                        'Score weaving (base 100)', 'Score drifting (base 100)', 'Score overspeeding (base 100)', 'Score car-following (base 100)', 'Ratio normal (base 1)', 'Ratio drowsy (base 1)',
                        'Ratio aggressive (base 1)', 'Ratio distracted (1=distraction detected in last 2 seconds, 0=otherwise)']
    else:
        data_headers = []
        print("ERROR: No such header type or header misspelled, try to write it correctly or remove the '', \nyou can also use the header numbers")
...
def data_reader(path=''):
    ...
    print('\033[1m', '        Data:', header_type, '\n',
          '******* READ SUCCESSFULLY *******', '\033[0m')
    return data
...
def merge_dataframes(data1=[], data2=[]):
    try:
        while data1.dtypes['Timestamp (seconds)'] != 'int64':
            ...
            data1['Timestamp (seconds)'] = data1['Timestamp (seconds)'].astype(
                'int')
            print('data1 timestamp type changed to : ',
        ...
        while data2.dtypes['Timestamp (seconds)'] != 'int64':
            ...
            data2['Timestamp (seconds)'] = data2['Timestamp (seconds)'].astype(
                'int')
            print('data2 timestamp type changed to : ',
        ...
        print("ERROR: empty data entries or one data entry or both do not have 'Timestamp (seconds)' column, \nplease renter your two dataframes or check their columns before entry ")
...
def removing_incomplete_raws(data=[]):
    try:
        ...
        print(data.count())
        print('is there any missing data values? :',
              '\033[1m', data.isnull().values.any(), '\033[0m')
    except:
        print('ERROR: empty data entry or non dataframe type, please enter your data dataframe')
        print('\nplease import pandas for this function to work')
        print('\nEmpty data returned')
        data = []
    return (data)
...
def access_data(dataloc='', driver=1, state=1, road=1, selector=1, data_type1='', data_type2='', ws1=0, ws2=0):
    path1 = path_generator(
        dataloc, [driver, state, road, selector], data_type1)
    path2 = path_generator(
        dataloc, [driver, state, road, selector], data_type2)
    data1 = data_reader(path1)
    data2 = data_reader(path2)
    if ws1 != 0 and ws2 != 0:
        data = merge_dataframes(data1=window_stepping(
            data1, ws1), data2=window_stepping(data2, ws2))
