...
class Run(object):
    ...
    def get_env_name(self):
        current_env = conf.settings.current_env
        tag_name_list = conf.settings.run.get('tag_name_list', None)
        if tag_name_list is None or current_env not in tag_name_list:
            ...
            return conf.settings.run.tag_name_list[current_env]
                ...
                if name.startswith('test_') is True and name.endswith('.json') is True:
                    ...
                    for i in _data[1:]:
                        ...
                        for tag in list(set(conf.settings.run.test_tags).intersection(tags)):
                            subTags = i.get('subTags', ['default'])
                            is_sub = True
                            if isinstance(subTags, list) is False:
                                subTags = ['default']
                                is_sub = False
                            for subTag in subTags:
                                ...
                                i['tag'] = tag
                                i['subTag'] = subTag
                                i['untag'] = 'uninfo' if tag == 'info' else 'info'
    ...
    def get_env(self):
        _list = []
        _list.append(['环境:', self.get_env_name()])
        tag_name = ';'.join([conf.settings.run.tag_name_list[i] for i in conf.settings.run.test_tags])
        _list.append(['标签:', tag_name])
